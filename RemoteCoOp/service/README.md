...
The panel uses a generated self-signed HTTPS certificate unless `PIXELNOW_REMOTE_COOP_PANEL_CERT` and `PIXELNOW_REMOTE_COOP_PANEL_KEY` are configured. Browsers will warn on first access to a self-signed certificate.

## Login

Use a system username and password. Access is allowed for members of `pixelnow-coop-admin`. If that group does not exist, the panel falls back to local administrator groups.

The installers create `pixelnow-coop-admin` and add the invoking user plus the panel service user to it. Override the login account added by the installer with `LOGIN_USER=your-user`.
...
```sh
PIXELNOW_REMOTE_COOP_PANEL_UPDATE_AUTOMATIC=0
```

## Linux Service Commands

```sh
sudo systemctl status pixelnow-remote-coop-panel
sudo journalctl -u pixelnow-remote-coop-panel -f
sudo systemctl restart pixelnow-remote-coop-panel
```
...
```sh
sudo launchctl print system/com.pixelnow.remote-coop.panel
tail -f /var/log/pixelnow-remote-coop-panel.log
