...
PLIST=/Library/LaunchDaemons/com.pixelnow.remote-coop.panel.plist
...
if [ -z "$ADMIN_GROUP" ]; then
  if dscl . -read "/Groups/$LEGACY_ADMIN_GROUP" >/dev/null 2>&1; then
    ADMIN_GROUP=$LEGACY_ADMIN_GROUP
  else
    ADMIN_GROUP=pixelnow-coop-admin
  fi
fi
...
$SUDO install -o root -g "$ADMIN_GROUP" -m 4750 /tmp/pixelnow-remote-coop-pam-auth-helper "$HELPER"
rm -f /tmp/pixelnow-remote-coop-pam-auth-helper

if [ ! -f /etc/pam.d/pixelnow-remote-coop ]; then
  $SUDO install -o root -g wheel -m 0644 "$REPO_ROOT/RemoteCoOp/panel/auth/pixelnow-remote-coop.macos.pam.example" /etc/pam.d/pixelnow-remote-coop
fi

TMP_PLIST=/tmp/com.pixelnow.remote-coop.panel.plist
sed "s#__REPO_ROOT__#$REPO_ROOT#g; s#__NODE__#$NODE_BIN#g; s#__SERVICE_USER__#$SERVICE_USER#g; s#__PANEL_PORT__#$PANEL_PORT#g; s#__BROKER_PORT__#$BROKER_PORT#g; s#__BROKER_ALTERNATES__#$((BROKER_PORT + 1)),$((BROKER_PORT + 2))#g; s#__TURN_PORT__#$TURN_PORT#g; s#__TURN_MIN_PORT__#$TURN_MIN_PORT#g; s#__TURN_MAX_PORT__#$TURN_MAX_PORT#g" "$REPO_ROOT/RemoteCoOp/service/macos/com.pixelnow.remote-coop.panel.plist" > "$TMP_PLIST"
$SUDO install -o root -g wheel -m 0644 "$TMP_PLIST" "$PLIST"
rm -f "$TMP_PLIST"

if launchctl print system/com.pixelnow.remote-coop.panel >/dev/null 2>&1; then
  $SUDO launchctl bootout system "$PLIST" || true
fi
$SUDO launchctl bootstrap system "$PLIST"
$SUDO launchctl enable system/com.pixelnow.remote-coop.panel
$SUDO launchctl kickstart -k system/com.pixelnow.remote-coop.panel
