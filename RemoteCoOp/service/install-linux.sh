...
SUDO=${SUDO:-sudo}
SERVICE_GROUP=${SERVICE_GROUP:-pixelnow-coop}
ADMIN_GROUP=${ADMIN_GROUP:-pixelnow-coop-admin}
LOGIN_USER=${LOGIN_USER:-${SUDO_USER:-$(id -un)}}
ENV_DIR=/etc/pixelnow
ENV_FILE=$ENV_DIR/remote-coop-panel.env
HELPER=/usr/local/libexec/pixelnow-remote-coop-pam-auth-helper
PANEL_PORT=${PIXELNOW_REMOTE_COOP_PANEL_PORT:-}
BROKER_PORT=${PIXELNOW_REMOTE_COOP_PORT:-}
TURN_PORT=${PIXELNOW_REMOTE_COOP_TURN_PORT:-}
TURN_MIN_PORT=${PIXELNOW_REMOTE_COOP_TURN_MIN_PORT:-}
TURN_MAX_PORT=${PIXELNOW_REMOTE_COOP_TURN_MAX_PORT:-}
...
pam_helper_can_build() {
  ...
  TMP=${TMPDIR:-/tmp}/pixelnow-pam-build-check-$$
...
check_panel_health() {
  ...
  echo "Run: sudo systemctl status pixelnow-remote-coop-panel" >&2
  echo "Run: sudo journalctl -u pixelnow-remote-coop-panel -n 80 --no-pager" >&2
}

env_file_value() {
  KEY=$1
  for FILE in "$ENV_FILE"; do
    if ! $SUDO test -f "$FILE"; then continue; fi
    VALUE=$($SUDO sed -n "s/^$KEY=//p" "$FILE" | tail -n 1)
    if [ -n "$VALUE" ]; then
      echo "$VALUE"
      return 0
    fi
...
select_service_ports() {
  EXISTING_PANEL_PORT=$(env_file_value PIXELNOW_REMOTE_COOP_PANEL_PORT || true)
  EXISTING_BROKER_PORT=$(env_file_value PIXELNOW_REMOTE_COOP_PORT || true)
  EXISTING_TURN_PORT=$(env_file_value PIXELNOW_REMOTE_COOP_TURN_PORT || true)
  EXISTING_TURN_MIN_PORT=$(env_file_value PIXELNOW_REMOTE_COOP_TURN_MIN_PORT || true)
  EXISTING_TURN_MAX_PORT=$(env_file_value PIXELNOW_REMOTE_COOP_TURN_MAX_PORT || true)
...
write_panel_environment() {
  SECRET=$(env_file_value PIXELNOW_REMOTE_COOP_TURN_SHARED_SECRET || node -e 'console.log(require("crypto").randomBytes(48).toString("base64url"))')
  PUBLIC_HOST=$(env_file_value PIXELNOW_REMOTE_COOP_PUBLIC_HOST || echo "198.12.95.48")
  TMP_ENV=${TMPDIR:-/tmp}/pixelnow-remote-coop-panel-env-$$
  if $SUDO test -f "$ENV_FILE"; then
    $SUDO sed '/^PIXELNOW_REMOTE_COOP_PANEL_PORT=/d;/^PIXELNOW_REMOTE_COOP_PANEL_ALLOWED_GROUPS=/d;/^PIXELNOW_REMOTE_COOP_PUBLIC_HOST=/d;/^PIXELNOW_REMOTE_COOP_PORT=/d;/^PIXELNOW_REMOTE_COOP_PORT_ALTERNATES=/d;/^PIXELNOW_REMOTE_COOP_TURN_PORT=/d;/^PIXELNOW_REMOTE_COOP_TURN_TLS_PORT=/d;/^PIXELNOW_REMOTE_COOP_TURN_MIN_PORT=/d;/^PIXELNOW_REMOTE_COOP_TURN_MAX_PORT=/d;/^PIXELNOW_REMOTE_COOP_TURN_SHARED_SECRET=/d;/^PIXELNOW_REMOTE_COOP_AUTOSTART=/d;/^PIXELNOW_REMOTE_COOP_PANEL_UPDATE_AUTOMATIC=/d' "$ENV_FILE" > "$TMP_ENV" || true
  else
    : > "$TMP_ENV"
  fi
  cat >> "$TMP_ENV" <<EOF
PIXELNOW_REMOTE_COOP_PANEL_PORT=$PANEL_PORT
PIXELNOW_REMOTE_COOP_PANEL_ALLOWED_GROUPS=$ADMIN_GROUP
PIXELNOW_REMOTE_COOP_PANEL_UPDATE_AUTOMATIC=1
PIXELNOW_REMOTE_COOP_PUBLIC_HOST=$PUBLIC_HOST
PIXELNOW_REMOTE_COOP_PORT=$BROKER_PORT
PIXELNOW_REMOTE_COOP_PORT_ALTERNATES=$((BROKER_PORT + 1)),$((BROKER_PORT + 2))
PIXELNOW_REMOTE_COOP_TURN_PORT=$TURN_PORT
PIXELNOW_REMOTE_COOP_TURN_TLS_PORT=32443
PIXELNOW_REMOTE_COOP_TURN_MIN_PORT=$TURN_MIN_PORT
PIXELNOW_REMOTE_COOP_TURN_MAX_PORT=$TURN_MAX_PORT
PIXELNOW_REMOTE_COOP_AUTOSTART=1
PIXELNOW_REMOTE_COOP_TURN_SHARED_SECRET=$SECRET
EOF
...
if getent group "$LEGACY_SERVICE_GROUP" >/dev/null 2>&1; then
  $SUDO usermod -a -G "$LEGACY_SERVICE_GROUP" "$SERVICE_USER"
fi
if getent group "$LEGACY_ADMIN_GROUP" >/dev/null 2>&1; then
  if [ -n "$LOGIN_USER" ] && id "$LOGIN_USER" >/dev/null 2>&1; then
    $SUDO usermod -a -G "$LEGACY_ADMIN_GROUP" "$LOGIN_USER"
  fi
  if [ -n "$SERVICE_USER" ] && id "$SERVICE_USER" >/dev/null 2>&1; then
    $SUDO usermod -a -G "$LEGACY_ADMIN_GROUP" "$SERVICE_USER"
  fi
fi
...
"$REPO_ROOT/RemoteCoOp/panel/auth/build-pam-auth-helper.sh" /tmp/pixelnow-remote-coop-pam-auth-helper
$SUDO install -o root -g "$SERVICE_GROUP" -m 4750 /tmp/pixelnow-remote-coop-pam-auth-helper "$HELPER"
rm -f /tmp/pixelnow-remote-coop-pam-auth-helper

if [ ! -f /etc/pam.d/pixelnow-remote-coop ]; then
  $SUDO install -o root -g root -m 0644 "$REPO_ROOT/RemoteCoOp/panel/auth/pixelnow-remote-coop.pam.example" /etc/pam.d/pixelnow-remote-coop
fi

$SUDO sh -c "sed 's#__REPO_ROOT__#$REPO_ROOT#g; s#__NODE__#$NODE_BIN#g; s#__SERVICE_USER__#$SERVICE_USER#g; s#__SERVICE_GROUP__#$SERVICE_GROUP#g' '$REPO_ROOT/RemoteCoOp/service/linux/pixelnow-remote-coop-panel.service' > /etc/systemd/system/pixelnow-remote-coop-panel.service"
$SUDO systemctl daemon-reload
$SUDO systemctl enable pixelnow-remote-coop-panel.service
open_firewall_ports
$SUDO systemctl restart pixelnow-remote-coop-panel.service
check_panel_health

echo "PixelNOW Remote Co-Op panel installed: https://198.12.95.48:$PANEL_PORT/"
