...
function buildConfig() {
  const publicHost = stringEnv("PIXELNOW_REMOTE_COOP_PUBLIC_HOST", "") || stringEnv("PIXELNOW_REMOTE_COOP_TURN_PUBLIC_HOST", "") || productionHost;
  const generatedSecret = !stringEnv("PIXELNOW_REMOTE_COOP_TURN_SHARED_SECRET", "");
  const sharedSecret = generatedSecret ? randomBytes(32).toString("base64url") : stringEnv("PIXELNOW_REMOTE_COOP_TURN_SHARED_SECRET", "");
  const turnPort = integerEnv("PIXELNOW_REMOTE_COOP_TURN_PORT", 32189);
  const turnTLSPort = integerEnv("PIXELNOW_REMOTE_COOP_TURN_TLS_PORT", 32443);
  const turnCertificatePath = stringEnv("PIXELNOW_REMOTE_COOP_TURN_CERT", "");
  const turnKeyPath = stringEnv("PIXELNOW_REMOTE_COOP_TURN_KEY", "");
  const tlsEnabled = Boolean(turnCertificatePath && turnKeyPath);
  const brokerCertificatePath = stringEnv("PIXELNOW_REMOTE_COOP_BROKER_CERT", "") || stringEnv("PIXELNOW_REMOTE_COOP_TLS_CERT", "") || turnCertificatePath;
  const brokerKeyPath = stringEnv("PIXELNOW_REMOTE_COOP_BROKER_KEY", "") || stringEnv("PIXELNOW_REMOTE_COOP_TLS_KEY", "") || turnKeyPath;
  ...
  const env = {
    ...process.env,
    PIXELNOW_REMOTE_COOP_BIND_HOST: brokerBindHost,
    PIXELNOW_REMOTE_COOP_PORT: String(brokerPort),
    PIXELNOW_REMOTE_COOP_PORT_ALTERNATES: brokerPortCandidates.filter(candidate => candidate !== brokerPort).join(","),
    PIXELNOW_REMOTE_COOP_TURN_LISTENING_IP: turnListeningIP,
    PIXELNOW_REMOTE_COOP_TURN_PUBLIC_HOST: stringEnv("PIXELNOW_REMOTE_COOP_TURN_PUBLIC_HOST", publicHost),
    PIXELNOW_REMOTE_COOP_TURN_REALM: stringEnv("PIXELNOW_REMOTE_COOP_TURN_REALM", publicHost),
    PIXELNOW_REMOTE_COOP_TURN_SHARED_SECRET: sharedSecret,
...
PIXELNOW_REMOTE_COOP_TURN_PUBLIC_HOST and PIXELNOW_REMOTE_COOP_TURN_URLS variables
directly.
...
Useful environment:
  ...
  PIXELNOW_REMOTE_COOP_TURN_CERT            Enables turns: URL and broker HTTPS fallback when paired with key
  PIXELNOW_REMOTE_COOP_TURN_KEY             Enables turns: URL and broker HTTPS fallback when paired with cert
  PIXELNOW_REMOTE_COOP_TURNSERVER_BIN       Path/name of coturn turnserver binary

Examples:
  node RemoteCoOp/run-servers.mjs --dry-run
  PIXELNOW_REMOTE_COOP_PUBLIC_HOST=192.168.1.25 node RemoteCoOp/run-servers.mjs
