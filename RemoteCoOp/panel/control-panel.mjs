...
const bindHost = stringEnv("PIXELNOW_REMOTE_COOP_PANEL_BIND_HOST", "0.0.0.0");
const port = integerEnv("PIXELNOW_REMOTE_COOP_PANEL_PORT", 32187);
const sessionTimeoutMs = integerEnv("PIXELNOW_REMOTE_COOP_PANEL_SESSION_TIMEOUT_SECONDS", 3_600) * 1_000;
const loginWindowMs = 5 * 60_000;
const maxLoginFailures = 8;
const childAutostart = booleanEnv("PIXELNOW_REMOTE_COOP_AUTOSTART", true);
const childAutoRestart = booleanEnv("PIXELNOW_REMOTE_COOP_CHILD_AUTORESTART", false);
const automaticUpdates = booleanEnv("PIXELNOW_REMOTE_COOP_PANEL_UPDATE_AUTOMATIC", true);
const updateIntervalMs = Math.max(60, integerEnv("PIXELNOW_REMOTE_COOP_PANEL_UPDATE_INTERVAL_SECONDS", 300)) * 1_000;
const updateBranch = stringEnv("PIXELNOW_REMOTE_COOP_PANEL_UPDATE_BRANCH", "");
const updateValidationCommand = stringEnv("PIXELNOW_REMOTE_COOP_PANEL_UPDATE_VALIDATE", "node RemoteCoOp/run-servers.mjs --dry-run");
const updateAutoRestart = booleanEnv("PIXELNOW_REMOTE_COOP_PANEL_UPDATE_AUTO_RESTART", true);
...
const logs = [];
const maxLogs = integerEnv("PIXELNOW_REMOTE_COOP_PANEL_LOG_LINES", 1_000);
let manager;
...
async function route(request, response) {
  ...
  if (request.method === "POST" && url.pathname === "/admin/api/logout") {
    ...
    setCookie(response, "pixelnow_coop_panel", "", { maxAge: 0 });
    sendJSON(response, 200, { ok: true });
    return;
  }
...
async function handleLogin(request, response) {
  ...
  setCookie(response, "pixelnow_coop_panel", signSessionID(session.id), { maxAge: Math.floor(sessionTimeoutMs / 1_000) });
...
server.listen(port, bindHost, () => {
  appendLog("panel", `PixelNOW Remote Co-Op panel listening on https://${bindHost}:${port}`);
...
function childEnvironment() {
  const publicHost = stringEnv("PIXELNOW_REMOTE_COOP_PUBLIC_HOST", productionHost);
  const env = {
    ...process.env,
    PIXELNOW_REMOTE_COOP_PUBLIC_HOST: publicHost,
    PIXELNOW_REMOTE_COOP_PORT: stringEnv("PIXELNOW_REMOTE_COOP_PORT", "32188"),
    PIXELNOW_REMOTE_COOP_TURN_SHARED_SECRET: stringEnv("PIXELNOW_REMOTE_COOP_TURN_SHARED_SECRET", generatedTurnSecret)
  };
  return env;
}
...
function authenticatedSession(request) {
  ...
  const cookie = cookies.pixelnow_coop_panel;
...
function readOrCreateTLSMaterial() {
  const certPath = stringEnv("PIXELNOW_REMOTE_COOP_PANEL_CERT", join(stateRoot, "panel-cert.pem"));
  const keyPath = stringEnv("PIXELNOW_REMOTE_COOP_PANEL_KEY", join(stateRoot, "panel-key.pem"));
  if (existsSync(certPath) && existsSync(keyPath)) return { certPath, keyPath, generated: false };

  const host = stringEnv("PIXELNOW_REMOTE_COOP_PANEL_TLS_HOST", stringEnv("PIXELNOW_REMOTE_COOP_PUBLIC_HOST", productionHost));
