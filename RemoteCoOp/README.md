# PixelNOW Remote Co-Op Operations
...
- `server/broker.mjs` with `PIXELNOW_REMOTE_COOP_BIND_HOST=198.12.95.48`.
- `turn/turn-server.mjs` with `PIXELNOW_REMOTE_COOP_TURN_LISTENING_IP=198.12.95.48`.

It defaults printed join/TURN URLs to `198.12.95.48`, generates an ephemeral TURN shared secret when one is not provided, and injects matching `PIXELNOW_REMOTE_COOP_TURN_URLS` into the broker.
...
```sh
PIXELNOW_REMOTE_COOP_PUBLIC_HOST=192.168.1.25 \
node RemoteCoOp/run-servers.mjs
```
...
```sh
PIXELNOW_REMOTE_COOP_PUBLIC_HOST=198.12.95.48 \
...
PIXELNOW_REMOTE_COOP_PORT_ALTERNATES=8789,8790
...
PIXELNOW_REMOTE_COOP_TURN_SHARED_SECRET=shared-coturn-rest-secret
PIXELNOW_REMOTE_COOP_TURN_TTL_SECONDS=3600
PIXELNOW_REMOTE_COOP_LOG_NETWORK=1
PIXELNOW_REMOTE_COOP_LOG_MESSAGES=0
```

When `PIXELNOW_REMOTE_COOP_PORT` is unavailable, the broker retries the comma-separated `PIXELNOW_REMOTE_COOP_PORT_ALTERNATES` list. Keep PixelNOW's Remote Co-Op Signaling Server and Guest Join URL settings aligned with the actual broker URL printed at startup.

If broker certificate/key paths are configured, the broker serves HTTPS/WSS. Without them, it serves HTTP/WS.

Static TURN credentials are also supported with `PIXELNOW_REMOTE_COOP_TURN_USERNAME` and `PIXELNOW_REMOTE_COOP_TURN_CREDENTIAL`, but shared-secret REST credentials are preferred for production.
...
```sh
PIXELNOW_REMOTE_COOP_LOG_NETWORK=0 node RemoteCoOp/server/broker.mjs
```
...
```sh
PIXELNOW_REMOTE_COOP_LOG_MESSAGES=1 node RemoteCoOp/server/broker.mjs
```
...
PIXELNOW_REMOTE_COOP_TURN_SHARED_SECRET=pixelnow-remote-coop-local-secret \
...
```sh
PIXELNOW_REMOTE_COOP_TURN_URLS='turn:127.0.0.1:32189?transport=udp,turn:127.0.0.1:32189?transport=tcp' \
PIXELNOW_REMOTE_COOP_TURN_SHARED_SECRET=pixelnow-remote-coop-local-secret \
node RemoteCoOp/server/broker.mjs
```
...
The Node broker can still terminate TLS directly with `PIXELNOW_REMOTE_COOP_BROKER_CERT` and `PIXELNOW_REMOTE_COOP_BROKER_KEY`, or it can keep binding to `127.0.0.1` behind a reverse proxy. The default production path intentionally uses the public IP over HTTP/WS.
...
```sh
PIXELNOW_REMOTE_COOP_TURN_URLS='turn:127.0.0.1:32189?transport=udp' \
PIXELNOW_REMOTE_COOP_TURN_SHARED_SECRET=pixelnow-remote-coop-local-secret \
...
3. Configure PixelNOW Remote Co-Op invites to use the deployed broker URL.
...
- Use `PIXELNOW_REMOTE_COOP_TURN_SHARED_SECRET` with short-lived REST credentials.
- Keep `PIXELNOW_REMOTE_COOP_TURN_DEV_ALLOW_LOOPBACK=1` limited to local development.
