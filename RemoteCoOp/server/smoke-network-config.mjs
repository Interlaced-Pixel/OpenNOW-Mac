...
const verbose = envFlag("PIXELNOW_REMOTE_COOP_SMOKE_VERBOSE", false);
const smokeSecret = environmentValue("PIXELNOW_REMOTE_COOP_TURN_SHARED_SECRET") || "pixelnow-remote-coop-smoke-secret";
const smokeTurnURLs = environmentValue("PIXELNOW_REMOTE_COOP_TURN_URLS") || "turn:127.0.0.1:32189?transport=udp,turn:127.0.0.1:32189?transport=tcp";
const smokeStunURLs = environmentValue("PIXELNOW_REMOTE_COOP_STUN_URLS") || "stun:stun.l.google.com:19302";
const smokeTTLSeconds = environmentValue("PIXELNOW_REMOTE_COOP_TURN_TTL_SECONDS") || "3600";
...
async function startBroker() {
  ...
  brokerProcess = spawn(process.execPath, [brokerScript], {
    ...
    env: {
      ...process.env,
      PIXELNOW_REMOTE_COOP_PORT: String(port),
      PIXELNOW_REMOTE_COOP_BIND_HOST: "127.0.0.1",
      PIXELNOW_REMOTE_COOP_STUN_URLS: smokeStunURLs,
      PIXELNOW_REMOTE_COOP_TURN_URLS: smokeTurnURLs,
      PIXELNOW_REMOTE_COOP_TURN_SHARED_SECRET: smokeSecret,
      PIXELNOW_REMOTE_COOP_TURN_TTL_SECONDS: smokeTTLSeconds,
...
async function assertRelayOnly(brokerURL) {
  ...
  if (!brokerURLArg || environmentValue("PIXELNOW_REMOTE_COOP_TURN_SHARED_SECRET")) {
...
async function assertAutomatic(brokerURL) {
  ...
  if (!brokerURLArg || environmentValue("PIXELNOW_REMOTE_COOP_STUN_URLS")) assert(urls.some(url => url.startsWith("stun:")), "automatic should include STUN URLs");
  if (!brokerURLArg || environmentValue("PIXELNOW_REMOTE_COOP_TURN_URLS")) assert(urls.some(url => url.startsWith("turn:")), "automatic should include TURN URLs when configured");
}
...
async function assertPortFallback() {
  ...
  const child = spawn(process.execPath, [brokerScript], {
    ...
    env: {
      ...process.env,
      PIXELNOW_REMOTE_COOP_PORT: String(blockedPort),
      PIXELNOW_REMOTE_COOP_PORT_ALTERNATES: String(alternatePort),
      PIXELNOW_REMOTE_COOP_BIND_HOST: "127.0.0.1",
      PIXELNOW_REMOTE_COOP_STUN_URLS: smokeStunURLs,
      PIXELNOW_REMOTE_COOP_TURN_URLS: smokeTurnURLs,
      PIXELNOW_REMOTE_COOP_TURN_SHARED_SECRET: smokeSecret,
...
function httpBrokerEnvironment() {
  return {
    PIXELNOW_REMOTE_COOP_BROKER_CERT: "",
    PIXELNOW_REMOTE_COOP_BROKER_KEY: "",
    PIXELNOW_REMOTE_COOP_TLS_CERT: "",
    PIXELNOW_REMOTE_COOP_TLS_KEY: "",
    PIXELNOW_REMOTE_COOP_TURN_CERT: "",
    PIXELNOW_REMOTE_COOP_TURN_KEY: ""
  };
}
...
broker, set PIXELNOW_REMOTE_COOP_TURN_URLS and PIXELNOW_REMOTE_COOP_TURN_SHARED_SECRET
