...
const port = integerEnv("PIXELNOW_REMOTE_COOP_PORT", 32188);
const portAlternates = portCandidates(port, environmentValue("PIXELNOW_REMOTE_COOP_PORT_ALTERNATES"));
const bindHost = stringEnv("PIXELNOW_REMOTE_COOP_BIND_HOST", productionHost);
const root = normalize(join(fileURLToPath(new URL(".", import.meta.url)), "../browser"));
const brokerCertificatePath = stringEnv("PIXELNOW_REMOTE_COOP_BROKER_CERT", "") || stringEnv("PIXELNOW_REMOTE_COOP_TLS_CERT", "") || stringEnv("PIXELNOW_REMOTE_COOP_TURN_CERT", "");
const brokerKeyPath = stringEnv("PIXELNOW_REMOTE_COOP_BROKER_KEY", "") || stringEnv("PIXELNOW_REMOTE_COOP_TLS_KEY", "") || stringEnv("PIXELNOW_REMOTE_COOP_TURN_KEY", "");
...
const stunURLs = splitEnv("PIXELNOW_REMOTE_COOP_STUN_URLS", "stun:stun.l.google.com:19302");
const turnURLs = splitEnv("PIXELNOW_REMOTE_COOP_TURN_URLS", `turn:${productionHost}:32189?transport=udp,turn:${productionHost}:32189?transport=tcp`);
const turnUsername = stringEnv("PIXELNOW_REMOTE_COOP_TURN_USERNAME", "");
const turnCredential = stringEnv("PIXELNOW_REMOTE_COOP_TURN_CREDENTIAL", "");
const turnSharedSecret = stringEnv("PIXELNOW_REMOTE_COOP_TURN_SHARED_SECRET", "");
const turnCredentialTTLSeconds = integerEnv("PIXELNOW_REMOTE_COOP_TURN_TTL_SECONDS", 3600);
const networkLoggingEnabled = booleanEnv("PIXELNOW_REMOTE_COOP_LOG_NETWORK", true);
const messageFlowLoggingEnabled = booleanEnv("PIXELNOW_REMOTE_COOP_LOG_MESSAGES", false);
const rooms = new Map();
...
if (Boolean(brokerCertificatePath) !== Boolean(brokerKeyPath)) {
  console.error("PixelNOW Remote Co-Op broker HTTPS requires both PIXELNOW_REMOTE_COOP_BROKER_CERT and PIXELNOW_REMOTE_COOP_BROKER_KEY, or matching TLS/TURN cert and key environment variables.");
  process.exit(1);
}
...
function listenOnAvailablePort(index) {
  ...
  const onError = error => {
    ...
    if (error.code === "EADDRINUSE" && index + 1 < portAlternates.length) {
      console.warn(`PixelNOW Remote Co-Op broker port ${candidate} is in use; trying ${portAlternates[index + 1]}.`);
      listenOnAvailablePort(index + 1);
      return;
    }
    console.error(`PixelNOW Remote Co-Op broker failed to listen on ${bindHost}:${candidate}: ${error.message}`);
    process.exit(1);
  };
  const onListening = () => {
    ...
    console.log(`PixelNOW Remote Co-Op broker listening on ${brokerHTTPProtocol}://${bindHost}:${actualPort}`);
...
async function makeBrokerServer(handler) {
  ...
  try {
    ...
    console.error(`PixelNOW Remote Co-Op broker failed to load HTTPS certificate/key: ${error.message}`);
    process.exit(1);
  }
}
