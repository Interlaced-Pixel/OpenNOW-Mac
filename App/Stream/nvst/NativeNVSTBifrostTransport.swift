...
public actor NativeNVSTBifrostTransport: NativeNVSTTransport {
    ...
    public func setMaximumBitrateKbps(_ bitrateKbps: UInt32) async throws {
        ...
        try await Self.applyRuntimeNetworkControlOnMainActor(sessionAddress: sessionAddress, fallback: "Native NVST bitrate update failed.") { session, errorBuffer, length in
            NativeNVSTGeronimoSetStreamingMaxBitrate(session, bitrateKbps, errorBuffer, length)
        }
    }
    ...
    public func setDynamicStreamingMode(_ mode: NativeNVSTDynamicStreamingMode) async throws {
        ...
        try await Self.applyRuntimeNetworkControlOnMainActor(sessionAddress: sessionAddress, fallback: "Native NVST dynamic streaming update failed.") { session, errorBuffer, length in
            NativeNVSTGeronimoSetDynamicStreamingMode(session, mode.rawValue, errorBuffer, length)
        }
    }
    ...
    public func setL4SEnabled(_ enabled: Bool) async throws {
        ...
        try await Self.applyRuntimeNetworkControlOnMainActor(sessionAddress: sessionAddress, fallback: "Native NVST L4S update failed.") { session, errorBuffer, length in
            NativeNVSTGeronimoSetL4SState(session, enabled ? 1 : 0, errorBuffer, length)
        }
    }
    ...
    public func updateGamepadTopology(_ topology: NativeNVSTGamepadTopology) async throws {
        ...
        try await Self.applyRuntimeNetworkControlOnMainActor(sessionAddress: sessionAddress, fallback: "Native NVST gamepad topology update failed.") { session, errorBuffer, length in
            NativeNVSTGeronimoUpdateGamepadTopology(session, topology.connectedPlayerBitmap, topology.hapticPlayerBitmap, errorBuffer, length)
        }
    }
    ...
    @MainActor private static func startGeronimoOnMainActor(allocation: NativeNVSTSessionAllocation,
        ...
        let presentationCapabilities = StreamPreferences.loadDeviceCapabilities(screen: streamScreen)
        ...
        guard StreamPreferences.codecSupported(
            StreamCodecOption(label: selectedCodec, value: selectedCodec),
        ...
        let presentationCapability = StreamPreferences.presentationCapability(codec: selectedCodec, capabilities: presentationCapabilities)
        if let nativeWindow {
        ...
        let launchPayload = NativeNVSTLaunchPayload(allocation: allocation, streamingProfileJSON: streamingProfileJSON, clientAppVersion: Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String ?? "PixelNOW")
        ...
        let clientVersion = Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String ?? "PixelNOW"
        ...
        guard let session = frameworksPath.withUnsafeBufferPointer({ pathBuffer in
            NativeNVSTGeronimoCreate(pathBuffer.baseAddress, errorBuffer, 1024)
        }) else {
        ...
        let callbackResult = NativeNVSTGeronimoSetEventHandler(session, nativeNVSTGeronimoEventCallback, eventContext, errorBuffer, 1024)
        ...
        let hapticResult = NativeNVSTGeronimoSetHapticHandler(session, nativeNVSTGeronimoHapticCallback, runtimeHandlers.hapticContext)
        let authRefreshResult = NativeNVSTGeronimoSetAuthRefreshHandler(session, nativeNVSTGeronimoAuthRefreshCallback, runtimeHandlers.authRefreshContext)
        ...
        let surfaceResult = NativeNVSTGeronimoSetVideoSurface(session, nativeVideoSurface, errorBuffer, 1024)
                                    ...
                                    allocation.authTokenType.withCString { authTokenTypePointer in
                                        allocation.authToken.withCString { authTokenPointer in
                                            allocation.rawSessionJSON.withCString { cloudSessionPointer in
                                                NativeNVSTGeronimoResume(session, rawSessionPointer, profilePointer, cloudSessionPointer, languagePointer, versionPointer, localePointer, traceParentPointer, authTokenTypePointer, authTokenPointer, microphoneAvailable ? 1 : 0, microphoneEnabled ? 1 : 0, errorBuffer, 1024)
                                            }
                                        }
                                    }
                                }
    ...
    private static func destroyGeronimoOnBackground(sessionAddress: UInt,
        ...
        let result = await Task.detached(priority: .userInitiated) {
            NativeNVSTGeronimoDestroyWithCompletion(
    ...
    private static func scheduleGeronimoDestructionOnBackground(sessionAddress: UInt,
        ...
        Task.detached(priority: .userInitiated) {
            let result = NativeNVSTGeronimoDestroyWithCompletion(
                UnsafeMutableRawPointer(bitPattern: sessionAddress),
                NativeNVSTDeferredShutdownLease.nativeDestroyCompletion,
                completionContext
            )
    ...
    private static func readMicrophoneStatus(session: UnsafeMutableRawPointer) -> (code: Int32, detail: String?) {
        ...
        let code = errorBuffer.withUnsafeMutableBufferPointer { buffer in
            NativeNVSTGeronimoGetMicrophoneStatus(session, buffer.baseAddress, buffer.count)
        }
    ...
    @MainActor private static func requestPauseGeronimoOnMainActor(sessionAddress: UInt) throws {
        ...
        defer { errorBuffer.deallocate() }
        errorBuffer.initialize(repeating: 0, count: 1024)
        let result = NativeNVSTGeronimoPause(UnsafeMutableRawPointer(bitPattern: sessionAddress), errorBuffer, 1024)
        guard result == 0 else {
    ...
    @MainActor private static func toggleGeronimoPerformanceOverlayOnMainActor(sessionAddress: UInt) throws {
        ...
        defer { errorBuffer.deallocate() }
        errorBuffer.initialize(repeating: 0, count: 1024)
        let result = NativeNVSTGeronimoTogglePerformanceOverlay(UnsafeMutableRawPointer(bitPattern: sessionAddress), errorBuffer, 1024)
        guard result == 0 else {
    ...
    @MainActor private static func setGeronimoMicrophoneEnabledOnMainActor(_ enabled: Bool, sessionAddress: UInt) throws {
        ...
        defer { errorBuffer.deallocate() }
        errorBuffer.initialize(repeating: 0, count: 1024)
        let result = NativeNVSTGeronimoSetMicrophoneEnabled(UnsafeMutableRawPointer(bitPattern: sessionAddress), enabled ? 1 : 0, errorBuffer, 1024)
        guard result == 0 else {
    ...
    @MainActor private static func setGeronimoMicrophoneConfigurationOnMainActor(_ configuration: NativeNVSTMicrophoneConfiguration, session: UnsafeMutableRawPointer?) throws {
        let volumeResult = NativeNVSTGeronimoSetMicrophoneVolume(session, configuration.volume)
        let vadResult = NativeNVSTGeronimoSetVoiceActivityEnabled(session, configuration.voiceActivityEnabled ? 1 : 0)
    ...
    private static func copyGeronimoPerformanceSnapshot(sessionAddress: UInt) -> NativeNVSTPerformanceSnapshot? {
        ...
        let result = nativeStatsBytes.withUnsafeMutableBytes { statsBuffer in
            serverLocation.withUnsafeMutableBufferPointer { locationBuffer in
                NativeNVSTGeronimoCopyPerformanceStats(
                    UnsafeMutableRawPointer(bitPattern: sessionAddress),
    ...
    @MainActor private static func stopGeronimoOnMainActor(sessionAddress: UInt) -> (result: Int32, message: String?) {
        ...
        defer { errorBuffer.deallocate() }
        errorBuffer.initialize(repeating: 0, count: 1024)
        let result = "PixelNOW native NVST disconnect".withCString { reason in
            NativeNVSTGeronimoStopWithResult(UnsafeMutableRawPointer(bitPattern: sessionAddress), reason, 0, errorBuffer, 1024)
        }
    ...
    private static func sendGeronimoInput(sessionAddress: UInt, encoded: NativeNVSTEncodedInputEvent) throws {
        ...
        case .event(let payload):
            result = payload.withUnsafeBytes { buffer in
                NativeNVSTGeronimoSendInput(UnsafeMutableRawPointer(bitPattern: sessionAddress), buffer.bindMemory(to: UInt8.self).baseAddress, payload.count, errorBuffer, 1024)
            }
        case .text(let payload):
            result = payload.withUnsafeBytes { buffer in
                NativeNVSTGeronimoSendText(UnsafeMutableRawPointer(bitPattern: sessionAddress), buffer.bindMemory(to: UInt8.self).baseAddress, payload.count, errorBuffer, 1024)
            }
        }
        guard result == 0 else {
    ...
    private static func sendGeronimoAbsoluteMouse(sessionAddress: UInt, event: NativeNVSTAbsoluteMouseEvent) throws {
        ...
        defer { errorBuffer.deallocate() }
        errorBuffer.initialize(repeating: 0, count: 1024)
        let result = NativeNVSTGeronimoSendAbsoluteMouse(
            UnsafeMutableRawPointer(bitPattern: sessionAddress),
            event.x,
            event.y,
    ...
    static func streamingProfileJSON(rawSessionJSON: String,
                                     ...
                                     settingsJSON: String = "{}",
                                     presentationCapabilities: StreamDeviceCapabilities? = nil) throws -> String {
        ...
        var modeSelection = modeSelectionProfile(
            ...
            presentationCapability: presentationCapabilities.map {
                StreamPreferences.presentationCapability(codec: string(profile["codec"], fallback: "H264"), capabilities: $0)
            }
        )
    ...
    static func geronimoSessionJSON(allocation: NativeNVSTSessionAllocation, streamingProfileJSON: String) throws -> String {
        ...
        let deviceId = firstNonEmpty(string(requestData["deviceHashId"], fallback: ""), string(rawSession["deviceId"], fallback: ""), DeviceIdentity.stableCloudmatchDeviceId())
    ...
    private static func modeSelectionProfile(from profile: [String: Any], dimensions: (width: Int, height: Int), finalizedFeatures: [String: Any], requestedFeatures: [String: Any], settings: [String: Any], presentationCapability: StreamPresentationCapability?) -> [String: Any] {
    ...
    private static func selectedFeatures(from profile: [String: Any], colorQuality: String, finalizedFeatures: [String: Any], requestedFeatures: [String: Any], settings: [String: Any], presentationCapability: StreamPresentationCapability?) -> [String: Any] {
        ...
        return [
            ...
            "mouseMovementFlags": int(featureValue(in: sources, keys: ["mouseMovementFlags"])),
            "qosPolicy": int(featureValue(in: sources, keys: ["qosPolicy"])),
            "touchSupport": bool(featureValue(in: sources, keys: ["touchSupport"])),
    ...
    private static func normalizedNativeStreamingProfile(rawSession: [String: Any], sessionInfo: [String: Any], settings: [String: Any], allocation: NativeNVSTSessionAllocation, streamingProfileJSON: String) -> [String: Any] {
        ...
        if string(profile["streamingProfileGuid"], fallback: "").isEmpty {
            profile["streamingProfileGuid"] = streamingProfileGuid(allocation: allocation, streamingProfileJSON: streamingProfileJSON)
        }
    ...
    private static func streamingProfileGuid(allocation: NativeNVSTSessionAllocation, streamingProfileJSON: String) -> String {
        let signature = "\(allocation.session.applicationID)|\(streamingProfileJSON)"
        let key = "PixelNOW.NativeNVST.StreamingProfileGuid.\(stableProfileHash(signature))"
...
private final class NativeNVSTGeronimoPumpDriver {
    ...
    private func pumpOnce() {
        ...
        let result = errorBuffer.withUnsafeMutableBufferPointer { buffer -> Int32 in
            ...
            return NativeNVSTGeronimoPump(UnsafeMutableRawPointer(bitPattern: sessionAddress), 0, baseAddress, buffer.count)
        }
        let elapsedNanoseconds = DispatchTime.now().uptimeNanoseconds - startedAt
...
public final class NativeNVSTAuthRefreshCoordinator: @unchecked Sendable {
    ...
    public static func productionToken(authType: UInt32) async throws -> String {
        let session = try await AuthService.shared.refreshSession(forceRefresh: true)
    ...
    static func token(from session: AuthSession, authType: UInt32) -> String {
...
@_silgen_name("NativeNVSTGeronimoCreate")
private func NativeNVSTGeronimoCreate(_ frameworksPath: UnsafePointer<CChar>?, _ errorBuffer: UnsafeMutablePointer<CChar>?, _ errorBufferLength: Int) -> UnsafeMutableRawPointer?

@_silgen_name("NativeNVSTGeronimoSetEventHandler")
private func NativeNVSTGeronimoSetEventHandler(_ session: UnsafeMutableRawPointer?, _ eventHandler: NativeNVSTGeronimoEventHandler?, _ eventContext: UnsafeMutableRawPointer?, _ errorBuffer: UnsafeMutablePointer<CChar>?, _ errorBufferLength: Int) -> Int32

@_silgen_name("NativeNVSTGeronimoSetHapticHandler")
private func NativeNVSTGeronimoSetHapticHandler(_ session: UnsafeMutableRawPointer?, _ handler: NativeNVSTGeronimoHapticHandler?, _ context: UnsafeMutableRawPointer?) -> Int32

@_silgen_name("NativeNVSTGeronimoSetAuthRefreshHandler")
private func NativeNVSTGeronimoSetAuthRefreshHandler(_ session: UnsafeMutableRawPointer?, _ handler: NativeNVSTGeronimoAuthRefreshHandler?, _ context: UnsafeMutableRawPointer?) -> Int32

@_silgen_name("NativeNVSTGeronimoSetVideoSurface")
private func NativeNVSTGeronimoSetVideoSurface(_ session: UnsafeMutableRawPointer?, _ nativeHandle: UnsafeMutableRawPointer?, _ errorBuffer: UnsafeMutablePointer<CChar>?, _ errorBufferLength: Int) -> Int32

@_silgen_name("NativeNVSTGeronimoResume")
private func NativeNVSTGeronimoResume(_ session: UnsafeMutableRawPointer?, _ rawSessionJSON: UnsafePointer<CChar>?, _ streamingProfileJSON: UnsafePointer<CChar>?, _ cloudSessionJSON: UnsafePointer<CChar>?, _ gameLanguage: UnsafePointer<CChar>?, _ clientAppVersion: UnsafePointer<CChar>?, _ clientLocale: UnsafePointer<CChar>?, _ traceParent: UnsafePointer<CChar>?, _ authTokenType: UnsafePointer<CChar>?, _ authToken: UnsafePointer<CChar>?, _ microphoneAvailable: Int32, _ microphoneEnabled: Int32, _ errorBuffer: UnsafeMutablePointer<CChar>?, _ errorBufferLength: Int) -> Int32

@_silgen_name("NativeNVSTGeronimoGetMicrophoneStatus")
private func NativeNVSTGeronimoGetMicrophoneStatus(_ session: UnsafeMutableRawPointer?, _ errorBuffer: UnsafeMutablePointer<CChar>?, _ errorBufferLength: Int) -> Int32

@_silgen_name("NativeNVSTGeronimoSetMicrophoneEnabled")
private func NativeNVSTGeronimoSetMicrophoneEnabled(_ session: UnsafeMutableRawPointer?, _ enabled: Int32, _ errorBuffer: UnsafeMutablePointer<CChar>?, _ errorBufferLength: Int) -> Int32

@_silgen_name("NativeNVSTGeronimoSetMicrophoneVolume")
private func NativeNVSTGeronimoSetMicrophoneVolume(_ session: UnsafeMutableRawPointer?, _ volume: Double) -> Int32

@_silgen_name("NativeNVSTGeronimoSetVoiceActivityEnabled")
private func NativeNVSTGeronimoSetVoiceActivityEnabled(_ session: UnsafeMutableRawPointer?, _ enabled: Int32) -> Int32

@_silgen_name("NativeNVSTGeronimoPump")
private func NativeNVSTGeronimoPump(_ session: UnsafeMutableRawPointer?, _ waitTimeoutMilliseconds: Int32, _ errorBuffer: UnsafeMutablePointer<CChar>?, _ errorBufferLength: Int) -> Int32

@_silgen_name("NativeNVSTGeronimoPause")
private func NativeNVSTGeronimoPause(_ session: UnsafeMutableRawPointer?, _ errorBuffer: UnsafeMutablePointer<CChar>?, _ errorBufferLength: Int) -> Int32

@_silgen_name("NativeNVSTGeronimoSendInput")
private func NativeNVSTGeronimoSendInput(_ session: UnsafeMutableRawPointer?, _ inputEventBytes: UnsafePointer<UInt8>?, _ inputEventByteCount: Int, _ errorBuffer: UnsafeMutablePointer<CChar>?, _ errorBufferLength: Int) -> Int32

@_silgen_name("NativeNVSTGeronimoSendAbsoluteMouse")
private func NativeNVSTGeronimoSendAbsoluteMouse(_ session: UnsafeMutableRawPointer?, _ windowX: Int32, _ windowY: Int32, _ timestampNanoseconds: UInt64, _ errorBuffer: UnsafeMutablePointer<CChar>?, _ errorBufferLength: Int) -> Int32

@_silgen_name("NativeNVSTGeronimoTogglePerformanceOverlay")
private func NativeNVSTGeronimoTogglePerformanceOverlay(_ session: UnsafeMutableRawPointer?, _ errorBuffer: UnsafeMutablePointer<CChar>?, _ errorBufferLength: Int) -> Int32

@_silgen_name("NativeNVSTGeronimoSetStreamingMaxBitrate")
private func NativeNVSTGeronimoSetStreamingMaxBitrate(_ session: UnsafeMutableRawPointer?, _ bitrateKbps: UInt32, _ errorBuffer: UnsafeMutablePointer<CChar>?, _ errorBufferLength: Int) -> Int32

@_silgen_name("NativeNVSTGeronimoSetDynamicStreamingMode")
private func NativeNVSTGeronimoSetDynamicStreamingMode(_ session: UnsafeMutableRawPointer?, _ mode: UInt32, _ errorBuffer: UnsafeMutablePointer<CChar>?, _ errorBufferLength: Int) -> Int32

@_silgen_name("NativeNVSTGeronimoSetL4SState")
private func NativeNVSTGeronimoSetL4SState(_ session: UnsafeMutableRawPointer?, _ enabled: Int32, _ errorBuffer: UnsafeMutablePointer<CChar>?, _ errorBufferLength: Int) -> Int32

@_silgen_name("NativeNVSTGeronimoUpdateGamepadTopology")
private func NativeNVSTGeronimoUpdateGamepadTopology(_ session: UnsafeMutableRawPointer?, _ connectedPlayerBitmap: UInt8, _ hapticPlayerBitmap: UInt8, _ errorBuffer: UnsafeMutablePointer<CChar>?, _ errorBufferLength: Int) -> Int32

@_silgen_name("NativeNVSTGeronimoCopyPerformanceStats")
private func NativeNVSTGeronimoCopyPerformanceStats(_ session: UnsafeMutableRawPointer?, _ performanceStatsBytes: UnsafeMutableRawPointer?, _ performanceStatsByteCount: Int, _ serverLocationBuffer: UnsafeMutablePointer<CChar>?, _ serverLocationBufferLength: Int, _ errorBuffer: UnsafeMutablePointer<CChar>?, _ errorBufferLength: Int) -> Int32

@_silgen_name("NativeNVSTGeronimoSendText")
private func NativeNVSTGeronimoSendText(_ session: UnsafeMutableRawPointer?, _ utf8Bytes: UnsafePointer<UInt8>?, _ utf8ByteCount: Int, _ errorBuffer: UnsafeMutablePointer<CChar>?, _ errorBufferLength: Int) -> Int32

@_silgen_name("NativeNVSTGeronimoStopWithResult")
private func NativeNVSTGeronimoStopWithResult(_ session: UnsafeMutableRawPointer?, _ reason: UnsafePointer<CChar>?, _ code: Int32, _ errorBuffer: UnsafeMutablePointer<CChar>?, _ errorBufferLength: Int) -> Int32

@_silgen_name("NativeNVSTGeronimoDestroy")
private func NativeNVSTGeronimoDestroy(_ session: UnsafeMutableRawPointer?) -> Int32

private typealias NativeNVSTGeronimoDestroyCompletion = @convention(c) (UnsafeMutableRawPointer?) -> Void

@_silgen_name("NativeNVSTGeronimoDestroyWithCompletion")
private func NativeNVSTGeronimoDestroyWithCompletion(_ session: UnsafeMutableRawPointer?, _ completion: NativeNVSTGeronimoDestroyCompletion?, _ context: UnsafeMutableRawPointer?) -> Int32
