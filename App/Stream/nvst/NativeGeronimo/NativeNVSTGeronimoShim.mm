...
struct NativeNVSTPerformanceStats {
...
static_assert(offsetof(NativeNVSTPerformanceStats, gameFramesPerSecond) == 0x28, "Native performance stats double alignment changed");
static_assert(sizeof(NativeNVSTPerformanceStats) == 0x50, "Native performance stats ABI changed");
...
using GeronimoEventHandler = void (*)(void *, int32_t, uint32_t, uint32_t, uint32_t, int32_t, const char *, uint32_t, uint32_t, const char *);
using GeronimoHapticHandler = void (*)(void *, uint16_t, uint16_t, uint16_t, uint16_t);
using GeronimoAuthRefreshHandler = void (*)(void *, uint32_t, char *, size_t);
using NativeNVSTGeronimoDestroyCompletion = void (*)(void *);
...
NVbResult_t sendMicAudioFrame(void *client, const char *sessionIdentifier, NvstAudioFrame_t frame);
void *createBifrostClient();
NVbResult_t registerBifrostCallback(void *client, void *context, NVbCallback callback);
bool bifrostCallback(void *gridApp, uint32_t callbackType, void *callbackData);

struct NativeNVSTGeronimoSession {
    ...
    GeronimoEventHandler eventHandler = nullptr;
    void *eventContext = nullptr;
    std::mutex runtimeHandlerMutex;
    GeronimoHapticHandler hapticHandler = nullptr;
    void *hapticContext = nullptr;
    GeronimoAuthRefreshHandler authRefreshHandler = nullptr;
    ...
    NativeNVSTGeronimoDestroyCompletion destroyCompletion = nullptr;
...
std::unordered_map<void *, NativeNVSTGeronimoSession *> gGridAppSessions;
std::mutex gPlatformMutex;
size_t gPlatformReferenceCount = 0;

void setSessionFailure(NativeNVSTGeronimoSession *session, const char *message);
bool setSessionFailureUnlessStopping(NativeNVSTGeronimoSession *session, const char *message);
bool ensureVideoDecoderLocked(NativeNVSTGeronimoSession *session, uint32_t codec);
void waitForMicrophoneRouteDrain(NativeNVSTGeronimoSession *session);
int32_t destroyGeronimoSession(NativeNVSTGeronimoSession *session,
                               bool fromDeferredCleanup,
                               NativeNVSTGeronimoDestroyCompletion completion = nullptr,
                               void *completionContext = nullptr);
extern "C" int32_t NativeNVSTGeronimoDestroy(void *sessionPointer);
...
bool acquireBifrostRegistrationHook(void *geronimoHandle,
    ...
    void *createHook = reinterpret_cast<void *>(&createBifrostClient);
    void *registrationHook = reinterpret_cast<void *>(&registerBifrostCallback);
...
void *createBifrostClient() {
    ...
    NVbResult_t result = originalRegistration(client, context, &bifrostCallback);
    if (result.code != 0) {
        void *expectedRegistration = registrationHook;
        __atomic_compare_exchange_n(gBifrostRegistrationImportSlot, &expectedRegistration, reinterpret_cast<void *>(originalRegistration), false, __ATOMIC_ACQ_REL, __ATOMIC_ACQUIRE);
        return client;
...
bool acquireMicrophoneHook(void *geronimoHandle, void *bifrostHandle, char *errorBuffer, size_t errorBufferLength) {
    ...
    void *hook = reinterpret_cast<void *>(&sendMicAudioFrame);
...
const char *nvbResultName(NativeNVSTGeronimoSession *session, int32_t resultCode) {
...
void emitEvent(NativeNVSTGeronimoSession *session,
               int32_t phase,
    ...
    GeronimoEventHandler handler = nullptr;
...
NativeNVSTGeronimoSession *beginGridAppCallback(void *gridApp) {
    ...
    NativeNVSTGeronimoSession *session = iterator->second;
...
void endGridAppCallback(NativeNVSTGeronimoSession *session) {
    if (session == nullptr) { return; }
...
struct GridAppCallbackLease {
    NativeNVSTGeronimoSession *session = nullptr;
...
void emitHapticRecords(NativeNVSTGeronimoSession *session, const void *callbackData) {
    ...
    for (size_t index = 0; index < recordCount; ++index) {
        ...
        GeronimoHapticHandler handler = nullptr;
        void *context = nullptr;
        {
...
bool bifrostCallback(void *gridApp, uint32_t callbackType, void *callbackData) {
    NativeNVSTGeronimoSession *session = beginGridAppCallback(gridApp);
...
void gridAppUpdateAuthToken(void *gridApp, void *updateAuthToken) {
    NativeNVSTGeronimoSession *session = beginGridAppCallback(gridApp);
    if (session == nullptr) { return; }
    GridAppCallbackLease callbackLease{session};
...
void emitConnectedIfReady(NativeNVSTGeronimoSession *session) {
...
void gridAppPrepareResult(void *gridApp, const NVbResult_t *result, void *) {
    NativeNVSTGeronimoSession *session = beginGridAppCallback(gridApp);
    if (session == nullptr) { return; }
    GridAppCallbackLease callbackLease{session};
    try {
        ...
        try {
            ...
            fprintf(stderr, "PixelNOW failed to contain a GridApp prepare callback exception.\n");
        }
    }
}

void gridAppStreamingBegin(void *gridApp, const void *streamInfo) {
    NativeNVSTGeronimoSession *session = beginGridAppCallback(gridApp);
    if (session == nullptr) { return; }
    GridAppCallbackLease callbackLease{session};
    try {
        ...
        try {
            ...
            fprintf(stderr, "PixelNOW failed to contain a GridApp streaming-begin callback exception.\n");
        }
    }
}

void gridAppSetupFailure(void *gridApp, const void *failureInfo) {
    NativeNVSTGeronimoSession *session = beginGridAppCallback(gridApp);
    if (session == nullptr) { return; }
    GridAppCallbackLease callbackLease{session};
...
void gridAppResumeFailure(void *gridApp, const void *failureInfo) {
    NativeNVSTGeronimoSession *session = beginGridAppCallback(gridApp);
    if (session == nullptr) { return; }
    GridAppCallbackLease callbackLease{session};
    int32_t resultCode = failureInfo == nullptr ? -1 : loadUnaligned<int32_t>(failureInfo, 0);
...
void gridAppSetupSuccess(void *gridApp, const void *sessionInfo) {
    NativeNVSTGeronimoSession *session = beginGridAppCallback(gridApp);
    if (session == nullptr) { return; }
    GridAppCallbackLease callbackLease{session};
...
void gridAppStreamingTerminated(void *gridApp, const void *terminationInfo) {
    NativeNVSTGeronimoSession *session = beginGridAppCallback(gridApp);
    if (session == nullptr) { return; }
    GridAppCallbackLease callbackLease{session};
...
void *nativeSDLWindow(NativeNVSTGeronimoSession *session) {
...
void reconcileHostPointerCaptureAfterCursorUpdate(NativeNVSTGeronimoSession *session, uint32_t cursorState) {
...
void gridAppCursorInfoUpdate(void *gridApp, const void *cursorInfo) {
    NativeNVSTGeronimoSession *session = beginGridAppCallback(gridApp);
    if (session == nullptr) { return; }
    GridAppCallbackLease callbackLease{session};
    try {
...
void gridAppSetupProgress(void *gridApp, const void *parameters) {
    NativeNVSTGeronimoSession *session = beginGridAppCallback(gridApp);
    if (session == nullptr) { return; }
    GridAppCallbackLease callbackLease{session};
...
void gridAppActiveSessions(void *gridApp, const void *result) {
    NativeNVSTGeronimoSession *session = beginGridAppCallback(gridApp);
    if (session == nullptr) { return; }
    GridAppCallbackLease callbackLease{session};
...
void gridAppStopResult(void *gridApp, const void *failureInfo) {
    NativeNVSTGeronimoSession *session = beginGridAppCallback(gridApp);
    if (session == nullptr) { return; }
    GridAppCallbackLease callbackLease{session};
    int32_t resultCode = failureInfo == nullptr ? -1 : loadUnaligned<int32_t>(failureInfo, 0);
...
void gridAppPauseResult(void *gridApp, const void *failureInfo) {
    NativeNVSTGeronimoSession *session = beginGridAppCallback(gridApp);
    if (session == nullptr) { return; }
    GridAppCallbackLease callbackLease{session};
    int32_t resultCode = failureInfo == nullptr ? -1 : loadUnaligned<int32_t>(failureInfo, 0);
...
void stopAndDestroyPolymorphicObject(void *&object, size_t stopSlot, const char *label) {
    ...
    try {
        ...
        fprintf(stderr, "PixelNOW failed to stop native %s.\n", label == nullptr ? "media object" : label);
    }
    try {
        destroyPolymorphicObject(object);
    } catch (...) {
        fprintf(stderr, "PixelNOW failed to destroy native %s.\n", label == nullptr ? "media object" : label);
        object = nullptr;
    }
}
...
bool installGridAppCallbacks(NativeNVSTGeronimoSession *session, char *errorBuffer, size_t errorBufferLength) {
    ...
    addressPoint[GridAppPrepareResultSlotOffset / sizeof(void *)] = reinterpret_cast<void *>(&gridAppPrepareResult);
    addressPoint[GridAppStreamingBeginSlotOffset / sizeof(void *)] = reinterpret_cast<void *>(&gridAppStreamingBegin);
    addressPoint[GridAppSetupFailureSlotOffset / sizeof(void *)] = reinterpret_cast<void *>(&gridAppSetupFailure);
    addressPoint[GridAppResumeFailureSlotOffset / sizeof(void *)] = reinterpret_cast<void *>(&gridAppResumeFailure);
    addressPoint[GridAppSetupSuccessSlotOffset / sizeof(void *)] = reinterpret_cast<void *>(&gridAppSetupSuccess);
    addressPoint[GridAppStreamingTerminatedSlotOffset / sizeof(void *)] = reinterpret_cast<void *>(&gridAppStreamingTerminated);
    addressPoint[GridAppUpdateAuthTokenSlotOffset / sizeof(void *)] = reinterpret_cast<void *>(&gridAppUpdateAuthToken);
    addressPoint[GridAppCursorInfoSlotOffset / sizeof(void *)] = reinterpret_cast<void *>(&gridAppCursorInfoUpdate);
    addressPoint[GridAppSetupProgressSlotOffset / sizeof(void *)] = reinterpret_cast<void *>(&gridAppSetupProgress);
    addressPoint[GridAppActiveSessionsSlotOffset / sizeof(void *)] = reinterpret_cast<void *>(&gridAppActiveSessions);
    addressPoint[GridAppStopResultSlotOffset / sizeof(void *)] = reinterpret_cast<void *>(&gridAppStopResult);
    addressPoint[GridAppPauseResultSlotOffset / sizeof(void *)] = reinterpret_cast<void *>(&gridAppPauseResult);
...
bool detachGridAppCallbacks(NativeNVSTGeronimoSession *session) {
    ...
    try {
        ...
        fprintf(stderr, "PixelNOW could not schedule deferred native callback drainage.\n");
    }
    fprintf(stderr, "PixelNOW native callback drainage exceeded 3 seconds; native session destruction was deferred.\n");
    return false;
}
...
bool waitForNativeStopAcknowledgement(NativeNVSTGeronimoSession *session, std::chrono::seconds timeout) {
...
bool scheduleDeferredNativeDestruction(NativeNVSTGeronimoSession *session) {
    bool expected = false;
    if (!session->deferredDestroyScheduled.compare_exchange_strong(expected, true, std::memory_order_acq_rel)) {
        return true;
    }
    try {
        ...
        session->deferredDestroyScheduled.store(false, std::memory_order_release);
        fprintf(stderr, "PixelNOW could not schedule deferred native session destruction.\n");
        return false;
    }
}
...
bool setupPlatformMedia(NativeNVSTGeronimoSession *session) {
    std::lock_guard<std::mutex> mediaLock(session->mediaMutex);
    ...
    storeUnaligned<const char *>(windowParameters.bytes, 0x28, "PixelNOW");
    ...
    if (!session->functions.windowInitialize(session->window, session->ioInterface, windowParameters)) {
        setSessionFailure(session, "SDLWindow initialization failed for the PixelNOW native surface.");
        return false;
    }
...
void teardownPlatformMedia(NativeNVSTGeronimoSession *session) {
    ...
    if (capturer != nullptr) {
        ...
        try {
            ...
            try { shutdownCapturer(); } catch (...) {
                fprintf(stderr, "PixelNOW audio capturer fallback destruction failed.\n");
            }
        }
    }
...
int32_t completePreparedStart(NativeNVSTGeronimoSession *session) {
...
extern "C" int32_t NativeNVSTGeronimoInspectEndpoint(const char *address,
...
extern "C" void *NativeNVSTGeronimoCreate(const char *frameworksPath, char *errorBuffer, size_t errorBufferLength) {
    void *handle = nullptr;
    void *bifrostHandle = nullptr;
    void *gridApp = nullptr;
    ...
    try {
        ...
        session = new (std::nothrow) NativeNVSTGeronimoSession();
        if (session == nullptr) {
...
extern "C" const char *NativeNVSTGeronimoResultCodeName(void *sessionPointer, int32_t resultCode) {
    return nvbResultName(static_cast<NativeNVSTGeronimoSession *>(sessionPointer), resultCode);
}

extern "C" int32_t NativeNVSTGeronimoSetEventHandler(void *sessionPointer,
                                                              GeronimoEventHandler eventHandler,
                                                              void *eventContext,
                                                              char *errorBuffer,
                                                              size_t errorBufferLength) {
...
extern "C" int32_t NativeNVSTGeronimoSetHapticHandler(void *sessionPointer,
                                                                GeronimoHapticHandler handler,
                                                                void *context) {
    auto *session = static_cast<NativeNVSTGeronimoSession *>(sessionPointer);
...
extern "C" uint32_t NativeNVSTGeronimoVideoDecoderCreationCodec(uint32_t requestedCodec) {
    return videoDecoderCreationCodec(requestedCodec);
}

extern "C" int32_t NativeNVSTGeronimoAudioFrameTriggersRendererReopen(uint32_t configuredChannelCount,
                                                                                uint32_t incomingChannelCount) {
    return audioFrameTriggersRendererReopen(configuredChannelCount, incomingChannelCount) ? 1 : 0;
}

extern "C" int32_t NativeNVSTGeronimoSetAuthRefreshHandler(void *sessionPointer,
                                                                    GeronimoAuthRefreshHandler handler,
                                                                    void *context) {
    auto *session = static_cast<NativeNVSTGeronimoSession *>(sessionPointer);
    if (session == nullptr) { return -1; }
    std::lock_guard<std::mutex> lock(session->runtimeHandlerMutex);
...
extern "C" int32_t NativeNVSTGeronimoDecodeHapticCallbackData(const uint8_t *callbackData,
    ...
    NativeNVSTGeronimoSession session;
    session.hapticHandler = handler;
    session.hapticContext = context;
...
extern "C" int32_t NativeNVSTGeronimoUpdateGamepadTopology(void *sessionPointer,
...
extern "C" int32_t NativeNVSTGeronimoSetVideoSurface(void *sessionPointer,
                                                              void *nativeHandle,
                                                              char *errorBuffer,
                                                              size_t errorBufferLength) {
    auto *session = static_cast<NativeNVSTGeronimoSession *>(sessionPointer);
...
extern "C" int32_t NativeNVSTGeronimoSetMicrophoneEnabled(void *sessionPointer,
                                                                    int32_t enabled,
                                                                    char *errorBuffer,
                                                                    size_t errorBufferLength) {
    auto *session = static_cast<NativeNVSTGeronimoSession *>(sessionPointer);
...
extern "C" int32_t NativeNVSTGeronimoGetMicrophoneStatus(void *sessionPointer,
                                                                  char *errorBuffer,
                                                                  size_t errorBufferLength) {
    auto *session = static_cast<NativeNVSTGeronimoSession *>(sessionPointer);
    if (session == nullptr) {
...
extern "C" int32_t NativeNVSTGeronimoSetMicrophoneVolume(void *sessionPointer, double volume) {
    auto *session = static_cast<NativeNVSTGeronimoSession *>(sessionPointer);
...
extern "C" int32_t NativeNVSTGeronimoSetVoiceActivityEnabled(void *sessionPointer, int32_t enabled) {
    auto *session = static_cast<NativeNVSTGeronimoSession *>(sessionPointer);
...
extern "C" size_t NativeNVSTGeronimoTestVoiceActivityStateSize() {
    return sizeof(AdaptiveVoiceActivityState);
}

extern "C" int32_t NativeNVSTGeronimoTestResetVoiceActivityState(void *stateBytes, size_t stateByteCount) {
...
extern "C" int32_t NativeNVSTGeronimoTestProcessMicrophonePCM(int16_t *samples,
                                                                        size_t sampleCount,
                                                                        double volume,
                                                                        int32_t vadEnabled,
                                                                        void *stateBytes,
...
extern "C" int32_t NativeNVSTGeronimoTestMicrophoneFrameSupported(uint32_t sampleRate,
...
extern "C" int32_t NativeNVSTGeronimoTestProcessMicrophoneFrame(void *frameBytes,
...
extern "C" void *NativeNVSTGeronimoTestRegisterMicrophoneRoute(void *client) {
    return registerMicrophoneRoute(client);
}

extern "C" void NativeNVSTGeronimoTestUnregisterMicrophoneRoute(void *route) {
    unregisterMicrophoneRoute(static_cast<MicrophoneRouteSlot *>(route));
}

extern "C" size_t NativeNVSTGeronimoTestMicrophoneRouteCount() {
    size_t count = 0;
    for (MicrophoneRouteSlot &slot : gMicrophoneRoutes) {
...
extern "C" int32_t NativeNVSTGeronimoTestCallbackLeaseDrainage() {
    NativeNVSTGeronimoSession session;
    ...
    try {
        ...
        for (size_t index = 0; index < workerCount; ++index) {
            workers.emplace_back([gridApp = session.gridApp, index] {
                if (index % 2 == 0) {
                    bifrostCallback(gridApp, NVbCallbackTypeEvent, nullptr);
                } else {
                    gridAppUpdateAuthToken(gridApp, nullptr);
                }
            });
        }
    } catch (...) {
...
extern "C" int32_t NativeNVSTGeronimoTestMicrophoneRouteDrainage() {
    int clientMarker = 0;
...
int32_t startOrResumeGeronimo(void *sessionPointer,
    ...
    application.clientAppVersion = stringOrEmpty(clientAppVersion).empty() ? "PixelNOW" : stringOrEmpty(clientAppVersion);
    ...
    session->clientAppVersion = stringOrEmpty(clientAppVersion).empty() ? "PixelNOW" : stringOrEmpty(clientAppVersion);
...
extern "C" int32_t NativeNVSTGeronimoStart(void *sessionPointer,
                                                     const char *rawSessionJSON,
                                                     const char *streamingProfileJSON,
                                                     const char *cloudSessionJSON,
                                                     const char *gameLanguage,
...
extern "C" int32_t NativeNVSTGeronimoConvertServerType(int32_t serverType) {
    return convertedServerType(serverType);
}

extern "C" int32_t NativeNVSTGeronimoConvertAuthTokenType(const char *tokenType) {
    if (tokenType == nullptr) { return -1; }
    uint32_t authType = 0;
    return authTypeForTokenType(tokenType, authType) ? static_cast<int32_t>(authType) : -1;
}

extern "C" int32_t NativeNVSTGeronimoInspectMetadata(const char *geronimoJSON,
                                                               uint32_t index,
                                                               char *keyBuffer,
...
extern "C" int32_t NativeNVSTGeronimoResume(void *sessionPointer,
                                                      const char *rawSessionJSON,
                                                      const char *streamingProfileJSON,
                                                      const char *cloudSessionJSON,
                                                      const char *gameLanguage,
...
extern "C" int32_t NativeNVSTGeronimoPump(void *sessionPointer,
...
extern "C" int32_t NativeNVSTGeronimoPause(void *sessionPointer, char *errorBuffer, size_t errorBufferLength) {
    auto *session = static_cast<NativeNVSTGeronimoSession *>(sessionPointer);
...
extern "C" int32_t NativeNVSTGeronimoSendInput(void *sessionPointer,
...
extern "C" int32_t NativeNVSTGeronimoSendAbsoluteMouse(void *sessionPointer,
                                                                  int32_t windowX,
                                                                  int32_t windowY,
...
extern "C" int32_t NativeNVSTGeronimoTogglePerformanceOverlay(void *sessionPointer,
                                                                         char *errorBuffer,
                                                                         size_t errorBufferLength) {
    auto *session = static_cast<NativeNVSTGeronimoSession *>(sessionPointer);
...
extern "C" int32_t NativeNVSTGeronimoSetStreamingMaxBitrate(void *sessionPointer,
...
extern "C" int32_t NativeNVSTGeronimoSetDynamicStreamingMode(void *sessionPointer,
                                                                        uint32_t mode,
                                                                        char *errorBuffer,
                                                                        size_t errorBufferLength) {
    if (mode > 3) {
...
extern "C" int32_t NativeNVSTGeronimoSetL4SState(void *sessionPointer,
                                                            int32_t enabled,
                                                            char *errorBuffer,
                                                            size_t errorBufferLength) {
    auto *session = static_cast<NativeNVSTGeronimoSession *>(sessionPointer);
...
extern "C" int32_t NativeNVSTGeronimoCopyPerformanceStats(void *sessionPointer,
                                                                    ...
                                                                    size_t errorBufferLength) {
    auto *session = static_cast<NativeNVSTGeronimoSession *>(sessionPointer);
    if (performanceStatsBytes == nullptr || performanceStatsByteCount != sizeof(NativeNVSTPerformanceStats)) {
        if (errorBuffer != nullptr && errorBufferLength > 0) {
            snprintf(errorBuffer, errorBufferLength, "Native Geronimo performance stats require %zu output bytes.", sizeof(NativeNVSTPerformanceStats));
        }
        return -1;
    }
    ...
    try {
        ...
        NativeNVSTPerformanceStats performanceStats;
...
extern "C" int32_t NativeNVSTGeronimoSendText(void *sessionPointer,
...
extern "C" int32_t NativeNVSTGeronimoStopWithResult(void *sessionPointer,
                                                              const char *reason,
                                                              int32_t code,
                                                              char *errorBuffer,
                                                              size_t errorBufferLength) {
    ...
    try {
        if (session->gridApp == nullptr || session->functions.stop == nullptr ||
            !session->functions.stop(session->gridApp, reason == nullptr ? "PixelNOW native NVST stop" : reason, code)) {
            {
                std::lock_guard<std::mutex> stateLock(session->stateMutex);
                session->stopAcknowledged = true;
            }
...
extern "C" void NativeNVSTGeronimoStop(void *sessionPointer) {
    NativeNVSTGeronimoStopWithResult(sessionPointer, "PixelNOW native NVST stop", 0, nullptr, 0);
}
...
int32_t destroyGeronimoSession(NativeNVSTGeronimoSession *session,
                               bool fromDeferredCleanup,
                               NativeNVSTGeronimoDestroyCompletion completion,
    ...
    if (session->hapticFeatureEnabled && session->functions.controlFeatures != nullptr) {
        ...
        catch (...) { fprintf(stderr, "PixelNOW failed to disable native haptics during teardown.\n"); }
    }
    session->hapticFeatureEnabled = false;
    for (uint8_t sourceIndex = 0; sourceIndex < 4; ++sourceIndex) {
        if (session->registeredGamepads[sourceIndex] && session->functions.handleGamepadChanged != nullptr) {
            ...
            catch (...) { fprintf(stderr, "PixelNOW failed to disconnect a native gamepad during teardown.\n"); }
        }
    ...
    if (shouldForceStop) {
        ...
        try {
            if (!session->functions.stop(session->gridApp, "PixelNOW native NVST forced destroy", 0)) {
                std::lock_guard<std::mutex> stateLock(session->stateMutex);
                session->state = NativeSessionState::failed;
            ...
            session->stopAcknowledged = true;
            session->stopCompleted.notify_all();
            fprintf(stderr, "PixelNOW forced native stop raised an unexpected C++ exception.\n");
        }
        operationLock.lock();
    }
    if (!waitForNativeStopAcknowledgement(session, std::chrono::seconds(3))) {
        if (scheduleDeferredNativeDestruction(session)) {
            fprintf(stderr, "PixelNOW native stop acknowledgement exceeded 3 seconds; native session destruction was deferred.\n");
        }
        operationLock.unlock();
        return 1;
    }
    if (!unregisterMicrophoneRoute(session->microphoneRoute)) {
        if (scheduleDeferredNativeDestruction(session)) {
            fprintf(stderr, "PixelNOW native microphone callback drainage exceeded 3 seconds; native session destruction was deferred.\n");
        }
        operationLock.unlock();
        return 1;
    }
    ...
    catch (...) { fprintf(stderr, "PixelNOW native media destruction raised an unexpected C++ exception.\n"); }
    if (session->microphoneRoute != nullptr) {
        unregisterMicrophoneRoute(session->microphoneRoute);
        session->microphoneRoute = nullptr;
    }
    ...
    if (session->gridApp != nullptr) {
        if (session->initialized && session->functions.gridAppDtor != nullptr) {
            try {
                ...
                fprintf(stderr, "PixelNOW GridApp destruction raised an unexpected C++ exception.\n");
            }
        }
    ...
    if (session->platformStarted && session->platformShutdown != nullptr) {
        try {
            ...
            fprintf(stderr, "PixelNOW Geronimo platform shutdown raised an unexpected C++ exception.\n");
        }
...
extern "C" int32_t NativeNVSTGeronimoDestroy(void *sessionPointer) {
    return destroyGeronimoSession(static_cast<NativeNVSTGeronimoSession *>(sessionPointer), false, nullptr, nullptr);
}

extern "C" int32_t NativeNVSTGeronimoDestroyWithCompletion(void *sessionPointer,
                                                                  NativeNVSTGeronimoDestroyCompletion completion,
                                                                  void *completionContext) {
    return destroyGeronimoSession(
        static_cast<NativeNVSTGeronimoSession *>(sessionPointer),
        false,
        completion,
        completionContext
    );
