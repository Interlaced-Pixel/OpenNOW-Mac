...
struct NativeNVSTLaunchPayload: Equatable, Sendable {
    ...
    init(allocation: NativeNVSTSessionAllocation, streamingProfileJSON: String, clientAppVersion: String) {
        ...
        let deviceId = Self.firstNonEmpty(Self.string(requestData["deviceHashId"]), Self.string(rawSession["deviceId"]), DeviceIdentity.stableCloudmatchDeviceId())
        let locale = Self.firstNonEmpty(Self.string(settings["gameLanguage"]), Self.string(rawSession["locale"]), GFNLocale.currentGFNLocale())
