...
@objc(LibWebRTCSessionImpl)
final class LibWebRTCSessionImpl: NSObject, RTCPeerConnectionDelegate, RTCDataChannelDelegate, @unchecked Sendable {
    weak var owner: LibWebRTCStreamSession?
    @objc var factory: RTCPeerConnectionFactory?
    @objc var audioDevice: CoreAudioRTCDevice?
    ...
    @MainActor private func attachRemoteVideoTrack(_ videoTrack: RTCVideoTrack) {
        ...
        let metalView = MetalVideoView(frame: parentView.bounds, targetFps: targetFps, owner: owner)
        let videoView: NSView = metalView
        let videoRenderer: RTCVideoRenderer = metalView
        owner.setVideoRendererState(sink: "MetalVideoView", pipelineMode: "libwebrtc Metal display")
