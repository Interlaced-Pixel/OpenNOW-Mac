...
extension Font {
    static func streamNvidia(size: CGFloat, weight: NVIDIAFont.Weight = .regular) -> Font {
...
public struct WebRTCMediaStreamSurface: View {
    ...
    @State private var latestStats: StreamStatsSnapshot?
    ...
    @State private var remoteCoOpHostSession = RemoteCoOpHostSession()
    @State private var remoteCoOpHostCoordinator: RemoteCoOpHostCoordinator?
    @State private var remoteCoOpSignalingSession: (any RemoteCoOpSignalingSession)?
    @State private var remoteCoOpPeerController: RemoteCoOpHostPeerController?
    @State private var remoteCoOpVideoRelay = RemoteCoOpHostVideoRelay()
    @State private var remoteCoOpAudioRelay = RemoteCoOpHostAudioRelay()
    @State private var remoteCoOpListenTask: Task<Void, Never>?
    @State private var remoteCoOpSnapshot = RemoteCoOpHostSnapshot(preferences: RemoteCoOpPreferencesStore.load(), invite: nil, participants: [])
    @State private var remoteCoOpNetworkConfiguration = RemoteCoOpNetworkConfiguration(transportMode: RemoteCoOpPreferencesStore.load().transportMode, latencyMode: RemoteCoOpPreferencesStore.load().latencyMode)
    ...
    private func refreshRemoteCoOpState() {
        ...
        remoteCoOpNetworkConfiguration = RemoteCoOpNetworkConfiguration(transportMode: preferences.transportMode, latencyMode: preferences.latencyMode)
        remoteCoOpSnapshot = RemoteCoOpHostSnapshot(preferences: preferences, invite: remoteCoOpSnapshot.invite, participants: remoteCoOpSnapshot.participants)
        Task { @MainActor in
    ...
    private func copyRemoteCoOpInvite(_ invite: RemoteCoOpInvite) {
        let pasteboard = NSPasteboard.general
        pasteboard.clearContents()
        pasteboard.setString(remoteCoOpClipboardText(invite), forType: .string)
    }

    private func remoteCoOpClipboardText(_ invite: RemoteCoOpInvite) -> String {
    ...
    private var remoteCoOpLaunchPreferences: RemoteCoOpPreferences {
        RemoteCoOpPreferences.launchPreferences(from: configuration.metadata, fallback: RemoteCoOpPreferencesStore.load())
    }

    private func makeRemoteCoOpCoordinator(preferences: RemoteCoOpPreferences) -> RemoteCoOpHostCoordinator {
        if let remoteCoOpHostCoordinator {
        ...
        let coordinator = RemoteCoOpHostCoordinator(hostSession: remoteCoOpHostSession, signaling: signaling)
        remoteCoOpNetworkConfiguration = RemoteCoOpNetworkConfiguration(transportMode: preferences.transportMode, latencyMode: preferences.latencyMode)
    ...
    private func makeRemoteCoOpPeerController(signaling: any RemoteCoOpSignalingSession, coordinator: RemoteCoOpHostCoordinator) -> RemoteCoOpHostPeerController {
        let inputTransport = transport
        return RemoteCoOpHostPeerController(
            signaling: signaling,
    ...
    private func makeRemoteCoOpSignalingSession(preferences: RemoteCoOpPreferences) -> any RemoteCoOpSignalingSession {
        if let serverURL = URL(string: preferences.signalingServerURL.trimmingCharacters(in: .whitespacesAndNewlines)), serverURL.scheme?.hasPrefix("ws") == true {
            return RemoteCoOpWebSocketSignalingSession(serverURL: serverURL)
        }
        return InProcessRemoteCoOpSignalingSession()
    }

    private func remoteCoOpJoinBaseURL(_ preferences: RemoteCoOpPreferences) -> URL? {
    ...
    private func beginStreamingPerformanceMode() {
        ...
        streamingPerformanceActivity = ProcessInfo.processInfo.beginActivity(options: streamingPerformanceActivityOptions, reason: "PixelNOW active cloud gaming stream")
