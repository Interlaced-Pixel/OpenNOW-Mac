...
@objc private protocol RTCMetalRenderer: NSObjectProtocol {
...
private final class ObjCMetalRenderer: NSObject, RTCMetalRenderer {
...
@objc(MetalVideoView)
@MainActor
final class MetalVideoView: NSView, RTCVideoRenderer, MTKViewDelegate {
    private let metalView: MTKView
    nonisolated(unsafe) private var videoFrame: RTCVideoFrame?
    private var rendererNV12: RTCMetalRenderer?
    private var rendererRGB: RTCMetalRenderer?
    private var rendererI420: RTCMetalRenderer?
    private var commandQueue: (any MTLCommandQueue)?
    private var enhancementRenderer: VideoEnhancementRenderer?
    ...
    private var enhancementResult = VideoEnhancementResult()
    ...
    nonisolated(unsafe) private weak var owner: LibWebRTCStreamSession?

    init(frame frameRect: NSRect, targetFps: Int32, owner: LibWebRTCStreamSession?) {
        self.owner = owner
        ...
        if let device = metalView.device {
            ...
            enhancementRenderer = VideoEnhancementRenderer(device: device, commandQueue: commandQueue)
        }
    }
    ...
    private func renderEnhancedFrame(_ frame: RTCVideoFrame, drawSerial: UInt64, sourceSize: CGSize, enhancement: VideoEnhancement, diagnostics: inout RenderDiagnostics) -> Bool {
        ...
        if enhancementRenderer.renderFrame(frame, to: metalView, settings: settings, result: result) {
            ...
            diagnostics.renderPath = result.renderPath.isEmpty ? "VideoEnhancementRenderer" : result.renderPath
    ...
    private func renderTenBitFrame(_ frame: RTCVideoFrame, drawSerial: UInt64, sourceSize: CGSize, diagnostics: inout RenderDiagnostics) -> Bool {
        ...
        if enhancementRenderer.renderFrame(frame, to: metalView, settings: settings, result: result) {
            ...
            diagnostics.renderPath = result.renderPath.isEmpty ? "MetalSpatialUpscalerSwift" : result.renderPath
            diagnostics.fallback = result.fallbackReason
    ...
    private func rendererForFrame(_ frame: RTCVideoFrame, diagnostics: inout RenderDiagnostics) -> RTCMetalRenderer? {
    ...
    private func newRenderer(named className: String, fallback: inout String) -> RTCMetalRenderer? {
        guard let rendererClass = NSClassFromString(className) as? NSObject.Type else {
            fallback = "\(className) unavailable"
            return nil
        }
        guard let renderer = ObjCMetalRenderer(rendererClass.init()) else {
    ...
    private func i420Renderer(fallback: inout String) -> RTCMetalRenderer? {
...
@MainActor
private func automaticEnhancementTier(renderer: VideoEnhancementRenderer, device: (any MTLDevice)?) -> VideoEnhancementTier {
