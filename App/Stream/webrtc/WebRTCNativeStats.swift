...
@objc(LibWebRTCStats)
final class LibWebRTCStats: NSObject, @unchecked Sendable {
    ...
    func requestStats(sessionImpl: LibWebRTCSessionImpl?, queue: DispatchQueue) {
        guard Self.envFlagEnabled("_ENABLE_WEBRTC_STATS", defaultValue: true) else { return }
        guard let peerConnection = sessionImpl?.peerConnection else { return }
        guard peerConnection.connectionState == .connected,
    ...
    func startPolling(sessionImpl: LibWebRTCSessionImpl?, queue: DispatchQueue) {
    ...
    func applyRuntimeBitrateLimit(mbps: Int, reason: String, sessionImpl: LibWebRTCSessionImpl?) {
        guard let peerConnection = sessionImpl?.peerConnection else { return }
