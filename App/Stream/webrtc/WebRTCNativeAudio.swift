...
private let audioDeviceChangedCallback: AudioObjectPropertyListenerProc = { _, _, _, clientData in
    ...
    let audio = Unmanaged<LibWebRTCAudio>.fromOpaque(clientData).takeUnretainedValue()
    audio.scheduleAudioDeviceChange()
    return noErr
}

final class CoreAudioCallbackContext: @unchecked Sendable {
    private let condition = NSCondition()
    private weak var device: CoreAudioRTCDevice?
    private var isTerminating = false
    private var callbacksInFlight = 0

    func activate(device: CoreAudioRTCDevice) {
        condition.lock()
        self.device = device
        isTerminating = false
        condition.unlock()
    ...
    func withDevice<T>(_ body: (CoreAudioRTCDevice) -> T) -> T? {
        condition.lock()
...
private final class CoreAudioTerminationResources: @unchecked Sendable {
    private let callbackContext: CoreAudioCallbackContext
    ...
    init(queue: DispatchQueue,
         callbackContext: CoreAudioCallbackContext,
...
private let coreAudioPlayoutCallback: AURenderCallback = { refCon, actionFlags, timestamp, busNumber, frameCount, outputData in
    let context = Unmanaged<CoreAudioCallbackContext>.fromOpaque(refCon).takeUnretainedValue()
...
@objc(CoreAudioRTCDevice)
final class CoreAudioRTCDevice: NSObject, RTCAudioDevice, @unchecked Sendable {
    weak var owner: LibWebRTCStreamSession?

    private let audioQueue = DispatchQueue(label: "io.opencg.pixelnow.webrtc.coreaudio")
    ...
    @objc(initWithOwner:)
    init(owner: LibWebRTCStreamSession?) {
        self.owner = owner
        super.init()
    ...
    deinit {
        ...
        let termination = CoreAudioTerminationResources(
            queue: audioQueue,
    ...
    private func initializePlayoutLocked() -> Bool {
        ...
        var callback = AURenderCallbackStruct(inputProc: coreAudioPlayoutCallback, inputProcRefCon: Unmanaged.passUnretained(callbackContext).toOpaque())
    ...
    private func initializeRecordingLocked() -> Bool {
        ...
        var callback = AURenderCallbackStruct(inputProc: coreAudioRecordingCallback, inputProcRefCon: Unmanaged.passUnretained(callbackContext).toOpaque())
...
@objc(LibWebRTCAudio)
final class LibWebRTCAudio: NSObject, @unchecked Sendable {
    private weak var owner: LibWebRTCStreamSession?
    ...
    private weak var sessionImpl: LibWebRTCSessionImpl?
    ...
    func setMicrophoneEnabled(_ enabled: Bool, sessionImpl: LibWebRTCSessionImpl?) {
    ...
    func setGameVolume(_ volume: Double, sessionImpl: LibWebRTCSessionImpl?) {
        gameVolume = min(max(volume, 0), 1)
        sessionImpl?.remoteAudioTrack?.source.volume = gameVolume
    }

    @objc(setMicrophoneVolume:sessionImpl:)
    func setMicrophoneVolume(_ volume: Double, sessionImpl: LibWebRTCSessionImpl?) {
    ...
    func refreshAudioDevices(sessionImpl: LibWebRTCSessionImpl?) {
        self.sessionImpl = sessionImpl
    ...
    func handleAudioDeviceChange(sessionImpl: LibWebRTCSessionImpl?) {
        guard audioMonitoringActive else { return }
        self.sessionImpl = sessionImpl
        ...
        if !customAudioDeviceActive, Self.envFlagEnabled("_ENABLE_WEBRTC_AUDIO_HOTSWAP_RECOVERY", defaultValue: true) {
    ...
    func startMicrophoneLevelPolling(sessionImpl: LibWebRTCSessionImpl?, statsQueue: DispatchQueue) {
    ...
    private static func envFlagEnabled(_ name: String, defaultValue: Bool) -> Bool {
        guard let rawValue = environmentValue(name) else { return defaultValue }
        let normalized = rawValue.lowercased()
        return !(normalized == "0" || normalized == "false" || normalized == "no" || normalized == "off")
    }

    private static func environmentValue(_ name: String) -> String? {
        let candidates = [name, "PIXELNOW\(name)", "OPN\(name)"]
        for candidate in candidates {
            guard let rawValue = getenv(candidate), rawValue.pointee != 0 else { continue }
