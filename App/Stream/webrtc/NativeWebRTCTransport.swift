...
public final class NativeWebRTCTransport: NSObject, WebRTCStreamTransport, @unchecked Sendable {
    ...
    private let session = LibWebRTCStreamSession()
    ...
    private var remoteCoOpVideoRelay: RemoteCoOpHostVideoRelay?
    private let remoteCoOpAudioRelayLock = NSLock()
    private var remoteCoOpAudioRelay: RemoteCoOpHostAudioRelay?
    ...
    public func setRemoteCoOpVideoRelay(_ relay: RemoteCoOpHostVideoRelay?) {
        remoteCoOpVideoRelayLock.withLock { remoteCoOpVideoRelay = relay }
    }

    public func setRemoteCoOpAudioRelay(_ relay: RemoteCoOpHostAudioRelay?) {
    ...
    public func latestStatsSnapshot() -> StreamStatsSnapshot {
        session.latestStatsSnapshot()
    }

    public func statsSnapshots(intervalSeconds: Double = 1.0) -> AsyncStream<StreamStatsSnapshot> {
    ...
    static func gfnControllerBitmap(playerIndex: Int) -> UInt16 {
        let configuredCount = NativeWebRTCGamepadMonitor.connectedGamepadCount() + RemoteCoOpPreferencesStore.reservedControllerSlotsForLaunch()
    ...
    private func currentRemoteCoOpVideoRelay() -> RemoteCoOpHostVideoRelay? {
        remoteCoOpVideoRelayLock.withLock { remoteCoOpVideoRelay }
    }

    private func currentRemoteCoOpAudioRelay() -> RemoteCoOpHostAudioRelay? {
