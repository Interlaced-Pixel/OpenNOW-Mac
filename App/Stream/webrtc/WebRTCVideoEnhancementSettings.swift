...
@objc enum VideoEnhancementTier: Int {
    case off = 0
    case spatial = 1
    case metalFX = 2
    case temporal = 3
...
@objc(VideoEnhancementSettings)
final class VideoEnhancementSettings: NSObject {
    @objc var configuredTier: VideoEnhancementTier = .off
...
@objc(VideoEnhancementResult)
final class VideoEnhancementResult: NSObject {
...
@objc(VideoTextureFrame)
final class VideoTextureFrame: NSObject {
    @objc var kind = 0
...
@objc(VideoTextureSource)
final class VideoTextureSource: NSObject {
    ...
    func newTextureFrame(
        ...
        guard let cvBuffer = buffer as? RTCCVPixelBuffer else {
            ...
            let textureFrame = VideoTextureFrame()
            textureFrame.kind = 2
            ...
            textureFrame.lumaTexture = reusablePlaneTexture(&i420LumaTexture, width: Int(i420.width), height: Int(i420.height), bytes: i420.dataY, bytesPerRow: Int(i420.strideY), label: "PixelNOW I420 Y")
            textureFrame.chromaUTexture = reusablePlaneTexture(&i420ChromaUTexture, width: Int(i420.chromaWidth), height: Int(i420.chromaHeight), bytes: i420.dataU, bytesPerRow: Int(i420.strideU), label: "PixelNOW I420 U")
            textureFrame.chromaVTexture = reusablePlaneTexture(&i420ChromaVTexture, width: Int(i420.chromaWidth), height: Int(i420.chromaHeight), bytes: i420.dataV, bytesPerRow: Int(i420.strideV), label: "PixelNOW I420 V")
...
vertex VertexOut video_video_vertex(uint vid [[vertex_id]]) {
...
static float2 video_crop_uv(float2 texCoord, float4 crop) {
    return mix(crop.xy, crop.zw, clamp(texCoord, float2(0.0), float2(1.0)));
}
static float2 video_clamp_crop(float2 uv, float4 crop) {
    return clamp(uv, crop.xy, crop.zw);
}
static float3 video_nv12_rgb(texture2d<float> yTexture, texture2d<float> uvTexture, sampler s, float2 uv) {
...
static float3 video_i420_rgb(texture2d<float> yTexture, texture2d<float> uTexture, texture2d<float> vTexture, sampler s, float2 uv) {
    float y = yTexture.sample(s, uv).r;
...
static float3 video_finish(float3 center, float3 blur, float sharpness, float denoise) {
    float3 denoised = mix(center, blur, clamp(denoise, 0.0, 1.0));
    return clamp(denoised + (denoised - blur) * sharpness, float3(0.0), float3(1.0));
}
static float video_luma(float3 color) {
    return dot(color, float3(0.2126, 0.7152, 0.0722));
}
static float video_block_luma(texture2d<float> sourceTexture, sampler s, float2 uv, float2 texel) {
    float center = video_luma(sourceTexture.sample(s, clamp(uv, float2(0.0), float2(1.0))).rgb);
    float horizontal = video_luma(sourceTexture.sample(s, clamp(uv + float2(texel.x, 0.0), float2(0.0), float2(1.0))).rgb) + video_luma(sourceTexture.sample(s, clamp(uv - float2(texel.x, 0.0), float2(0.0), float2(1.0))).rgb);
    float vertical = video_luma(sourceTexture.sample(s, clamp(uv + float2(0.0, texel.y), float2(0.0), float2(1.0))).rgb) + video_luma(sourceTexture.sample(s, clamp(uv - float2(0.0, texel.y), float2(0.0), float2(1.0))).rgb);
    return (center * 2.0 + horizontal + vertical) / 6.0;
}
static float3 video_rgb_spatial(texture2d<float> sourceTexture, sampler s, float2 uv, float2 texel, float4 crop, float sharpness, float denoise) {
    float3 center = sourceTexture.sample(s, uv).rgb;
    float3 blur = (sourceTexture.sample(s, video_clamp_crop(uv + float2(texel.x, 0.0), crop)).rgb + sourceTexture.sample(s, video_clamp_crop(uv - float2(texel.x, 0.0), crop)).rgb + sourceTexture.sample(s, video_clamp_crop(uv + float2(0.0, texel.y), crop)).rgb + sourceTexture.sample(s, video_clamp_crop(uv - float2(0.0, texel.y), crop)).rgb) * 0.25;
    return video_finish(center, blur, sharpness, denoise);
}
fragment float4 video_video_spatial_rgb(VertexOut in [[stage_in]], texture2d<float> sourceTexture [[texture(0)]], constant float2 &scale [[buffer(0)]], constant float &sharpness [[buffer(1)]], constant float &denoise [[buffer(2)]], constant float4 &crop [[buffer(3)]], constant float2 &jitter [[buffer(4)]]) {
    constexpr sampler s(address::clamp_to_edge, filter::linear);
    float2 uv = video_clamp_crop(video_crop_uv(in.texCoord, crop) + jitter, crop);
    float2 texel = max(scale, float2(1.0 / 8192.0));
    return float4(video_rgb_spatial(sourceTexture, s, uv, texel, crop, sharpness, denoise), 1.0);
}
fragment float4 video_video_spatial_nv12(VertexOut in [[stage_in]], texture2d<float> yTexture [[texture(0)]], texture2d<float> uvTexture [[texture(1)]], constant float2 &scale [[buffer(0)]], constant float &sharpness [[buffer(1)]], constant float &denoise [[buffer(2)]], constant float4 &crop [[buffer(3)]], constant float2 &jitter [[buffer(4)]]) {
    constexpr sampler s(address::clamp_to_edge, filter::linear);
    float2 uv = video_clamp_crop(video_crop_uv(in.texCoord, crop) + jitter, crop);
    float2 texel = max(scale, float2(1.0 / 8192.0));
    float3 center = video_nv12_rgb(yTexture, uvTexture, s, uv);
    float3 blur = (video_nv12_rgb(yTexture, uvTexture, s, video_clamp_crop(uv + float2(texel.x, 0.0), crop)) + video_nv12_rgb(yTexture, uvTexture, s, video_clamp_crop(uv - float2(texel.x, 0.0), crop)) + video_nv12_rgb(yTexture, uvTexture, s, video_clamp_crop(uv + float2(0.0, texel.y), crop)) + video_nv12_rgb(yTexture, uvTexture, s, video_clamp_crop(uv - float2(0.0, texel.y), crop))) * 0.25;
    return float4(video_finish(center, blur, sharpness, denoise), 1.0);
}
fragment float4 video_video_spatial_i420(VertexOut in [[stage_in]], texture2d<float> yTexture [[texture(0)]], texture2d<float> uTexture [[texture(1)]], texture2d<float> vTexture [[texture(2)]], constant float2 &scale [[buffer(0)]], constant float &sharpness [[buffer(1)]], constant float &denoise [[buffer(2)]], constant float4 &crop [[buffer(3)]], constant float2 &jitter [[buffer(4)]]) {
    constexpr sampler s(address::clamp_to_edge, filter::linear);
    float2 uv = video_clamp_crop(video_crop_uv(in.texCoord, crop) + jitter, crop);
    float2 texel = max(scale, float2(1.0 / 8192.0));
    float3 center = video_i420_rgb(yTexture, uTexture, vTexture, s, uv);
    float3 blur = (video_i420_rgb(yTexture, uTexture, vTexture, s, video_clamp_crop(uv + float2(texel.x, 0.0), crop)) + video_i420_rgb(yTexture, uTexture, vTexture, s, video_clamp_crop(uv - float2(texel.x, 0.0), crop)) + video_i420_rgb(yTexture, uTexture, vTexture, s, video_clamp_crop(uv + float2(0.0, texel.y), crop)) + video_i420_rgb(yTexture, uTexture, vTexture, s, video_clamp_crop(uv - float2(0.0, texel.y), crop))) * 0.25;
    return float4(video_finish(center, blur, sharpness, denoise), 1.0);
}
fragment float4 video_video_fast_rgb(VertexOut in [[stage_in]], texture2d<float> sourceTexture [[texture(0)]], constant float2 &scale [[buffer(0)]], constant float &sharpness [[buffer(1)]], constant float &denoise [[buffer(2)]], constant float4 &crop [[buffer(3)]], constant float2 &jitter [[buffer(4)]]) {
    constexpr sampler s(address::clamp_to_edge, filter::linear);
    return float4(sourceTexture.sample(s, video_crop_uv(in.texCoord, crop)).rgb, 1.0);
}
fragment float4 video_video_fast_nv12(VertexOut in [[stage_in]], texture2d<float> yTexture [[texture(0)]], texture2d<float> uvTexture [[texture(1)]], constant float2 &scale [[buffer(0)]], constant float &sharpness [[buffer(1)]], constant float &denoise [[buffer(2)]], constant float4 &crop [[buffer(3)]], constant float2 &jitter [[buffer(4)]]) {
    constexpr sampler s(address::clamp_to_edge, filter::linear);
    return float4(video_nv12_rgb(yTexture, uvTexture, s, video_crop_uv(in.texCoord, crop)), 1.0);
}
fragment float4 video_video_fast_i420(VertexOut in [[stage_in]], texture2d<float> yTexture [[texture(0)]], texture2d<float> uTexture [[texture(1)]], texture2d<float> vTexture [[texture(2)]], constant float2 &scale [[buffer(0)]], constant float &sharpness [[buffer(1)]], constant float &denoise [[buffer(2)]], constant float4 &crop [[buffer(3)]], constant float2 &jitter [[buffer(4)]]) {
    constexpr sampler s(address::clamp_to_edge, filter::linear);
    return float4(video_i420_rgb(yTexture, uTexture, vTexture, s, video_crop_uv(in.texCoord, crop)), 1.0);
}
fragment float4 video_video_temporal_motion(VertexOut in [[stage_in]], texture2d<float> currentTexture [[texture(0)]], texture2d<float> historyTexture [[texture(1)]], constant float2 &texel [[buffer(0)]], constant int &hasHistory [[buffer(1)]], constant float2 &jitterDelta [[buffer(2)]]) {
    constexpr sampler s(address::clamp_to_edge, filter::linear);
    ...
    float currentLuma = video_block_luma(currentTexture, s, uv, blockTexel);
    float bestDiff = 1.0;
    float bestScore = 1.0;
    ...
    for (int y = -2; y <= 2; ++y) {
        for (int x = -2; x <= 2; ++x) {
            ...
            float diff = fabs(currentLuma - video_block_luma(historyTexture, s, uv + offset, blockTexel));
...
fragment float4 video_video_temporal_composite(VertexOut in [[stage_in]], texture2d<float> currentTexture [[texture(0)]], texture2d<float> historyTexture [[texture(1)]], texture2d<float> motionTexture [[texture(2)]], constant float2 &texel [[buffer(0)]], constant float &historyWeight [[buffer(1)]], constant float &sharpness [[buffer(2)]], constant int &hasHistory [[buffer(3)]]) {
    ...
    if (hasHistory != 0) {
        ...
        historyDiff = fabs(video_luma(current) - video_luma(history));
...
fragment float4 video_video_present_rgb(VertexOut in [[stage_in]], texture2d<float> sourceTexture [[texture(0)]]) {
    constexpr sampler s(address::clamp_to_edge, filter::linear);
    float2 uv = clamp(in.texCoord, float2(0.0), float2(1.0));
    ...
    float centerLuma = video_luma(center);
    float horizontalContrast = abs(video_luma(left) - centerLuma) + abs(video_luma(right) - centerLuma);
    float verticalContrast = abs(video_luma(up) - centerLuma) + abs(video_luma(down) - centerLuma);
...
@objc(MetalFXUpscaler)
final class MetalFXUpscaler: NSObject {
    private let device: (any MTLDevice)?
...
@objc(VideoEnhancementRenderer)
@MainActor
final class VideoEnhancementRenderer: NSObject {
    ...
    private let metalFXUpscaler: MetalFXUpscaler
    private let textureSource: VideoTextureSource
    ...
    @objc init(device: (any MTLDevice)?, commandQueue: (any MTLCommandQueue)?) {
        ...
        self.metalFXUpscaler = MetalFXUpscaler(device: device)
        self.textureSource = VideoTextureSource(device: device)
        self.shaderLibrary = Self.makeShaderLibrary(device: device)
        super.init()
        spatialRGBPipeline = newSpatialPipeline(fragmentFunctionName: "video_video_spatial_rgb")
        spatialNV12Pipeline = newSpatialPipeline(fragmentFunctionName: "video_video_spatial_nv12")
        spatialI420Pipeline = newSpatialPipeline(fragmentFunctionName: "video_video_spatial_i420")
        fastSpatialRGBPipeline = newSpatialPipeline(fragmentFunctionName: "video_video_fast_rgb")
        fastSpatialNV12Pipeline = newSpatialPipeline(fragmentFunctionName: "video_video_fast_nv12")
        fastSpatialI420Pipeline = newSpatialPipeline(fragmentFunctionName: "video_video_fast_i420")
        temporalMotionPipeline = newSpatialPipeline(fragmentFunctionName: "video_video_temporal_motion", pixelFormat: .rgba16Float)
        temporalCompositePipeline = newSpatialPipeline(fragmentFunctionName: "video_video_temporal_composite")
        temporalPresentPipeline = newSpatialPipeline(fragmentFunctionName: "video_video_present_rgb")
    }
    ...
    func renderFrame(
        ...
        if (settings.configuredTier == .spatial || settings.configuredTier == .metalFX || settings.configuredTier == .temporal), !settings.captureEnhancedPixelBuffer {
            ...
            let textureFrame = textureSource.newTextureFrame(for: frame, pixelFormat: &pixelFormat, frameSource: &frameSource, fallback: &textureFallback) as? VideoTextureFrame
        ...
        result.renderPath = "VideoEnhancementRendererSwift"
    ...
    private func renderMetalFXFrame(
        ...
        guard let sourceTexture = reusableTexture(&metalFXIntermediateTexture, width: sourceWidth, height: sourceHeight, pixelFormat: Self.renderTargetPixelFormat, usage: [.shaderRead, .renderTarget], label: "PixelNOW MetalFX source"),
              let outputTexture = reusableTexture(&metalFXOutputTexture, width: outputWidth, height: outputHeight, pixelFormat: Self.renderTargetPixelFormat, usage: [.shaderRead, .shaderWrite, .renderTarget], label: "PixelNOW MetalFX output") else {
        ...
        result.renderPath = "MetalFXSpatialScalerSwift"
    ...
    private func renderMetalFXTextureFrame(
        _ textureFrame: VideoTextureFrame,
        drawable: any CAMetalDrawable,
        commandBuffer: any MTLCommandBuffer,
        settings: VideoEnhancementSettings,
        result: VideoEnhancementResult,
        ...
        let outputWidth = drawable.texture.width
        let outputHeight = drawable.texture.height
        guard let outputTexture = reusableTexture(&metalFXOutputTexture, width: outputWidth, height: outputHeight, pixelFormat: Self.renderTargetPixelFormat, usage: [.shaderRead, .shaderWrite, .renderTarget], label: "PixelNOW MetalFX output") else {
        ...
        guard let sourceTexture = reusableTexture(&metalFXIntermediateTexture, width: width, height: height, pixelFormat: Self.renderTargetPixelFormat, usage: [.renderTarget, .shaderRead], label: "PixelNOW MetalFX source intermediate") else {
    ...
    private func renderSpatialTextureFrame(
        ...
        commandBuffer.present(drawable)
        commandBuffer.commit()
        result.renderPath = "MetalSpatialUpscalerSwift"
    ...
    private func renderTemporalTextureFrame(
        ...
        guard let currentTexture = reusableTexture(&temporalCurrentTexture, width: width, height: height, pixelFormat: Self.renderTargetPixelFormat, usage: [.renderTarget, .shaderRead], label: "PixelNOW temporal current"),
              let outputTexture = reusableTexture(&temporalOutputTexture, width: width, height: height, pixelFormat: Self.renderTargetPixelFormat, usage: [.renderTarget, .shaderRead], label: "PixelNOW temporal output"),
              let historyTexture = reusableTexture(&temporalHistoryTexture, width: width, height: height, pixelFormat: Self.renderTargetPixelFormat, usage: [.renderTarget, .shaderRead], label: "PixelNOW temporal history"),
              let motionTexture = reusableTexture(&temporalMotionTexture, width: motionWidth, height: motionHeight, pixelFormat: .rgba16Float, usage: [.renderTarget, .shaderRead], label: "PixelNOW temporal half-res motion") else {
        ...
        result.renderPath = "MetalTemporalUpscalerSwift"
    ...
    private func newSpatialPipeline(fragmentFunctionName: String, pixelFormat: MTLPixelFormat = renderTargetPixelFormat) -> (any MTLRenderPipelineState)? {
        ...
        do {
            ...
            descriptor.vertexFunction = shaderLibrary.makeFunction(name: "video_video_vertex")
    ...
    private static func makeShaderLibrary(device: (any MTLDevice)?) -> (any MTLLibrary)? {
        ...
        do {
            return try device.makeLibrary(source: VideoTextureSource.spatialShaderSource, options: nil)
        } catch {
    ...
    private func populateResult(_ result: VideoEnhancementResult?, settings: VideoEnhancementSettings?) {
    ...
    private func image(for frame: RTCVideoFrame, result: VideoEnhancementResult) -> CIImage? {
        let buffer = frame.buffer
    ...
    private func enhancedImage(_ image: CIImage, settings: VideoEnhancementSettings) -> CIImage {
    ...
    private func enhancedImageWithoutScale(_ image: CIImage, settings: VideoEnhancementSettings) -> CIImage {
        var output = image
    ...
    private func recordDrop(in result: VideoEnhancementResult) {
        droppedFrames += 1
        result.droppedFrames = droppedFrames
        result.activeTier = "Off"
        result.frameTimeMs = 0
    ...
    private func tierName(for tier: VideoEnhancementTier) -> String {
        switch tier {
    ...
    private func activeTierName(for tier: VideoEnhancementTier) -> String {
        switch tier {
        case .spatial: return "Spatial"
    ...
    private func renderMode(for tier: VideoEnhancementTier) -> String {
        switch tier {
    ...
    private static func textureFrameUsesFullCrop(_ textureFrame: VideoTextureFrame) -> Bool {
