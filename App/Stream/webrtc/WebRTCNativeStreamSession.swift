...
typealias LibWebRTCAnswerHandler = @convention(block) (NSString, NSString) -> Void
typealias LibWebRTCIceCandidateHandler = @convention(block) (NSDictionary) -> Void
typealias LibWebRTCStateHandler = @convention(block) (Bool, NSString) -> Void

final class LibWebRTCStreamSession: NSObject, @unchecked Sendable {
    static let maxGamepadControllers = 4

    private let statsQueue = DispatchQueue(label: "io.opencg.pixelnow.webrtc.stats")
    private var inputController: LibWebRTCInput!
    private var audioController: LibWebRTCAudio!
    private var statsController: LibWebRTCStats!
    private let statsLock = NSLock()
    private let remoteIceLock = NSLock()

    private var impl: LibWebRTCSessionImpl?
    ...
    private var latestStats = StreamStatsState()
    ...
    override init() {
        super.init()
        inputController = LibWebRTCInput(owner: self)
        audioController = LibWebRTCAudio(owner: self)
        statsController = LibWebRTCStats(owner: self)
    }
    ...
    func start(sessionInfo: [String: Any], offerSdp: String, settings: [String: Any], answerHandler: @escaping LibWebRTCAnswerHandler, localIceCandidateHandler: @escaping LibWebRTCIceCandidateHandler, stateHandler: @escaping LibWebRTCStateHandler) {
    ...
    func start(sessionInfo: [String: Any], offerSdp: String, settings: [String: Any]) {
        ...
        let impl = LibWebRTCSessionImpl(owner: self)
        let encoderFactory = RTCDefaultVideoEncoderFactory()
        let decoderFactory = RTCDefaultVideoDecoderFactory()
        let audioDevice = CoreAudioRTCDevice(owner: self)
        ...
        configuration.continualGatheringPolicy = envFlagEnabled("_ENABLE_WEBRTC_CONTINUAL_ICE_GATHERING", defaultValue: false) ? .gatherContinually : .gatherOnce
        ...
        let requestedCodecSupported = WebRTCCodecSupport.supportsCodec(factory: factory, normalizedCodec: requestedCodec)
        LogCapture.appendEvent("[LibWebRTC] Start settings resolution=\(string(settings["resolution"], fallback: "unknown")) fps=\(int(settings["fps"])) codec=\(requestedCodec.isEmpty ? "unknown" : requestedCodec) requestedCodecSupported=\(requestedCodecSupported ? "yes" : "no") bitrate=\(int(settings["maxBitrateMbps"]))Mbps h265Rewrite=\(envFlagEnabled("_ENABLE_LIBWEBRTC_H265_OFFER_REWRITE", defaultValue: true) ? "on" : "off") codecFilter=\(envFlagEnabled("_ENABLE_LIBWEBRTC_CODEC_FILTER", defaultValue: false) ? "on" : "off") answerMunge=\(envFlagEnabled("_ENABLE_LIBWEBRTC_ANSWER_MUNGE", defaultValue: false) ? "on" : "off") receiverCapabilities=\(WebRTCCodecSupport.receiverCapabilitiesSummary(factory: factory))")
        if requestedCodec == "H265", requestedCodecSupported, envFlagEnabled("_ENABLE_LIBWEBRTC_H265_OFFER_REWRITE", defaultValue: true) {
            let support = WebRTCCodecSupport.h265ReceiverSupport(factory: factory)
        ...
        if isSupportedCodecPreference(requestedCodec), requestedCodecSupported, envFlagEnabled("_ENABLE_LIBWEBRTC_CODEC_FILTER", defaultValue: false) {
        ...
        @Sendable func handleRemoteDescriptionSet(impl: LibWebRTCSessionImpl, peerConnection: RTCPeerConnection, factory: RTCPeerConnectionFactory) {
            ...
            if !answerCodec.isEmpty {
                if videoSdpContainsCodec(remoteOfferSdp, normalizedCodec: answerCodec) {
                    codecPreferenceApplied = WebRTCCodecSupport.applyVideoCodecPreference(factory: factory, peerConnection: peerConnection, normalizedCodec: answerCodec)
            ...
            @Sendable func createAndSendAnswer(retriedWithoutCodecPreference: Bool) {
                peerConnection.answer(for: constraints) { [weak self, weak impl] answer, answerError in
                    ...
                    let mungedAnswer = envFlagEnabled("_ENABLE_LIBWEBRTC_ANSWER_MUNGE", defaultValue: false) ? mungeAnswerSdp(answer.sdp, maxBitrateKbps: max(1000, int(self.settings["maxBitrateMbps"], fallback: 50) * 1000)) : answer.sdp
    ...
    func latestStatsSnapshot() -> StreamStatsSnapshot {
        let stats = statsLock.withLock { latestStats }
        return StreamStatsSnapshot(available: stats.available,
    ...
    private func resetStats(sessionInfo: [String: Any], settings: [String: Any]) {
        statsLock.withLock {
            ...
            latestStats.videoSink = "MetalVideoView"
    ...
    private func prepareMicrophoneIfNeeded(impl: LibWebRTCSessionImpl, factory: RTCPeerConnectionFactory) {
        ...
        let audioTrack = factory.audioTrack(with: audioSource, trackId: "pixelnow-microphone")
    ...
    private func attachMicrophoneTrack(impl: LibWebRTCSessionImpl, audioTrack: RTCAudioTrack) -> Bool {
...
private struct StreamStatsState {
    var available = false
...
private func envFlagEnabled(_ name: String, defaultValue: Bool) -> Bool {
    ...
    let value = environment[name] ?? environment["PIXELNOW\(name)"] ?? environment["OPN\(name)"]
...
private struct IceMediaTarget { var mid = "0"; var mLineIndex: Int32 = -1 }

private func extractIceTargets(from sdp: String) -> [IceMediaTarget] {
    var targets: [IceMediaTarget] = []
    var index: Int32 = -1
    var currentMid = "0"
    var hasOpenMediaSection = false
    for line in sdp.components(separatedBy: .newlines) {
        if line.hasPrefix("m=") {
            if hasOpenMediaSection { targets.append(IceMediaTarget(mid: currentMid, mLineIndex: index)) }
            index += 1
