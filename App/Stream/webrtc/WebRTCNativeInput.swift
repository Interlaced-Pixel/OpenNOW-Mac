...
@objc(LibWebRTCInput)
final class LibWebRTCInput: NSObject, @unchecked Sendable {
    private weak var owner: LibWebRTCStreamSession?
    private let encoder = InputProtocolEncoder()
    ...
    func sendInput(data: Data, partiallyReliable: Bool, sessionImpl: LibWebRTCSessionImpl?) {
    ...
    func createInputChannel(sessionImpl: LibWebRTCSessionImpl?) {
    ...
    func sendKey(keycode: UInt16, scancode: UInt16, modifiers: UInt16, down: Bool, sessionImpl: LibWebRTCSessionImpl?) {
        let encoded = encoderLock.withLock { encoder.encodeKey(keycode: keycode, scancode: scancode, modifiers: modifiers, timestampUs: InputProtocolEncoder.timestampUs(), down: down) }
        sendInput(data: encoded, partiallyReliable: false, sessionImpl: sessionImpl)
    }

    @objc(sendMouseMoveWithDx:dy:sessionImpl:)
    func sendMouseMove(dx: Int16, dy: Int16, sessionImpl: LibWebRTCSessionImpl?) {
        ...
        let encoded = encoderLock.withLock { encoder.encodeMouseMove(dx: dx, dy: dy, timestampUs: InputProtocolEncoder.timestampUs()) }
    ...
    func sendMouseButton(button: UInt8, down: Bool, sessionImpl: LibWebRTCSessionImpl?) {
        let encoded = encoderLock.withLock { encoder.encodeMouseButton(button: button, timestampUs: InputProtocolEncoder.timestampUs(), down: down) }
        sendInput(data: encoded, partiallyReliable: false, sessionImpl: sessionImpl)
    }

    @objc(sendMouseWheelWithDelta:sessionImpl:)
    func sendMouseWheel(delta: Int16, sessionImpl: LibWebRTCSessionImpl?) {
        let encoded = encoderLock.withLock { encoder.encodeMouseWheel(delta: delta, timestampUs: InputProtocolEncoder.timestampUs()) }
        sendInput(data: encoded, partiallyReliable: false, sessionImpl: sessionImpl)
    }

    @objc(sendUtf8Text:sessionImpl:)
    func sendUtf8Text(_ text: String, sessionImpl: LibWebRTCSessionImpl?) {
    ...
    func sendGamepadState(controllerId: UInt16,
                          ...
                          timestampUs: UInt64,
                          bitmap: UInt16,
                          sessionImpl: LibWebRTCSessionImpl?) {
    ...
    func handleDataChannelMessage(label: String, data: Data, sessionImpl: LibWebRTCSessionImpl?) {
    ...
    private func startHeartbeat(sessionImpl: LibWebRTCSessionImpl?) {
