...
final class SessionManager: NSObject, @unchecked Sendable {
    ...
    func createSession(appId: String, internalTitle: String, settings: [String: Any], completion: @escaping (Bool, [String: Any], String) -> Void) {
        ...
        let deviceId = DeviceIdentity.stableCloudmatchDeviceId()
        ...
        let language = string(effectiveSettings["gameLanguage"]).isEmpty ? GFNLocale.currentGFNLocale() : string(effectiveSettings["gameLanguage"])
        let body: [String: Any] = ["sessionRequestData": sessionRequestData]
        ProtocolDebug.logJSONObject(label: "session create request", object: body)
        let bodyData: Data
        do {
        ...
        URLSession.shared.dataTask(with: tracedRequest) { [weak self] data, response, error in
            ...
            ProtocolDebug.logJSONData(label: "session create response", data: data)
    ...
    func reportSessionAd(session: [String: Any], adId: String, action: String, watchedTimeInMs: Int, pausedTimeInMs: Int, cancelReason: String, completion: @escaping (Bool, [String: Any], String) -> Void) {
        ...
        URLSession.shared.dataTask(with: tracedRequest) { [weak self] data, response, error in
            ...
            if let sessionJson = json["session"] as? [String: Any] {
                let progress = SessionJSONParser.parseSessionProgress(from: sessionJson as NSDictionary)
    ...
    private func sendClaimSession(sessionId: String, serverIp: String, appId: ResolvedLaunchAppId, settings: [String: Any], token: String, deviceId: String, clientId: String, completion: @escaping (Bool, [String: Any], String) -> Void) {
        let capabilities = StreamPreferences.loadDeviceCapabilities()
        ...
        let language = string(settings["gameLanguage"]).isEmpty ? GFNLocale.currentGFNLocale() : string(settings["gameLanguage"])
        ProtocolDebug.logJSONObject(label: "session claim request", object: payload)
        let bodyData: Data
        do {
        ...
        URLSession.shared.dataTask(with: tracedRequest) { [weak self] data, response, error in
            ...
            ProtocolDebug.logJSONData(label: "session claim response", data: data)
            let body = String(data: data, encoding: .utf8) ?? ""
            let http = response as? HTTPURLResponse
            guard http?.statusCode == 200 else {
    ...
    private func pollClaimSession(sessionId: String, serverIp: String, deviceId: String, clientId: String, headers: CloudMatchClientHeaders, initialProfile: [String: Any], completion: @escaping (Bool, [String: Any], String) -> Void) {
        PollClaimSessionContext(manager: self,
                                   sessionId: sessionId,
    ...
    fileprivate func pollClaimSessionRequestFinished(context: PollClaimSessionContext, attempt: Int, data: Data?, error: Error?) {
    ...
    private func sessionInfo(from session: [String: Any], requestedSessionId: String, baseUrl: String, clientId: String, deviceId: String, initialProfile: [String: Any]) -> [String: Any] {
        ...
        let progress = SessionJSONParser.parseSessionProgress(from: session as NSDictionary)
    ...
    private func negotiatedStreamProfile(from session: [String: Any]) -> [String: Any] {
        let parsed = SessionJSONParser.parseNegotiatedStreamProfile(from: session as NSDictionary)
        return [
    ...
    private func sessionAdState(from session: [String: Any]) -> [String: Any] {
        let parsed = SessionJSONParser.parseSessionAdState(from: session as NSDictionary)
        return [
...
private final class PollClaimSessionContext: @unchecked Sendable {
    ...
    init(manager: SessionManager, sessionId: String, base: String, token: String, deviceId: String, clientId: String, headers: CloudMatchClientHeaders, initialProfile: [String: Any], completion: @escaping (Bool, [String: Any], String) -> Void) {
...
private func settingsByApplyingCloudVariables(_ settings: [String: Any], capabilities: StreamDeviceCapabilities) -> [String: Any] {
...
private func monitorSettings(_ settings: [String: Any], capabilities: StreamDeviceCapabilities, hdrEnabled: Bool) -> [String: Any] {
...
private func clientPhysicalResolutionMetadata(settings: [String: Any], capabilities: StreamDeviceCapabilities) -> String {
    let resolution = requestedResolution(settings)
...
private func clientDisplayHdrCapabilities(_ capabilities: StreamDeviceCapabilities) -> [String: Any] {
    [
