...
@objc(GFNLocale)
final class GFNLocale: NSObject {
    @objc(currentGFNLocale)
    static func currentGFNLocale() -> String {
        for language in Foundation.Locale.preferredLanguages where !language.isEmpty {
            return normalizedLocale(language)
        }
        return normalizedLocale(Foundation.Locale.current.identifier.isEmpty ? "en_US" : Foundation.Locale.current.identifier)
    }
