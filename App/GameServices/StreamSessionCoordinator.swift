...
private final class SessionStopCompletion: @unchecked Sendable {
...
public final class StreamSessionCoordinator: StreamSessionProvider, StreamSignalingChannel, StreamSessionStartCancellable, @unchecked Sendable {
    ...
    public func startSession(configuration: PreparedLaunchConfiguration) async throws -> StreamOffer {
        guard let launchAppId = LaunchAppId.resolve(configuration.applicationID) else {
            throw StreamSessionError.sessionAllocationFailed("This game does not include a launchable GeForce NOW app id.")
        }
    ...
    public func startNativeNVSTSession(configuration: PreparedLaunchConfiguration) async throws -> NativeNVSTSessionAllocation {
        guard let launchAppId = LaunchAppId.resolve(configuration.applicationID) else {
            throw StreamSessionError.sessionAllocationFailed("This game does not include a launchable GeForce NOW app id.")
        }
        let configuration = normalizedConfiguration(configuration, appId: launchAppId.stringValue)
        let capabilities = StreamPreferences.loadDeviceCapabilities()
        let profile = StreamPreferences.launchProfile(forGame: configuration.applicationID, capabilities: capabilities)
        guard profile.transportMode.value.caseInsensitiveCompare("nvst") == .orderedSame else {
            throw StreamSessionError.sessionAllocationFailed("Native NVST session requested while WebRTC transport is selected.")
        }
        let launch = await prepareLaunch(configuration: configuration)
        ...
        let serverType: Int
        do {
            serverType = try await StreamPreferences.fetchServerType(token: configuration.accessToken, streamingBaseUrl: launch.streamingBaseUrl) ?? 0
        } catch {
            try Task.checkCancellation()
            serverType = 0
        }
    ...
    public func recoverNativeNVSTSession(configuration: PreparedLaunchConfiguration, session: StreamSessionDescriptor) async throws -> NativeNVSTSessionAllocation {
        let recoveryConfiguration = PreparedLaunchConfiguration(
            id: configuration.id,
    ...
    public func sendAnswer(_ answer: StreamAnswer, for session: StreamSessionDescriptor) async throws {
        guard let signaling = lock.withLock({ self.signaling }) else {
            throw StreamSessionError.signalingUnavailable
        }
    ...
    private func allocateSession(configuration: PreparedLaunchConfiguration, launch: PreparedStreamLaunch) async throws -> AllocatedStreamSession {
        SessionManager.shared.setAccessToken(configuration.accessToken)
        SessionManager.shared.setStreamingBaseUrl(launch.streamingBaseUrl)
    ...
    private func createSession(configuration: PreparedLaunchConfiguration, settings: [String: Any]) async throws -> AllocatedStreamSession {
        return try await withCheckedThrowingContinuation { continuation in
            SessionManager.shared.createSession(appId: configuration.applicationID, internalTitle: configuration.title.isEmpty ? "PixelNOW" : configuration.title, settings: settings) { success, info, error in
                if success {
                    ...
                    continuation.resume(throwing: StreamSessionError.activeSessionConflict(StreamSessionConflict(
                    ...
                    continuation.resume(throwing: StreamSessionError.sessionAllocationFailed(error.isEmpty ? "Unable to allocate stream session." : error))
                }
            }
        }
    }
        ...
        try await withCheckedThrowingContinuation { continuation in
            SessionManager.shared.claimSession(sessionId: configuration.resumeSessionID, serverIp: configuration.resumeServer, appId: configuration.applicationID, settings: settings, recoveryMode: false) { success, info, error in
                if success {
                    ...
                    continuation.resume(throwing: StreamSessionError.sessionAllocationFailed(error.isEmpty ? "Unable to resume stream session." : error))
                }
            }
        }
    }

    private func waitForReadySession(_ initial: AllocatedStreamSession, configuration: PreparedLaunchConfiguration) async throws -> AllocatedStreamSession {
        if initial.isReady { return initial }
        guard !initial.sessionId.isEmpty, !initial.serverIp.isEmpty else {
            throw StreamSessionError.sessionAllocationFailed("Cloud session is missing session id or server address.")
        }
        ...
        while !latest.isReady {
            ...
            if attempts >= 60, !lastPollWasPendingProgress {
                throw StreamSessionError.sessionAllocationFailed("Session poll timeout")
            }
            ...
            if latest.status > 3, ![4, 5, 6].contains(latest.status) {
                throw StreamSessionError.sessionAllocationFailed("Session in terminal error state")
            }
    ...
    private func playRequiredAd(_ ad: AllocatedSessionAd, session: AllocatedStreamSession) async throws -> AllocatedStreamSession {
        guard !ad.mediaUrl.isEmpty else {
            throw StreamSessionError.sessionAllocationFailed("GeForce NOW requires an ad before launch, but no playable ad media was returned.")
        }
        guard let adPresenter else {
            throw StreamSessionError.sessionAllocationFailed("GeForce NOW requires an ad before launch, but the loading screen ad player is not available.")
        }
    ...
    private func publishAllocationProgress(_ session: AllocatedStreamSession, configuration: PreparedLaunchConfiguration, overrideMessage: String = "") {
    ...
    private func reportSessionAd(session: AllocatedStreamSession, ad: AllocatedSessionAd, action: String, watchedTimeInMs: Int, cancelReason: String) async throws -> AllocatedStreamSession {
        try await withCheckedThrowingContinuation { continuation in
            SessionManager.shared.reportSessionAd(session: session.reportableSession, adId: ad.adId, action: action, watchedTimeInMs: watchedTimeInMs, pausedTimeInMs: -1, cancelReason: cancelReason) { success, info, error in
                if success {
                    continuation.resume(returning: AllocatedStreamSession(info))
                } else {
                    continuation.resume(throwing: StreamSessionError.sessionAllocationFailed(error.isEmpty ? "Unable to update required ad state." : error))
                }
            }
        }
    }
    ...
    private func pollSession(sessionId: String, serverIp: String) async throws -> AllocatedStreamSession {
        try await withCheckedThrowingContinuation { continuation in
            SessionManager.shared.pollSession(sessionId: sessionId, serverIp: serverIp) { success, info, error in
                if success {
                    continuation.resume(returning: AllocatedStreamSession(info))
                } else {
                    continuation.resume(throwing: StreamSessionError.sessionAllocationFailed(error.isEmpty ? "Unable to poll stream session." : error))
                }
            }
        }
    }
    ...
    private func prepareLaunch(configuration: PreparedLaunchConfiguration) async -> PreparedStreamLaunch {
        ...
        let streamingBaseUrl = preflight.streamingBaseUrl.isEmpty ? StreamPreferences.loadSelectedStreamingBaseUrl(forGame: configuration.applicationID) : preflight.streamingBaseUrl
        return PreparedStreamLaunch(settings: settings, streamingBaseUrl: streamingBaseUrl)
    }

    private func fetchCloudVariables(configuration: PreparedLaunchConfiguration) async -> StreamCloudVariables {
        await withCheckedContinuation { continuation in
            StreamPreferences.fetchCloudVariables(token: configuration.accessToken, userId: configuration.metadata["userId"] ?? "", idpId: configuration.metadata["idpId"] ?? "") { variables in
                continuation.resume(returning: variables)
            }
        }
    }

    private func runNetworkPreflight(token: String, requestedMaxBitrateMbps: Int) async -> StreamNetworkPreflightResult {
        await withCheckedContinuation { continuation in
            StreamPreferences.runNetworkPreflight(
                token: token,
                providerStreamingBaseUrl: GameService.shared.providerStreamingBaseURL(),
    ...
    private func settingsByApplyingCloudVariables(_ settings: [String: Any], variables: StreamCloudVariables) -> [String: Any] {
        let capabilities = StreamPreferences.loadDeviceCapabilities()
        ...
        var result = settings
        result.merge(resolved.dictionary(gameLanguage: string(settings["gameLanguage"], fallback: GFNLocale.currentGFNLocale()), accountLinked: bool(settings["accountLinked"], fallback: true), selectedStore: string(settings["selectedStore"]))) { _, new in new }
        return result
    }
    ...
    private func connectSignaling(sessionInfo: AllocatedStreamSession, settings: [String: Any], descriptor: StreamSessionDescriptor) async throws -> StreamOffer {
        try await withCheckedThrowingContinuation { continuation in
            ...
            client.onClosed = { [weak self] clean, reason in
                ...
                self.resumeOffer(error: StreamSessionError.signalingFailed(reason.isEmpty ? "Signaling connection closed before receiving an offer." : reason))
            }
            ...
            client.connect { [weak self] success, error in
                ...
                self.resumeOffer(error: StreamSessionError.signalingFailed(error.isEmpty ? "Unable to connect signaling." : error))
            }
        }
    }
    ...
    private func startOfferTimeout(client: NVSTWebSocketSignalingClient, descriptor: StreamSessionDescriptor) {
        DispatchQueue.main.asyncAfter(deadline: .now() + 20.0) { [weak self, weak client] in
            ...
            client.disconnect()
            self.resumeOffer(error: StreamSessionError.signalingFailed("Signaling connected but no stream offer was received within 20 seconds for session \(descriptor.id)."))
        }
    }
    ...
    private func streamDescriptor(sessionInfo: AllocatedStreamSession, configuration: PreparedLaunchConfiguration) -> StreamSessionDescriptor {
    ...
    private func isFreeTierSession(configuration: PreparedLaunchConfiguration, sessionInfo: AllocatedStreamSession) -> Bool {
    ...
    private func stopCloudMatchSession(_ session: StreamSessionDescriptor) async -> Error? {
        await withCheckedContinuation { continuation in
            let completion = SessionStopCompletion(continuation: continuation)
            DispatchQueue.global(qos: .utility).asyncAfter(deadline: .now() + 10) {
                completion.resume(StreamSessionError.sessionStopFailed("Cloud session stop timed out."))
            }
            ActiveSessionService.stopSession(
                accessToken: session.metadata["accessToken"] ?? "",
                sessionId: session.id,
                serverIp: session.serverAddress,
                streamingBaseUrl: session.metadata["streamingBaseUrl"] ?? StreamPreferences.loadSelectedStreamingBaseUrl()
            ) { success, error in
                if success || (session.metadata["accessToken"] ?? "").isEmpty {
                    completion.resume(nil)
                } else {
                    completion.resume(StreamSessionError.sessionStopFailed(error.isEmpty ? "Unable to stop stream session." : error))
                }
            }
        }
    }
    ...
    private func reportUDSEndOfSession(_ session: StreamSessionDescriptor, reason: StreamEndReason) async {
        ...
        let payload = UDSReportPayload(
            source: .endOfSession,
            locale: GFNLocale.currentGFNLocale(),
            deviceId: DeviceIdentity.stableCloudmatchDeviceId(),
            sessionId: session.id,
    ...
    private func makeSettings(configuration: PreparedLaunchConfiguration) -> [String: Any] {
        let capabilities = StreamPreferences.loadDeviceCapabilities()
        let profile = StreamPreferences.launchProfile(forGame: configuration.applicationID, capabilities: capabilities)
        let resolved = WebRTCMediaStreamSettingsResolver.resolve(
        ...
        return resolved.dictionary(gameLanguage: GFNLocale.currentGFNLocale(), accountLinked: configuration.accountLinked, selectedStore: configuration.selectedStore)
    }

    private func normalizedConfiguration(_ configuration: PreparedLaunchConfiguration, appId: String) -> PreparedLaunchConfiguration {
...
public enum StreamSessionError: LocalizedError, Sendable {
