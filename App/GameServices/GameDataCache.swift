...
@objc(GameDataCache)
final class GameDataCache: NSObject, @unchecked Sendable {
    @objc(shared)
    ...
    private let ioQueue = DispatchQueue(label: "com.pixelnow.game-data-cache.io", qos: .utility)

    private override init() {
        let baseURL = FileManager.default.urls(for: .cachesDirectory, in: .userDomainMask).first
            ?? URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
        let currentRootURL = baseURL.appendingPathComponent("PixelNOW/GameData", isDirectory: true)
        rootPath = currentRootURL.path
    ...
    func loadCatalog(key: String) -> CatalogBrowseResult? {
        loadCatalog(key: key, requireFreshness: false, maxAgeSeconds: 0)
    }

    func loadFreshCatalog(key: String, maxAgeSeconds: TimeInterval) -> CatalogBrowseResult? {
    ...
    func loadFreshCatalogAndDefinitions(
        ...
        completion: @escaping @Sendable (CatalogBrowseResult?, NSDictionary?) -> Void
    ) {
    ...
    func loadCatalogAsync(key: String, completion: @escaping @Sendable (CatalogBrowseResult?) -> Void) {
        ioQueue.async { [self] in
    ...
    func saveCatalog(key: String, result: CatalogBrowseResult) {
    ...
    private func loadCatalog(key: String, requireFreshness: Bool, maxAgeSeconds: TimeInterval) -> CatalogBrowseResult? {
    ...
    private func gameDictionary(_ game: GameInfo) -> [String: Any] {
    ...
    private func gameInfo(_ value: Any) -> GameInfo {
    ...
    private func variantDictionary(_ variant: GameVariant) -> [String: Any] {
        var dictionary: [String: Any] = [:]
    ...
    private func gameVariant(_ value: Any) -> GameVariant {
        let dictionary = value as? [String: Any] ?? [:]
        return GameVariant(
