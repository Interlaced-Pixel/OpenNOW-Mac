...
enum GameOwnershipRemediationKind: Int, Sendable {
    case none = 0
    case purchaseOrAdd
    case linkAccount
    case installToPlay
...
struct GameOwnershipRemediation: Equatable, Sendable {
    var kind: GameOwnershipRemediationKind = .none
...
enum GameRemediation {
    ...
    static func gameOwnershipRemediationForLaunch(
        ...
        guard storeVariantIndex >= 0 else { return GameOwnershipRemediation() }
        ...
        return GameOwnershipRemediation()
    }

    private static func firstVariantWithStoreURL(in game: GameInfo) -> Int {
        game.variants.firstIndex { !$0.storeUrl.isEmpty } ?? -1
    }

    private static func variant(at index: Int, in game: GameInfo) -> GameVariant? {
        guard index >= 0, index < game.variants.count else { return nil }
        return game.variants[index]
    }

    private static func gameTitle(_ game: GameInfo) -> String {
        game.title.isEmpty ? "Selected Game" : game.title
    }

    private static func makeRemediation(
        kind: GameOwnershipRemediationKind,
        storeVariantIndex: Int,
        gameTitle: String,
        storeName: String
    ) -> GameOwnershipRemediation {
        var remediation = GameOwnershipRemediation()
...
@objc(GameRemediationBridge)
final class GameRemediationBridge: NSObject {
    ...
    static func gameVariantOwnedForLaunch(inLibrary: Bool, librarySelected: Bool, serviceStatus: String) -> Bool {
        inLibrary || librarySelected || GameRemediation.gameServiceStatusOwnedForLaunch(serviceStatus)
    }

    @objc(gameStoreDisplayName:)
    static func gameStoreDisplayName(_ store: String) -> String {
        GameRemediation.gameStoreDisplayName(store)
    }
}
