...
typealias CatalogCallback = @Sendable (_ success: Bool, _ games: [GameInfo], _ error: String) -> Void
typealias CatalogBrowseCallback = @Sendable (_ success: Bool, _ result: CatalogBrowseResult, _ error: String) -> Void
typealias SubscriptionCallback = @Sendable (_ success: Bool, _ subscription: SubscriptionInfo, _ error: String) -> Void
typealias StoreURLCallback = @Sendable (_ success: Bool, _ storeURL: String, _ error: String) -> Void
typealias LaunchAppIdCallback = @Sendable (_ appId: String) -> Void
typealias ProviderInfoCallback = @Sendable (_ success: Bool, _ providerInfo: GameProviderInfo, _ selectedEndpoint: GameProviderEndpoint, _ error: String) -> Void
typealias OwnershipActionCallback = @Sendable (_ success: Bool, _ error: String) -> Void
typealias FavoriteActionCallback = @Sendable (_ success: Bool, _ error: String) -> Void
typealias UserAccountCallback = @Sendable (_ success: Bool, _ accountInfo: UserAccountInfo, _ error: String) -> Void
typealias StoreDefinitionsCallback = @Sendable (_ success: Bool, _ definitions: [StoreDefinition], _ error: String) -> Void
typealias SubscriptionDefinitionsCallback = @Sendable (_ success: Bool, _ definitions: [SubscriptionDefinition], _ error: String) -> Void
typealias AppPatchStatusesCallback = @Sendable (_ success: Bool, _ statuses: [String: AppPatchStatus], _ error: String) -> Void

public struct AppPatchStatus: Equatable, Sendable {
    public var appId = ""
...
final class GameService: @unchecked Sendable {
    ...
    private static let workQueue = DispatchQueue(label: "com.pixelnow.game-service.swift.work")
    ...
    private var providerStreamingBaseUrl = GameService.defaultStreamingBaseUrl
    ...
    func fetchProviderInfo(idpId: String, completion: @escaping ProviderInfoCallback) {
        guard let url = URL(string: Self.providerServiceUrlsEndpoint) else {
            dispatchProviderInfo(completion, false, GameProviderInfo(), GameProviderEndpoint(), "Invalid provider info URL")
            return
        }
        ...
        URLSession.shared.dataTask(with: tracedRequest) { [weak self] data, response, error in
            ...
            guard error == nil, let data, statusCode == 200 else {
                var endpoint = GameProviderEndpoint()
                endpoint.streamingServiceUrl = Self.defaultStreamingBaseUrl
                self.providerStreamingBaseUrl = endpoint.streamingServiceUrl
                self.dispatchProviderInfo(completion, false, GameProviderInfo(), endpoint, error?.localizedDescription ?? "Provider info request failed")
                return
            }
    ...
    func fetchMarqueePanels(completion: @escaping PanelCallback) {
    ...
    func fetchMainPanels(completion: @escaping PanelCallback) {
    ...
    func fetchCatalogGames(completion: @escaping CatalogCallback) {
    ...
    private func continueBrowseCatalogGames(
        ...
        GameDataCache.shared.loadFreshCatalogAndDefinitions(
            key: catalogCacheKey,
            locale: locale,
        ...
        let loadDefinitions: @Sendable () -> Void = { [weak self] in
            ...
            GameDataCache.shared.loadCatalogDefinitionsAsync(locale: parameters.locale, maxAgeSeconds: Self.catalogDefinitionsFreshSeconds) { [weak self] cachedDefinitions in
                ...
                self.postGraphQlJson(query: definitionsQuery, variables: ["locale": parameters.locale] as NSDictionary) { [weak self] data, error in
                    guard let self else { return }
                    if error.isEmpty, let data { GameDataCache.shared.saveCatalogDefinitionsAsync(locale: parameters.locale, definitions: data) }
    ...
    private func handleCatalogDefinitions(
        ...
        if !definitionsError.isEmpty {
            if !deliveredCachedResult.value { dispatchCatalogBrowse(completion, false, CatalogBrowseResult(), definitionsError) }
            return
        }
    ...
    private static func defaultSortOption(searchQuery: String) -> CatalogSortOption {
        if defaultSortId(searchQuery: searchQuery) == defaultSearchSortId {
            return CatalogSortOption(id: defaultSearchSortId, label: "Relevance", orderBy: "itemMetadata.relevance:DESC,sortName:ASC")
        }
    ...
    private func fetchDefaultLibrarySort(locale: String, completion: @escaping @Sendable (CatalogSortOption) -> Void) {
        let fallback = Self.defaultSortOption(searchQuery: "")
        GameDataCache.shared.loadCatalogDefinitionsAsync(locale: locale, maxAgeSeconds: Self.catalogDefinitionsFreshSeconds) { [weak self] cachedDefinitions in
            guard let self else { return }
            if let cachedDefinitions {
            ...
            self.postGraphQlJson(query: query, variables: ["locale": locale] as NSDictionary) { [weak self] data, error in
                ...
                if error.isEmpty, let data {
                    GameDataCache.shared.saveCatalogDefinitionsAsync(locale: locale, definitions: data)
    ...
    private func defaultLibrarySort(from definitionsData: NSDictionary, fallback: CatalogSortOption) -> CatalogSortOption {
        var result = CatalogBrowseResult()
    ...
    func fetchPublicGames(completion: @escaping CatalogCallback) {
    ...
    func fetchSubscriptionInfo(userId: String, completion: @escaping SubscriptionCallback) {
        let token = accessToken
        guard !token.isEmpty else {
            dispatchSubscription(completion, false, SubscriptionInfo(), "No access token")
            return
        }
        guard !userId.isEmpty else {
            dispatchSubscription(completion, false, SubscriptionInfo(), "No user ID")
            return
        }
        ...
        getServerVpcId(token: token, providerStreamingBaseUrl: providerStreamingBaseURL()) { [weak self] resolvedVpcId in
            ...
            guard let url = components?.url else {
                self.dispatchSubscription(completion, false, SubscriptionInfo(), "Invalid subscription URL")
                return
            }
            ...
            URLSession.shared.dataTask(with: tracedRequest) { data, response, error in
                ...
                Self.workQueue.async {
                    ...
                    guard statusCode == 200, let json else {
                        self.dispatchSubscription(completion, false, SubscriptionInfo(), "Subscription API failed (\(statusCode))")
                        return
                    }
    ...
    func resolveStoreURL(game: GameInfo, variantIndex: Int, completion: @escaping StoreURLCallback) {
    ...
    func resolveLaunchAppId(game: GameInfo, variantIndex: Int, completion: @escaping LaunchAppIdCallback) {
    ...
    func fetchGameByCMSId(_ cmsId: String, completion: @escaping CatalogCallback) {
    ...
    func fetchUserAccount(completion: @escaping UserAccountCallback) {
        let query = """
        ...
        postGraphQlJson(query: query, variables: [:] as NSDictionary) { [weak self] data, error in
            guard let self else { return }
            if !error.isEmpty {
                self.dispatchUserAccount(completion, false, UserAccountInfo(), error)
                return
            }
    ...
    func fetchStoreDefinitions(completion: @escaping StoreDefinitionsCallback) {
        let query = """
    ...
    func fetchSubscriptionDefinitions(completion: @escaping SubscriptionDefinitionsCallback) {
        let query = """
    ...
    func fetchAppPatchStatuses(appIds: [String], completion: @escaping AppPatchStatusesCallback) {
    ...
    func fetchLibraryPatchStatuses(completion: @escaping AppPatchStatusesCallback) {
        getServerVpcId(token: accessToken, providerStreamingBaseUrl: providerStreamingBaseURL()) { [weak self] resolvedVpcId in
            guard let self else { return }
            let query = """
    ...
    func addOwnedVariant(_ variantId: String, completion: @escaping OwnershipActionCallback) {
        ownedVariantMutation(mutationName: "AddOwnedVariant", fieldName: "addOwnedVariant", variantId: variantId, completion: completion)
    }

    func removeOwnedVariant(_ variantId: String, completion: @escaping OwnershipActionCallback) {
    ...
    func selectOwnedVariant(_ variantId: String, completion: @escaping OwnershipActionCallback) {
    ...
    func addFavoriteApp(_ appId: String, completion: @escaping FavoriteActionCallback) {
        favoriteAppMutation(mutationName: "AddFavoriteApp", fieldName: "addFavoriteApp", appId: appId, completion: completion)
    }

    func removeFavoriteApp(_ appId: String, completion: @escaping FavoriteActionCallback) {
        favoriteAppMutation(mutationName: "RemoveFavoriteApp", fieldName: "removeFavoriteApp", appId: appId, completion: completion)
    }

    func syncAccountProvider(store: String, completion: @escaping OwnershipActionCallback) {
    ...
    func startAccountLinking(store: String, completion: @escaping OwnershipActionCallback) {
        let token = accountLinkingToken.isEmpty ? accessToken : accountLinkingToken
        guard !token.isEmpty else {
            dispatchOwnership(completion, false, "Missing account-linking token")
            return
    ...
    private func fetchPanels(operationName: String, hash: String, panelNames: [String], missingMessage: String, completion: @escaping PanelCallback) {
        getServerVpcId(token: accessToken, providerStreamingBaseUrl: providerStreamingBaseURL()) { [weak self] resolvedVpcId in
            guard let self else { return }
            ...
            postGraphQlJson(query: query, variables: variables as NSDictionary) { [weak self] data, error in
                ...
                Self.workQueue.async { [dataBox] in
                    ...
                    guard let apps = dataBox?.value["apps"] as? NSDictionary else {
                        if !deliveredCachedResult.value { self.dispatchCatalogBrowse(completion, false, CatalogBrowseResult(), "No apps data") }
                        return
                    }
    ...
    private func enrichPanelResults(_ panels: [PanelResult], vpcId: String, completion: @escaping @Sendable ([PanelResult]) -> Void) {
        ...
        enrichGames(games, vpcId: vpcId) { enrichedGames in
            var enrichedById: [String: GameInfo] = [:]
    ...
    private func parseGameItem(_ app: NSDictionary?) -> GameInfo {
        guard let app else { return GameInfo() }
        var game = GameInfo()
    ...
    private func parsePanelResults(_ rawPanels: [NSDictionary]) -> [PanelResult] {
    ...
    private func parsePanelTile(_ item: NSDictionary) -> PanelTile? {
    ...
    private func parseCatalogDefinitions(_ definitionsData: NSDictionary?, result: inout CatalogBrowseResult) -> [String: [String: Any]] {
        ...
        for groupRaw in groups {
            ...
            for entry in filters {
                ...
                group.options.append(CatalogFilterOption(id: filterId, rawId: filterId, label: safeString(entry["label"]) ?? filterId, groupId: group.id, groupLabel: group.label))
            }
        ...
        for sort in sorts {
            ...
            if !id.isEmpty, !orderBy.isEmpty {
                result.sortOptions.append(CatalogSortOption(id: id, label: safeString(sort["label"]) ?? id, orderBy: orderBy))
            }
        }
        return filterPayloadById
    }

    private func parseUserAccountInfo(_ data: NSDictionary?) -> UserAccountInfo {
    ...
    private func parseStoreDefinitions(_ data: NSDictionary?) -> [StoreDefinition] {
        var definitions: [StoreDefinition] = []
        for storeData in data?["appStoreDefinitions"] as? [NSDictionary] ?? [] {
            var definition = StoreDefinition()
            ...
            for featureData in storeData["features"] as? [NSDictionary] ?? [] {
                let feature = StoreFeatureInfo(type: safeString(featureData["__typename"]) ?? "", displayProposition: safeString(featureData["displayProposition"]) ?? "", supported: safeBool(featureData["supported"]))
    ...
    private func parseSubscriptionDefinitions(_ data: NSDictionary?) -> [SubscriptionDefinition] {
    ...
    private func parseAppPatchStatuses(_ apps: [NSDictionary]) -> [String: AppPatchStatus] {
        var statuses: [String: AppPatchStatus] = [:]
        for app in apps {
            let appId = safeString(app["id"]) ?? ""
            guard !appId.isEmpty else { continue }
            var status = AppPatchStatus(appId: appId)
    ...
    private func parseSubscriptionInfo(_ json: NSDictionary) -> SubscriptionInfo {
    ...
    fileprivate func parseGameProviderInfo(_ json: NSDictionary?) -> GameProviderInfo {
        let raw = providerInfoDictionary(json)
        var info = GameProviderInfo()
    ...
    fileprivate func selectGameProviderEndpoint(_ info: GameProviderInfo, idpId: String) -> GameProviderEndpoint {
        ...
        var fallback = GameProviderEndpoint()
        fallback.loginProvider = info.defaultProvider
        fallback.streamingServiceUrl = Self.defaultStreamingBaseUrl
        return fallback
    }

    private func ownedVariantMutation(mutationName: String, fieldName: String, variantId: String, completion: @escaping OwnershipActionCallback) {
    ...
    private func favoriteAppMutation(mutationName: String, fieldName: String, appId: String, completion: @escaping FavoriteActionCallback) {
        guard !appId.isEmpty else {
    ...
    private func isAppPatchingStatus(_ value: Any?) -> Bool {
    ...
    private func appendDirectImage(_ value: String?, type: String, width: Int, game: inout GameInfo) {
    ...
    private func assignContentRatings(_ rawValue: Any?, to game: inout GameInfo) {
        guard let rawValue, !(rawValue is NSNull) else { return }
        var values: [String] = []
    ...
    private func mergeVariantFromSameStore(target: inout GameVariant, source: GameVariant) -> Bool {
        var changed = false
    ...
    private func mergeMissingStoreMetadata(target: inout GameInfo, metadata: GameInfo) {
    ...
    private func variantMatchesStoreMetadata(target: GameVariant, metadata: GameVariant) -> Bool {
    ...
    private static func variantIsRelevant(_ variant: GameVariant) -> Bool {
        !variant.appStore.isEmpty || variant.inLibrary || variant.librarySelected || LaunchAppId.resolve(variant.id) != nil
    }
    ...
    private func storeURLForKnownGame(_ game: GameInfo, variantIndex: Int) -> String? {
    ...
    private func launchableAppId(for game: GameInfo, variantIndex: Int) -> String? {
    ...
    private func launchableAppId(for game: GameInfo, preferredStore: String) -> String? {
    ...
    private func validLaunchAppId(_ value: String) -> String? {
        LaunchAppId.resolve(value)?.stringValue
    }

    private func storeURLForMetadataGame(_ metadataGame: GameInfo, variantId: String, store: String) -> String {
    ...
    private func deduplicateGames(_ games: [GameInfo]) -> [GameInfo] {
        var byId: [String: GameInfo] = [:]
    ...
    private func fetchPublicGamesLocale(locales: [String], index: Int, completion: @escaping CatalogCallback) {
        ...
        URLSession.shared.dataTask(with: tracedRequest) { [weak self] data, response, error in
            ...
            Self.workQueue.async {
                ...
                let games = publicGames.compactMap { item -> GameInfo? in
    ...
    private func dispatchPanel(_ completion: @escaping PanelCallback, _ success: Bool, _ panels: [PanelResult], _ error: String) {
        DispatchQueue.main.async { completion(success, panels, error) }
    }

    private func dispatchCatalog(_ completion: @escaping CatalogCallback, _ success: Bool, _ games: [GameInfo], _ error: String) {
        DispatchQueue.main.async { completion(success, games, error) }
    }

    private func dispatchCatalogBrowse(_ completion: @escaping CatalogBrowseCallback, _ success: Bool, _ result: CatalogBrowseResult, _ error: String) {
        DispatchQueue.main.async { completion(success, result, error) }
    }

    private func dispatchSubscription(_ completion: @escaping SubscriptionCallback, _ success: Bool, _ subscription: SubscriptionInfo, _ error: String) {
        DispatchQueue.main.async { completion(success, subscription, error) }
    }

    private func dispatchStoreURL(_ completion: @escaping StoreURLCallback, _ success: Bool, _ storeURL: String, _ error: String) {
        DispatchQueue.main.async { completion(success, storeURL, error) }
    }

    private func dispatchProviderInfo(_ completion: @escaping ProviderInfoCallback, _ success: Bool, _ providerInfo: GameProviderInfo, _ selectedEndpoint: GameProviderEndpoint, _ error: String) {
        DispatchQueue.main.async { completion(success, providerInfo, selectedEndpoint, error) }
    }

    fileprivate func dispatchOwnership(_ completion: @escaping OwnershipActionCallback, _ success: Bool, _ error: String) {
        DispatchQueue.main.async { completion(success, error) }
    }

    fileprivate func dispatchFavorite(_ completion: @escaping FavoriteActionCallback, _ success: Bool, _ error: String) {
        DispatchQueue.main.async { completion(success, error) }
    }

    private func dispatchUserAccount(_ completion: @escaping UserAccountCallback, _ success: Bool, _ accountInfo: UserAccountInfo, _ error: String) {
        DispatchQueue.main.async { completion(success, accountInfo, error) }
    }

    private func dispatchStoreDefinitions(_ completion: @escaping StoreDefinitionsCallback, _ success: Bool, _ definitions: [StoreDefinition], _ error: String) {
        DispatchQueue.main.async { completion(success, definitions, error) }
    }

    private func dispatchSubscriptionDefinitions(_ completion: @escaping SubscriptionDefinitionsCallback, _ success: Bool, _ definitions: [SubscriptionDefinition], _ error: String) {
        DispatchQueue.main.async { completion(success, definitions, error) }
    }

    private func dispatchAppPatchStatuses(_ completion: @escaping AppPatchStatusesCallback, _ success: Bool, _ statuses: [String: AppPatchStatus], _ error: String) {
        DispatchQueue.main.async { completion(success, statuses, error) }
    }
}

@objc(GameServiceSwiftAdapter)
public final class GameServiceSwiftAdapter: NSObject {
    ...
    static func fetchProviderInfo(idpId: String, completion: @escaping @Sendable (Bool, ParsedGameProviderInfo, ParsedGameProviderEndpoint, String) -> Void) {
        GameService.shared.fetchProviderInfo(idpId: idpId) { success, info, endpoint, error in
            completion(success, ParsedGameProviderInfo(info: info), ParsedGameProviderEndpoint(endpoint: endpoint), error)
        }
    }

    public static func fetchGameProviderInfo(idpId: String, completion: @escaping @Sendable (Bool, GameProviderInfo, GameProviderEndpoint, String) -> Void) {
        GameService.shared.fetchProviderInfo(idpId: idpId) { success, info, endpoint, error in
            completion(success, info, endpoint, error)
        }
    }

    public static func selectGameProviderEndpoint(from info: GameProviderInfo, idpId: String) -> GameProviderEndpoint {
        GameService.shared.selectGameProviderEndpoint(info, idpId: idpId)
    }

    @objc(fetchSubscriptionInfoWithUserId:completion:)
    public static func fetchSubscriptionInfo(userId: String, completion: @escaping @Sendable (Bool, ParsedSubscriptionInfo, String) -> Void) {
        GameService.shared.fetchSubscriptionInfo(userId: userId) { success, subscription, error in
            completion(success, ParsedSubscriptionInfo(subscription: subscription), error)
        }
    }

    @objc(fetchMainPanelObjectsWithCompletion:)
    public static func fetchMainPanelObjects(completion: @escaping @Sendable (Bool, [CatalogPanelObject], String) -> Void) {
        GameService.shared.fetchMainPanels { success, panels, error in
            completion(success, panels.map(CatalogPanelObject.init), error)
        }
    }

    @objc(fetchMarqueePanelObjectsWithCompletion:)
    public static func fetchMarqueePanelObjects(completion: @escaping @Sendable (Bool, [CatalogPanelObject], String) -> Void) {
        GameService.shared.fetchMarqueePanels { success, panels, error in
            completion(success, panels.map(CatalogPanelObject.init), error)
        }
    }

    @objc(browseCatalogObjectWithSearchQuery:sortId:filterIds:fetchCount:completion:)
    public static func browseCatalogObject(searchQuery: String, sortId: String, filterIds: [String], fetchCount: Int, completion: @escaping @Sendable (Bool, CatalogBrowseResultObject, String) -> Void) {
        GameService.shared.browseCatalogGames(searchQuery: searchQuery, sortId: sortId, filterIds: filterIds, fetchCount: fetchCount, forceRefresh: false) { success, result, error in
            completion(success, CatalogBrowseResultObject(result: result), error)
        }
    }

    public static func browseCatalogObject(searchQuery: String, sortId: String, filterIds: [String], fetchCount: Int, forceRefresh: Bool, completion: @escaping @Sendable (Bool, CatalogBrowseResultObject, String) -> Void) {
        GameService.shared.browseCatalogGames(searchQuery: searchQuery, sortId: sortId, filterIds: filterIds, fetchCount: fetchCount, forceRefresh: forceRefresh) { success, result, error in
            completion(success, CatalogBrowseResultObject(result: result), error)
        }
    }
    ...
    public static func fetchFavoriteGameObjects(completion: @escaping @Sendable (Bool, [CatalogGameObject], String) -> Void) {
        GameService.shared.fetchFavoriteGames { success, games, error in
            completion(success, games.map(CatalogGameObject.init), error)
        }
    }

    public static func fetchGameObjectByCMSId(_ cmsId: String, completion: @escaping @Sendable (Bool, CatalogGameObject?, String) -> Void) {
        GameService.shared.fetchGameByCMSId(cmsId) { success, games, error in
            completion(success, games.first.map(CatalogGameObject.init), error)
        }
    }
    ...
    public static func fetchUserAccountDictionary(completion: @escaping @Sendable (Bool, NSDictionary, String) -> Void) {
        GameService.shared.fetchUserAccount { success, account, error in
    ...
    public static func fetchStoreDefinitionDictionaries(completion: @escaping @Sendable (Bool, [NSDictionary], String) -> Void) {
        GameService.shared.fetchStoreDefinitions { success, definitions, error in
    ...
    public static func fetchSubscriptionDefinitionDictionaries(completion: @escaping @Sendable (Bool, [NSDictionary], String) -> Void) {
        GameService.shared.fetchSubscriptionDefinitions { success, definitions, error in
            completion(success, definitions.map(subscriptionDefinitionDictionary), error)
        }
    }

    public static func fetchAppPatchStatuses(appIds: [String], completion: @escaping @Sendable (Bool, [String: AppPatchStatus], String) -> Void) {
        GameService.shared.fetchAppPatchStatuses(appIds: appIds, completion: completion)
    }

    public static func fetchLibraryPatchStatuses(completion: @escaping @Sendable (Bool, [String: AppPatchStatus], String) -> Void) {
        GameService.shared.fetchLibraryPatchStatuses(completion: completion)
    }
    ...
    public static func addOwnedVariant(_ variantId: String, completion: @escaping @Sendable (Bool, String) -> Void) {
        GameService.shared.addOwnedVariant(variantId, completion: completion)
    }
    ...
    public static func removeOwnedVariant(_ variantId: String, completion: @escaping @Sendable (Bool, String) -> Void) {
        GameService.shared.removeOwnedVariant(variantId, completion: completion)
    }
    ...
    public static func selectOwnedVariant(_ variantId: String, completion: @escaping @Sendable (Bool, String) -> Void) {
        GameService.shared.selectOwnedVariant(variantId, completion: completion)
    }
    ...
    public static func addFavoriteApp(_ appId: String, completion: @escaping @Sendable (Bool, String) -> Void) {
        GameService.shared.addFavoriteApp(appId, completion: completion)
    }
    ...
    public static func removeFavoriteApp(_ appId: String, completion: @escaping @Sendable (Bool, String) -> Void) {
        GameService.shared.removeFavoriteApp(appId, completion: completion)
    }
    ...
    public static func syncAccountProvider(store: String, completion: @escaping @Sendable (Bool, String) -> Void) {
        GameService.shared.syncAccountProvider(store: store, completion: completion)
    }
    ...
    public static func startAccountLinking(store: String, completion: @escaping @Sendable (Bool, String) -> Void) {
        GameService.shared.startAccountLinking(store: store, completion: completion)
    }

    @objc(resolveStoreURLWithGameObject:variantIndex:completion:)
    public static func resolveStoreURL(game: CatalogGameObject, variantIndex: Int, completion: @escaping @Sendable (Bool, String, String) -> Void) {
        GameService.shared.resolveStoreURL(game: game.swiftValue, variantIndex: variantIndex, completion: completion)
    }

    @objc(optimizeImageURL:width:)
    public static func optimizeImageURL(_ url: String, width: Int) -> String {
        GameService.optimizeImageURL(url, width: width)
    }

    @objc(parseProviderInfoFromJSON:)
    static func parseProviderInfo(from json: NSDictionary?) -> ParsedGameProviderInfo {
        ParsedGameProviderInfo(info: GameService.shared.parseGameProviderInfo(json))
    }

    @objc(selectProviderEndpointFromInfo:idpId:)
    static func selectProviderEndpoint(from info: ParsedGameProviderInfo, idpId: String) -> ParsedGameProviderEndpoint {
        let swiftInfo = info.swiftValue
        return ParsedGameProviderEndpoint(endpoint: GameService.shared.selectGameProviderEndpoint(swiftInfo, idpId: idpId))
    }

    private static func userAccountDictionary(_ account: UserAccountInfo) -> NSDictionary {
        [
    ...
    private static func storeAccountDictionary(_ account: StoreAccountInfo) -> NSDictionary {
        [
            "store": account.store,
    ...
    private static func storeDefinitionDictionary(_ definition: StoreDefinition) -> NSDictionary {
        [
    ...
    private static func subscriptionDefinitionDictionary(_ definition: SubscriptionDefinition) -> NSDictionary {
        [
...
@objcMembers
public final class ParsedSubscriptionInfo: NSObject {
...
@objcMembers
final class ParsedGameProviderEndpoint: NSObject {
    ...
    init(endpoint: GameProviderEndpoint) {
    ...
    var swiftValue: GameProviderEndpoint {
        var endpoint = GameProviderEndpoint()
...
@objcMembers
final class ParsedGameProviderInfo: NSObject {
    let defaultProvider: String
    let loggedInProvider: String
    let loginRequired: Bool
    let loginPreferredProviders: [String]
    let endpoints: [ParsedGameProviderEndpoint]

    init(info: GameProviderInfo) {
        ...
        endpoints = info.endpoints.map(ParsedGameProviderEndpoint.init(endpoint:))
    }

    var swiftValue: GameProviderInfo {
        var info = GameProviderInfo()
...
private final class CatalogPageState: @unchecked Sendable {
    ...
    init(result: CatalogBrowseResult) {
        self.result = result
    }
}
...
private struct RatingMetadata: Sendable {
    ...
    func apply(to game: inout GameInfo) {
...
private final class AccountLinkingCallbackListener: @unchecked Sendable {
    ...
    func wait(timeout: TimeInterval, completion: @escaping OwnershipActionCallback) {
