...
public struct NetworkLogContext: Sendable {
    ...
    public let trace: SentryTransaction?

    fileprivate init(operation: String, startedAt: Date, requestSummary: String, trace: SentryTransaction?) {
...
public enum NetworkLog {
    public static func start(_ request: inout URLRequest, operation: String) -> NetworkLogContext {
        let trace = Sentry.traceHTTPRequest(&request, name: readableOperationName(operation))
    ...
    public static func finish(_ request: URLRequest, operation: String, startedAt context: NetworkLogContext, data: Data?, response: URLResponse?, error: Error?) {
        ...
        if outcome == "success" {
            Sentry.logInfoMessage(message)
        } else {
            Sentry.logWarningMessage(message)
        }
    }

    public static func graphQLStart(_ request: inout URLRequest, operationName: String, queryHash: String, variables: NSDictionary?) -> NetworkLogContext {
        let trace = Sentry.traceHTTPRequest(&request, name: "GraphQL \(operationName.isEmpty ? "operation" : operationName)")
    ...
    public static func graphQLFinish(_ request: URLRequest, operationName: String, queryHash: String, startedAt context: NetworkLogContext, data: Data?, response: URLResponse?, error: Error?, responseMessage: String) {
        let durationMilliseconds = context.durationMilliseconds
        let byteCount = data?.count ?? 0
        let statusCode = (response as? HTTPURLResponse)?.statusCode ?? -1
    ...
    public static func webSocketEvent(_ event: String, url: URL?, detail: String = "") {
        ...
        Sentry.logInfoMessage(logMessage(level: "info", area: "WebSocket", message: "Event \(event) url=\(sanitizedURL(url))\(detailText)"))
    }
    ...
    private static func startContext(_ request: URLRequest, operation: String, trace: SentryTransaction?) -> NetworkLogContext {
        let context = NetworkLogContext(operation: operation, startedAt: Date(), requestSummary: requestSummary(request), trace: trace)
        trace?.setTag("pixelnow.operation", value: operation)
        if shouldLogStart(operation: operation) {
            Sentry.logInfoMessage(logMessage(level: "info", area: "Network", message: "HTTP request started operation=\(operation) request=\(context.requestSummary)"))
        }
        return context
    }

    private static func graphQLStartContext(_ request: URLRequest, operationName: String, queryHash: String, variables: NSDictionary?, trace: SentryTransaction?) -> NetworkLogContext {
        let operation = "graphql.\(operationName.isEmpty ? "operation" : operationName)"
        let context = NetworkLogContext(operation: operation, startedAt: Date(), requestSummary: requestSummary(request), trace: trace)
        if shouldLogGraphQLStart(operationName: operationName) {
            Sentry.logInfoMessage(logMessage(level: "info", area: "GraphQL", message: "Request started operation=\(operationName) hash=\(queryHash) variableKeys=\(sortedKeys(in: variables)) request=\(context.requestSummary)"))
        }
        return context
    }

    private static func finishTrace(_ trace: SentryTransaction?, operation: String, statusCode: Int, durationMilliseconds: Int, byteCount: Int, outcome: String) {
        guard let trace else { return }
        trace.setTag("pixelnow.operation", value: operation)
        trace.setTag("pixelnow.outcome", value: outcome)
        if statusCode >= 0 {
    ...
    private static func recordHTTPMetrics(request: URLRequest, operation: String, statusCode: Int, durationMilliseconds: Int, byteCount: Int, outcome: String, error: Error?) {
        ...
        _ = Sentry.recordCounterMetric(key: "pixelnow.http.requests.count", value: 1, attributes: attributes)
        _ = Sentry.recordDistributionMetric(key: "pixelnow.http.duration_ms", value: Double(durationMilliseconds), unit: "millisecond", attributes: attributes)
        if byteCount > 0 {
            _ = Sentry.recordDistributionMetric(key: "pixelnow.http.response_bytes", value: Double(byteCount), unit: "byte", attributes: attributes)
        }
    }
    ...
    static func sanitizedURL(_ url: URL?) -> String {
        ...
        return Sentry.sanitizedLogMessage(components.string ?? url.host ?? "unknown-url")
    }
    ...
    private static func shouldLogWebSocketEvent(_ event: String) -> Bool {
        if ["iceCandidateReceived", "receiveStopped"].contains(event) {
            return Sentry.shouldLogVerbose()
        }
        return true
    }

    private static func sanitizedDetail(_ detail: String) -> String {
        Sentry.sanitizedLogMessage(detail.replacingOccurrences(
    ...
    private static func graphQLErrorDetail(data: Data?) -> String {
        ...
        return String(Sentry.sanitizedLogMessage(singleLine).prefix(500))
    }
    ...
    private static func readableOperationName(_ operation: String) -> String {
        operation.isEmpty ? "PixelNOW network request" : operation
    }

    private static func logMessage(level: String, area: String, message: String) -> String {
        Sentry.formattedLogMessage(level: level, area: area, message: message)
    }
}
