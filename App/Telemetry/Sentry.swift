...
public final class SentryTransaction {
    ...
    public func setTag(_ key: String, value: String) {
        guard Sentry.isTelemetryEnabled(), !key.isEmpty else { return }
    ...
    public func setStatus(_ success: Bool) {
        guard Sentry.isTelemetryEnabled() else { return }
        self.success = success
    }

    public func addTraceHeaders(_ request: NSMutableURLRequest) {
        Sentry.addTraceHeaders(from: span, to: request)
    }
...
extension SentryTransaction: @unchecked Sendable {}
...
final class Sentry {
    ...
    private static let diagnosticsLogQueue = DispatchQueue(label: "pixelnow.telemetry.diagnostics-log")
    private static let maxDiagnosticsLogBytes = 8 * 1024 * 1024
    private static let maxDiagnosticsUploadBytes = 384 * 1024
    private static let telemetryDisabledKey = "PixelNOW.Telemetry.Disabled"
    ...
    public static func initializeSentry() {
        ...
        SentrySDK.start { options in
            options.dsn = dsn
            options.debug = environmentFlagEnabled("_SENTRY_DEBUG")
            options.diagnosticLevel = options.debug ? .debug : .error
            options.sendDefaultPii = !environmentFlagDisabled("_SENTRY_SEND_PII")
            options.environment = resolvedEnvironment()
            options.releaseName = resolvedReleaseName()
            options.dist = resolvedDist()
            options.sampleRate = NSNumber(value: clampedSampleRate(environmentDouble("_SENTRY_EVENT_SAMPLE_RATE") ?? 1.0))
            options.tracesSampleRate = NSNumber(value: clampedSampleRate(environmentDouble("_SENTRY_TRACES_SAMPLE_RATE") ?? 0.25))
            options.configureProfiling = {
                $0.lifecycle = .trace
                $0.sessionSampleRate = Float(clampedSampleRate(environmentDouble("_SENTRY_PROFILES_SAMPLE_RATE") ?? 1.0))
            }
    ...
    static func shouldLogInfo() -> Bool {
        isTelemetryEnabled() && !environmentFlagEnabled("_DISABLE_INFO_LOGS")
    }

    public static func shouldLogDebug() -> Bool {
        isTelemetryEnabled() && (environmentFlagEnabled("_DEBUG_LOGS") || environmentFlagEnabled("_VERBOSE_LOGS"))
    }

    public static func shouldLogVerbose() -> Bool {
        isTelemetryEnabled() && environmentFlagEnabled("_VERBOSE_LOGS")
    }
    ...
    public static func formattedLogMessage(level: String, area: String, message: String) -> String {
        ...
        return "[PixelNOW][\(resolvedLevel)][\(resolvedArea)] \(message)"
    }
    ...
    public static func uploadDiagnosticsLog(_ logText: String) async throws -> URL {
        guard let url = URL(string: "https://paste.c-net.org/") else { throw SentryDiagnosticsUploadError.invalidServiceURL }
    ...
    static func uploadDiagnosticsLog(_ logText: String, session: URLSession, uploadURL: URL) async throws -> URL {
        guard uploadURL.scheme == "https" else { throw SentryDiagnosticsUploadError.invalidServiceURL }
        ...
        case 206:
            throw SentryDiagnosticsUploadError.partialUpload
        case 413:
            throw SentryDiagnosticsUploadError.logTooLarge
        case 429:
            throw SentryDiagnosticsUploadError.rateLimited
        case 500...599:
            throw SentryDiagnosticsUploadError.serviceUnavailable(http.statusCode)
        default:
            throw SentryDiagnosticsUploadError.httpStatus(http.statusCode)
        }
    }
    ...
    public static func startTransaction(name: String, operation: String, makeCurrent: Bool) -> SentryTransaction? {
        guard isTelemetryEnabled() else { return nil }
        let resolvedName = name.isEmpty ? "PixelNOW operation" : name
        ...
        return SentryTransaction(name: resolvedName, operation: resolvedOperation, makeCurrent: makeCurrent, span: span)
    }

    public static func traceHTTPRequest(_ request: NSMutableURLRequest, name: String) -> SentryTransaction? {
    ...
    public static func traceHTTPRequest(_ request: inout URLRequest, name: String) -> SentryTransaction? {
        let transaction = startHTTPTransaction(url: request.url, method: request.httpMethod, fallbackName: name)
        guard let transaction else { return nil }
    ...
    public static func startHTTPTransaction(for request: URLRequest, name: String) -> SentryTransaction? {
    ...
    private static func resolvedDsn() -> String? {
        guard !environmentFlagEnabled("_DISABLE_SENTRY") else { return nil }
        return environmentString("_SENTRY_DSN") ?? environmentString("SENTRY_DSN") ?? fallbackDsn
    }

    private static func resolvedEnvironment() -> String {
        if let environment = environmentString("_SENTRY_ENVIRONMENT") ?? environmentString("SENTRY_ENVIRONMENT") {
            return environment
        }
        #if DEBUG
        return "debug"
    ...
    private static func resolvedReleaseName() -> String? {
        if let release = environmentString("_SENTRY_RELEASE") ?? environmentString("SENTRY_RELEASE") {
            return release
        }
    ...
    private static func resolvedDist() -> String? {
        if let dist = environmentString("_SENTRY_DIST") ?? environmentString("SENTRY_DIST") {
            return dist
        }
    ...
    private static func diagnosticsLogURL() -> URL {
        ...
        return base.appendingPathComponent("PixelNOW", isDirectory: true).appendingPathComponent("PixelNOW-diagnostics-current.log")
    }
    ...
    private static func diagnosticsUploadData(_ text: String) throws -> Data {
        ...
        guard let sanitizedData = sanitized.data(using: .utf8), !sanitizedData.isEmpty else { throw SentryDiagnosticsUploadError.emptyLog }
        guard sanitizedData.count > maxDiagnosticsUploadBytes else { return sanitizedData }
        let notice = "PixelNOW diagnostics upload\nNotice: upload is limited to the most recent \(maxDiagnosticsUploadBytes / 1024) KiB because the diagnostics paste service rejects larger payloads.\n\n"
        guard let noticeData = notice.data(using: .utf8), noticeData.count < maxDiagnosticsUploadBytes else { throw SentryDiagnosticsUploadError.emptyLog }
        ...
        throw SentryDiagnosticsUploadError.emptyLog
    }
    ...
    private static func diagnosticsPasteURL(from data: Data) throws -> URL {
        guard let responseText = String(data: data, encoding: .utf8)?.trimmingCharacters(in: .whitespacesAndNewlines),
              ...
              pasteURL.host == "paste.c-net.org" else { throw SentryDiagnosticsUploadError.invalidResponse }
        return pasteURL
    }
    ...
    private static func startHTTPTransaction(url: URL?, method: String?, fallbackName: String) -> SentryTransaction? {
...
public enum SentryDiagnosticsUploadError: LocalizedError {
    case emptyLog
