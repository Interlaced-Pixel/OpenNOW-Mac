...
public final class RemoteCoOpWebSocketSignalingSession: RemoteCoOpSignalingSession, @unchecked Sendable {
    ...
    private func send(_ message: RemoteCoOpWireMessage) async {
        do {
            let task = try await connectedTask()
            try await task.send(.string(RemoteCoOpWireCodec.encode(message)))
        } catch {
            await close()
        }
    }
    ...
    private func handle(_ message: URLSessionWebSocketTask.Message) async {
        ...
        case .string(let text):
            wireMessage = try? RemoteCoOpWireCodec.decode(text)
        case .data(let data):
            wireMessage = try? RemoteCoOpWireCodec.decode(data)
        @unknown default:
            wireMessage = nil
        }
        guard let wireMessage else { return }
        if wireMessage.kind == .heartbeat {
            await send(RemoteCoOpWireMessage(kind: .heartbeat, roomID: wireMessage.roomID))
            return
        }
