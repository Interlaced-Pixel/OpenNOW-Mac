...
public struct RemoteCoOpHostAudioFrame: Sendable {
...
public protocol RemoteCoOpHostAudioSink: AnyObject, Sendable {
    var participantID: UUID { get }
    func renderAudioFrame(_ frame: RemoteCoOpHostAudioFrame)
}

public final class RemoteCoOpHostAudioRelay: @unchecked Sendable {
    private let lock = NSLock()
    private var sinks: [UUID: any RemoteCoOpHostAudioSink] = [:]

    public init() {}

    public func upsert(_ sink: any RemoteCoOpHostAudioSink) {
    ...
    public func renderAudioFrame(_ frame: RemoteCoOpHostAudioFrame) {
        let currentSinks = lock.withLock { Array(sinks.values) }
        for sink in currentSinks { sink.renderAudioFrame(frame) }
    }

    private static func audioFrame(from audioBufferList: UnsafePointer<AudioBufferList>, frameCount: UInt32, sampleRate: Double, channels: UInt32) -> RemoteCoOpHostAudioFrame? {
        guard let stereoSamples = stereoPCM(from: audioBufferList, frameCount: frameCount, channels: channels) else { return nil }
        let resampledSamples = resampledStereoPCM(stereoSamples, sourceSampleRate: sampleRate, targetSampleRate: RemoteCoOpHostAudioFrame.sampleRate)
        ...
        return RemoteCoOpHostAudioFrame(samples: data, frameCount: UInt32(resampledSamples.count / Int(RemoteCoOpHostAudioFrame.channels)))
    }
    ...
    private static func stereoPCM(from audioBufferList: UnsafePointer<AudioBufferList>, frameCount: UInt32, channels: UInt32) -> [Int16]? {
        ...
        var samples = [Int16](repeating: 0, count: outputFrames * Int(RemoteCoOpHostAudioFrame.channels))
    ...
    private static func resampledStereoPCM(_ samples: [Int16], sourceSampleRate: Double, targetSampleRate: Double) -> [Int16] {
        ...
        let sourceFrames = samples.count / Int(RemoteCoOpHostAudioFrame.channels)
        ...
        var output = [Int16](repeating: 0, count: targetFrames * Int(RemoteCoOpHostAudioFrame.channels))
        for frame in 0..<targetFrames {
            ...
            for channel in 0..<Int(RemoteCoOpHostAudioFrame.channels) {
...
@objc(RemoteCoOpHostAudioDevice)
final class RemoteCoOpHostAudioDevice: NSObject, RTCAudioDevice, @unchecked Sendable {
    private let lock = NSLock()
    private weak var delegate: RTCAudioDeviceDelegate?
    private var sampleIndex: UInt64 = 0

    private(set) var deviceInputSampleRate = RemoteCoOpHostAudioFrame.sampleRate
    private(set) var inputIOBufferDuration: TimeInterval = 0.01
    private(set) var inputNumberOfChannels = Int(RemoteCoOpHostAudioFrame.channels)
    private(set) var inputLatency: TimeInterval = 0
    private(set) var deviceOutputSampleRate = RemoteCoOpHostAudioFrame.sampleRate
    private(set) var outputIOBufferDuration: TimeInterval = 0.01
    private(set) var outputNumberOfChannels = Int(RemoteCoOpHostAudioFrame.channels)
    ...
    func renderAudioFrame(_ frame: RemoteCoOpHostAudioFrame) {
        guard frame.frameCount > 0, frame.channels == RemoteCoOpHostAudioFrame.channels else { return }
        ...
        frame.samples.withUnsafeBytes { bytes in
            ...
            var audioBufferList = AudioBufferList(
                mNumberBuffers: 1,
                mBuffers: AudioBuffer(mNumberChannels: RemoteCoOpHostAudioFrame.channels, mDataByteSize: UInt32(bytes.count), mData: UnsafeMutableRawPointer(mutating: baseAddress))
            )
