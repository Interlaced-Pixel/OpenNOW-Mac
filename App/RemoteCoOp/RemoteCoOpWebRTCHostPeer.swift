...
public struct RemoteCoOpWebRTCHostPeerFactory: RemoteCoOpHostPeerFactory {
    public init() {}
    ...
    public func makePeer(participantID: UUID,
        ...
        RemoteCoOpWebRTCHostPeer(participantID: participantID, networkConfiguration: networkConfiguration, qualityPreset: qualityPreset, latencyMode: latencyMode, callbacks: callbacks)
    }
}

public final class RemoteCoOpWebRTCHostPeer: NSObject, RemoteCoOpHostPeer, RemoteCoOpHostVideoSink, RemoteCoOpHostAudioSink, RTCPeerConnectionDelegate, RTCDataChannelDelegate, @unchecked Sendable {
    public let participantID: UUID
    private static let inputChannelLabel = "remote-coop-input"
    private let networkConfiguration: RemoteCoOpNetworkConfiguration
    private let qualityPreset: RemoteCoOpQualityPreset
    private let latencyMode: RemoteCoOpLatencyMode
    private let callbacks: RemoteCoOpHostPeerCallbacks
    ...
    private var audioDevice: RemoteCoOpHostAudioDevice?
    ...
    public init(participantID: UUID,
                ...
                qualityPreset: RemoteCoOpQualityPreset,
                latencyMode: RemoteCoOpLatencyMode,
                callbacks: RemoteCoOpHostPeerCallbacks) {
    ...
    public func apply(_ signal: RemoteCoOpWirePeerSignal) async throws {
        switch signal.kind {
        case .answer:
            guard let sdp = signal.sdp, !sdp.isEmpty else { throw RemoteCoOpHostPeerError.invalidSignal }
            try await setRemoteDescription(RTCSessionDescription(type: .answer, sdp: sdp))
        case .iceCandidate:
            guard let candidate = signal.candidate, !candidate.isEmpty else { throw RemoteCoOpHostPeerError.invalidSignal }
            try await addIceCandidate(RTCIceCandidate(sdp: candidate, sdpMLineIndex: Int32(signal.sdpMLineIndex ?? 0), sdpMid: signal.sdpMid))
        case .offer:
            throw RemoteCoOpHostPeerError.invalidSignal
        }
    }
    ...
    public func dataChannel(_ dataChannel: RTCDataChannel, didReceiveMessageWith buffer: RTCDataBuffer) {
        guard !closed else { return }
        let packets = RemoteCoOpHostPeerInputDecoder.decodePackets(buffer.data as Data, expectedParticipantID: participantID)
    ...
    private func makePeerConnection() throws -> RTCPeerConnection {
        ...
        let audioDevice = RemoteCoOpHostAudioDevice()
        ...
        try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<RTCSessionDescription, Error>) in
            peerConnection.offer(for: constraints) { offer, error in
                if let offer {
                    ...
                    continuation.resume(throwing: RemoteCoOpHostPeerError.negotiationFailed(error?.localizedDescription ?? "Unable to create Remote Co-Op WebRTC offer."))
                }
            }
        }
    }
    ...
    private func setLocalDescription(_ description: RTCSessionDescription, peerConnection: RTCPeerConnection) async throws {
        try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
            peerConnection.setLocalDescription(description) { error in
                if let error {
                    continuation.resume(throwing: RemoteCoOpHostPeerError.negotiationFailed(error.localizedDescription))
                } else {
                    continuation.resume()
                }
            }
    ...
    private func setRemoteDescription(_ description: RTCSessionDescription) async throws {
        guard let peerConnection = stateLock.withLock({ peerConnection }) else { throw RemoteCoOpHostPeerError.peerNotFound }
        try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
