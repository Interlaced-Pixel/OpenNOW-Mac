...
public enum RemoteCoOpInputRoutingResult: Equatable, Sendable {
...
public actor RemoteCoOpInputRouter {
    private var participantsByID: [UUID: RemoteCoOpParticipant] = [:]
    private var lastSequenceByParticipantID: [UUID: UInt64] = [:]

    public init(participants: [RemoteCoOpParticipant] = []) {
        for participant in participants {
            participantsByID[participant.id] = participant
        }
    }

    public func replaceParticipants(_ participants: [RemoteCoOpParticipant]) {
    ...
    public func upsertParticipant(_ participant: RemoteCoOpParticipant) {
        participantsByID[participant.id] = participant
    }
    ...
    public func route(_ packet: RemoteCoOpInputPacket, receivedAtNanoseconds: UInt64 = DispatchTime.now().uptimeNanoseconds) -> RemoteCoOpInputRoutingResult {
