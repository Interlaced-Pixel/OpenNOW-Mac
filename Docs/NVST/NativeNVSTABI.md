...
- `libGeronimo.dylib`, `libBifrost2.dylib`, `libGsAudioWebRTC.dylib`, and `SDL2.framework` use `@executable_path/../Frameworks` install names, matching PixelNOW.app embedding.
...
- Native NVST execution is arm64-only. The runtime loader rejects other host architectures, the bundle validator requires an arm64 slice, and Xcode CI targets `platform=macOS,arch=arm64`. The bundled WebRTC framework has no x86_64 slice, so Intel execution is not supported.
...
- Bifrost callback registration is single-assignment. During serialized `GridApp::initialize`, PixelNOW leaves the first SessionController client registration unchanged, intercepts creation of the second GridApp client, registers a callback that invokes `GridApp::onNVbCallback` first and then observes verified haptic events, acknowledges Geronimo's immediately following registration, and restores both imports. It does not attempt rejected post-initialization callback replacement and does not change Bifrost client ownership.
...
- Size used by PixelNOW shim: `0xa8` bytes.
...
- `0x6c`: converted server type. Synchronous initialization rejects `-1`; PixelNOW maps CloudMatch values `1...5` to `0...4` and `1001` to `0x33` and rejects every other value.
...
- Bifrost's verified connection-info storage contains 20 records. PixelNOW rejects converted counts above 20 rather than relying on larger Geronimo-side vector capacity.
...
- Geronimo expects `session.connectionInfo[].protocol` as a numeric transport enum. Disassembly verifies `2` maps to the UDP transfer path; emitting an empty string or `0` causes `getStreamStartParameters` to log `Unknown connection protocol detected in the SessionObject. Defaulting to UDP.` PixelNOW defaults missing protocol values to numeric `2` and keeps HTTP/HTTPS scheme selection in `appLevelProtocol`.
- Geronimo's mode-selection deserializer requires `selectedFeatures.prefilterParams.denoiseLevel` and `selectedFeatures.hudStreamingParams.scxQpDelta` to be floating-point JSON numbers. Swift `JSONSerialization` emits integral `Double` values as integers, and the Geronimo parser probe reproduces `0x80f10005` when these fields are `0` instead of `0.0`; PixelNOW post-processes generated mode JSON so integral values remain floating-point literals.
...
- `Nsk::convertToStreamingParams(StreamStartParameters, VideoDecoderInitParams, NVbStreamingParams_t)` and `Nsk::free(NVbStreamingParams_t&)` are private/non-external symbols in the current Geronimo build. PixelNOW resolves them from the `getStreamStartParameters` image base using verified arm64 text offsets `0x8a060` and `0x89a88`. Historical x86_64 offsets `0x9d740` and `0x9d3a0` remain recorded for ABI provenance, but Intel native NVST execution is unsupported.
...
- NVIDIA's pinned `SimpleGridApp::setupVideoDecoder` passes `0` as the second argument to `platformCreateVideoDecoder`; that parameter is not the negotiated codec. The codec is supplied later at `PlatformDecoderSettings + 0x0c` during decoder initialization. `SimpleGridApp::setDecoderDetails` forwards the negotiated dynamic-streaming mode as argument `x5` to `GridApp::setDecoderInfo`, so PixelNOW preserves `selectedFeatures.dynamicStreamingMode` instead of forcing mode `0`.
- `0x00`: app id.
...
- `0xb8`: application identifier `std::string`; PixelNOW sends `GFN-PC`.
- `0xd0`: application version `std::string`; PixelNOW sends `30.0`.
...
- PixelNOW preserves the raw CloudMatch session JSON for the native path so a future C++ shim can build `SessionControl::SessionParameters` without losing NVIDIA-specific fields.
...
- Geronimo's `getStreamStartParameters` parser expects the normalized session JSON to include a `streamingProfile` object. PixelNOW carries any NVIDIA-provided `streamingProfileGuid` from CloudMatch/session-info/settings; when none is present for PixelNOW's local custom profile, it persists a client-side UUID per app/profile signature and sends that as `streamingProfile.streamingProfileGuid`.
- CloudMatch raw session JSON can omit `tokenType` and `token`. PixelNOW must keep those secrets out of raw/persisted JSON and pass allocation auth as transient C strings into the Geronimo shim before `GridApp::setAuthInfo`.
...
- Geronimo/Bifrost deep-copies every `0x170`-byte `NVbStreamSettings_t` during startup. PixelNOW may therefore apply a server-measured packet size to the converted settings before `GridApp::start` or `GridApp::resume`.
...
- PixelNOW must not call `_nvbTestNetworkCapabilityAsync` until the provisioned UDP host and all native threshold mappings are present in an observed dispatcher contract. Passing the known HTTP endpoint would target the wrong protocol.
...
- Exported video/render symbols include `VideoDecoderSet::*`, `VTDecoder::*`, `SDLWindow::getNativeWindow`, `SDLWindow::getWindowHandle`, and `SDLWindowManager::createWindow`, but PixelNOW has no exported `GridApp` API to retrieve the active `SDLWindow *`, `IOInterface *`, `NSView`, `CAMetalLayer`, or decoded frame callback.
...
- The vendored SDL 2.32.10 Cocoa implementation accepts either an `NSWindow *` or `NSView *`, but SDL Metal always inserts its renderer into the owning window's content view. PixelNOW passes a transparent borderless proxy `NSWindow`, then reparents SDL's `CAMetalLayer`-backed view into the real AppKit video surface. The proxy remains attached only for SDL window lifecycle and drawable-size events; no visible renderer window overlaps the app content.
- `SDLWindow::initWindow(SDLWindow::InitParams const&)` reads `SDLWindow::InitParams + 0x70` as the create-from handle. When non-null, Geronimo calls `WindowNativeEventHandler::setCreateFromHandle(handle)` and then `_SDL_CreateWindowFrom(handle)` instead of creating a standalone SDL window. PixelNOW supplies and retains the transparent proxy `NSWindow` for the complete native session; passing a SwiftUI-owned `NSView` directly is unsafe because SDL Metal targets `NSWindow.contentView` rather than the supplied view.
...
- PixelNOW does not mutate `_ZTV16SDLWindowManager` or any other process-global vendor vtable. After prepare succeeds, the per-session shim directly constructs `SDLGraphicsContext`, `SDLEventProcessor`, and one `SDLWindow`, passing the stored native handle at `SDLWindow::InitParams + 0x70`.
- `SDLEventProcessor::InitParams + 0x03` selects SDL input when nonzero. PixelNOW enables this mode to prevent Geronimo's redundant `CocoaInputControllerImpl` from calling AppKit on `InputController.PostQ`; PixelNOW submits keyboard, pointer, text, and gamepad events through `GridApp::sendNvstInputEvent`.
- `SDLWindow::InitParams + 0x1c/+0x20` are the window width and height. Either value being zero makes `SDLWindow::initWindow` call `setFullscreen(true)`, which is invalid for an embedded host window. PixelNOW copies the proxy window's positive content dimensions into these fields and keeps that proxy synchronized with the embedded video surface. `+0x28` is the borrowed title C string, `+0x38` is the high-DPI flag, `+0x70` is the create-from handle, and `+0x78` enables async rendering. The embedded Metal path enables `+0x38` and declares `NSHighResolutionCapable`; disabling Retina rendering produces a half-scale viewport in the top-left quadrant. Zeroed `+0x48/+0x50` is a valid empty `std::shared_ptr`.
- `VideoDecoder::initialize` is virtual at decoder address-point slot `+0x38`. A `VTDecoder` must be initialized through that slot so `VTDecoder::initialize` creates its codec-specific decode-unit transformer at `VTDecoder + 0x250`; calling the exported base implementation directly leaves that member null and crashes on the first `VTDecoder::decode` call. PixelNOW verifies the slot against the exported `VTDecoder::initialize` symbol before invoking it.
- Decoder creation and initialization receive the requested or setup-negotiated NVB codec enum unchanged: H.264 is `1`, HEVC is `2`, and AV1 is `4`. PixelNOW now rejects H.265 or AV1 before native session creation when VideoToolbox capability checks fail. Setup success remains authoritative and recreates a decoder when its negotiated enum differs from the setup-time request.
...
- `NativeNVSTGeronimoSetVideoSurface(session, nativeHandle, ...)` stores the PixelNOW proxy `NSWindow` handle before window construction and emits native phase `22`.
- Exported audio symbols include `platformCreateAudioCapturer`, `platformCreateAudioRenderer`, `IOInterface::registerAudioCallback`, `IOInterface::registerAudioCaptureCallback`, `GridApp::sendMicAudioFrame`, `SDLAudio::renderAudio`, and `WebRTCAudioCapturer::*`; PixelNOW has no exported `GridApp` API to retrieve the active `IOInterface *` required for callback registration. Game-volume/mute control therefore remains outside the verified private ABI and must not be implemented by changing global macOS output volume.
- `SDLAudio::renderAudio` reads the incoming `NvstAudioFrame_t` channel count at `+0x10`. When it differs from the renderer state at `SDLAudio + 0x1a8`, the vendor implementation stores the incoming count, closes the SDL audio device, refreshes default-device information, resizes its queue, and calls `openSDLSource` again. PixelNOW therefore leaves `SessionParameters +0x233` at the vendor stereo baseline instead of forcing requested 5.1/7.1 bytes; incoming 6- and 8-channel frames drive the verified reopen path.
- A non-disabled microphone mode creates the Geronimo audio capturer even when it starts muted. `WebRTCAudioCapturer` address-point slots `+0x40` and `+0x48` pause and resume capture; `NativeNVSTGeronimoSetMicrophoneEnabled` invokes those slots on the main thread while streaming, matching the existing streaming-begin lifecycle.
...
- The verified canonical microphone descriptor has bits per sample `16` at `+0x08`, sample rate `48000` at `+0x0c`, channel count `2` at `+0x10`, format `4` at `+0x14`, PCM pointer at `+0x28`, and PCM byte count `1920` at `+0x30`. This is 480 interleaved stereo samples, or 10 ms. PixelNOW never processes any other descriptor.
...
- Supported frames are copied into thread-local descriptor and PCM storage. PixelNOW changes only the copied PCM pointer, preserving every descriptor field, side-data pointer, timestamp, and release field. Unsupported frames call the original import unchanged. VAD silence is forwarded as a full zeroed frame rather than dropped.
...
- libGeronimo exposes native stream-provider recording primitives, but PixelNOW's base `GridApp` session has no registered `IStreamRecorder` sink or file writer. Native NVST therefore reports recording as unavailable instead of swallowing `⌘R` or pretending a recording started.
...
- Gamepad event type `18` stores signed 16-bit controls from `+0x08`, source index at `+0x3e`, and microsecond timestamp at `+0x40`. PixelNOW registers each source once through `GridApp::handleGamepadChanged` before sending its first event.
...
- Native Geronimo input does not use PixelNOW's WebRTC data-channel envelope or partially reliable transfer flags.
...
- `GridApp::onCursorInfoUpdate(CursorInfo const&)` is the verified vtable slot at address-point offset `0x118`. PixelNOW calls NVIDIA's original handler before observing cursor state `1` as visible/client-side and state `2` as hidden/relative. Startup verifies the slot against the exact exported symbol and fails closed if the vendor ABI changes.
- Visible server cursor state selects absolute input. The vendor never confines or warps the local cursor in this mode; it leaves the macOS cursor associated and free, and clamps only the transmitted coordinates into the rendered frame (the vendor client clamps `sm/nm` to the video extent before sending). PixelNOW matches this: AppKit positions are converted into the native SDL window's top-left-origin logical coordinates, then the shim calls the exported `SDLWindow::convertPointToVideoFrame` for NVIDIA's exact live decoded-aspect, viewport, resize, clamping, and reference-extent behavior, and positions in letterbox or pillarbox regions clamp to the nearest rendered-frame edge. Geronimo remains responsible for applying server-provided system and bitmap cursor images to the local hardware cursor.
- Hidden server cursor state selects relative input when Direct Mouse Input is enabled. Geronimo's `SDLCursorManager` only engages `SDL_SetRelativeMouseMode` when `SDL_GetKeyboardFocus()` matches the SDL window created from PixelNOW's transparent proxy `NSWindow`. PixelNOW therefore routes keyboard focus to that proxy during relative mode while AppKit input and pointer lock remain on the parent stream view. Pointer lock stores the pre-capture global cursor position, dissociates physical mouse movement from the macOS cursor, hides the local cursor, and consumes AppKit movement, drag, and wheel events through one local monitor. Unlock reverses those steps and restores the saved cursor position. Because PixelNOW injects input through AppKit rather than Geronimo's SDL event loop, the shim still clears Geronimo's SDL window grab and mouse-rect confinement fallback after each `CursorInfo` update, but it leaves SDL relative mode untouched for hidden-cursor state `2` so vendor focus checks can keep relative capture enabled.
...
- Precise trackpad scrolling accumulates fractional deltas until a complete NVST wheel detent exists. Adjacent same-direction wheel packets coalesce without crossing movement or button edges, preventing tiny trackpad deltas from becoming full detents and reducing wheel loss under bounded input backpressure while preserving reversal order.
...
- `NativeNVSTGeronimoCopyPerformanceStats` reads this synchronized state while the session is streaming and returns only normalized scalar values plus the zone or region string. It does not expose the opaque native stats object to Swift.
- `Cmd+N` toggles PixelNOW's SwiftUI GFN-style panel and never calls Geronimo's internal performance indicator, which is restricted for external users and may render nothing.
...
- `SessionSetUpFailureInfo + 0x00` is the `NVbResultCode_t` delivered to Geronimo lifecycle callbacks. The object also owns strings at `+0x18`, `+0x30`, and `+0x60` and a vector at `+0x48`; PixelNOW does not inspect those opaque fields because their semantics and sensitivity are not verified.
- PixelNOW resolves `nvbEnumToString` from its owned Bifrost handle and synchronously forwards only the static `NVB_R_*` name with callback telemetry. Swift copies the borrowed C string before callback return.
...
- PixelNOW emits native phases `40`, `44`, and `46` after its start-delivered, streaming-began, and setup-succeeded readiness gates respectively, allowing timeout diagnostics to identify the missing gate.
- PixelNOW treats `callbackType=2`, `clientEvent=14`, `notification=1` as the native readiness gate for `NativeNVSTBifrostTransport.connect`.
- Session notification values in the `50...200` range enter Geronimo's streaming-end path and are treated by PixelNOW as terminal if observed before readiness.
- `_ZTV7GridApp` spans `0x220` bytes through the next symbol. The object address point is `vtable + 0x10`. PixelNOW verifies the original slot values before installing a per-session clone.
...
- `SessionTerminationInfo` stores termination reason at `+0x00`, extended code at `+0x04`, resumable at `+0x40`, and session-alive at `+0x41`. PixelNOW forwards reason and extended code as native phase `62` and suppresses that terminal event when a local stop is already pending.
- `GridApp::onPrepareResult` queues delivery under `GridApp + 0x430`; `GridApp::processEvents()` swaps and dispatches that queue on its caller. PixelNOW initializes the SDL/audio/video path after successful prepare delivery and before publishing native phase `30` or calling `GridApp::start`.
- `NativeNVSTGeronimoPump` must run on the main thread for the lifetime of the session. Its integer argument is the `SDL_WaitEventTimeout` duration in milliseconds; `0` performs nonblocking SDL polling. The shim forces a zero timeout while stopping so Geronimo can deliver the stop callback without blocking AppKit. A false `SDLEventProcessor::processEvents` result is terminal.
- The vendor main loop dispatches `GridApp::processEvents()` before SDL processing and services SDL at render-loop cadence with a maximum 16 ms wait. PixelNOW preserves that order and uses a nonblocking 60 Hz timer in the default AppKit run-loop mode. SDL's Cocoa event pump dequeues and redispatches `NSEvent`s, so the timer must yield during title-bar, menu, SwiftUI button, and other event-tracking loops rather than competing with AppKit for input delivery.
...
- Swift retains externally visible connection ownership until callback handling, pump invalidation, and native destruction have all completed during normal lifecycle operations. A new connect cannot enter while disconnect, recovery reset, or successful pause destruction is in progress. Application termination may release that ownership after stop acknowledgement, pump invalidation, and surface preparation while native destruction continues on a background cleanup task.
- Production recovery permits exactly one same-session attachment after an established stream terminates with both session-alive and resumable flags and a verified transient reason. Launch failures, permanent failures, and transport failures are never retried.
...
- `GridApp::updateAuthToken` exposes a response pointer but no verified writable capacity in the pinned ABI. PixelNOW writes only the empty-string byte required by the callback and returns immediately; it does not invoke asynchronous auth refresh from the main-thread pump or assume a fabricated 16 KiB destination. Native refresh remains fail-closed until a capacity field or another bounded ownership contract is verified.
- `GridAppD2` calls `GridApp::uninitialize()` internally. PixelNOW must not call uninitialize a second time, and it keeps the process platform and both dylib handles alive until the destructor returns.
- Destruction restores the embedded Metal view to the retained proxy window, detaches the per-session callback owner and waits for in-flight callbacks, rejects every callback without a currently registered owner, destroys shim-owned media, runs `GridAppD2`, releases the video surface and reference-counted process platform, frees the cloned vtable, and closes the dylib handles.
- PixelNOW polls Geronimo's native stream-frame rate together with the embedded Metal surface. It fails a connected stream when the renderer remains unavailable, initial native frames do not arrive within the bounded grace period, or native stream FPS remains zero after delivery has started.
