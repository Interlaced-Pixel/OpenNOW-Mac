...
- OpenNOW captures keyboard events only when their target is the stream window, then consumes its explicit stream commands and remote paste shortcut before forwarding input. Menu-panel events and other Command-modified key equivalents remain local so AppKit menus and standard window commands continue to work during a stream; a key pressed remotely before becoming Command-modified is released before local routing.
