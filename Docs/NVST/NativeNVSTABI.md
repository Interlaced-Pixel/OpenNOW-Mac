...
- The vendored SDL 2.32.10 Cocoa implementation accepts either an `NSWindow *` or `NSView *`. Passing an `NSView *` makes that view SDL's `sdlContentView`; OpenNOW passes its represented video-surface view so SDL does not insert renderer views into SwiftUI's `NSHostingController.view`.
...
- `OpenNOWNativeNVSTGeronimoSetVideoSurface(session, nativeHandle, ...)` stores the OpenNOW video-surface `NSView` handle before window construction and emits native phase `22`.
