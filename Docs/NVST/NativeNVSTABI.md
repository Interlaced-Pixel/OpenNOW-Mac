...
- `SDLWindow::initWindow(SDLWindow::InitParams const&)` reads `SDLWindow::InitParams + 0x70` as the create-from handle. When non-null, Geronimo calls `WindowNativeEventHandler::setCreateFromHandle(handle)` and then `_SDL_CreateWindowFrom(handle)` instead of creating a standalone SDL window. OpenNOW supplies the transparent proxy `NSWindow`; passing a SwiftUI-owned `NSView` directly is unsafe because SDL Metal targets `NSWindow.contentView` rather than the supplied view.
