...
- Decoder creation and initialization receive the requested or setup-negotiated NVB codec enum unchanged: H.264 is `1`, HEVC is `2`, and AV1 is `4`. OpenNOW now rejects H.265 or AV1 before native session creation when VideoToolbox capability checks fail. Setup success remains authoritative and recreates a decoder when its negotiated enum differs from the setup-time request.
...
- `OpenNOWNativeNVSTGeronimoPump` must run on the main thread for the lifetime of the session. Its integer argument is the `SDL_WaitEventTimeout` duration in milliseconds; `0` performs nonblocking SDL polling. The shim forces a zero timeout while stopping so Geronimo can deliver the stop callback without blocking AppKit. A false `SDLEventProcessor::processEvents` result is terminal.
- The vendor main loop dispatches `GridApp::processEvents()` before SDL processing and services SDL at render-loop cadence with a maximum 16 ms wait. OpenNOW preserves that order and uses a nonblocking 60 Hz timer in the default AppKit run-loop mode. SDL's Cocoa event pump dequeues and redispatches `NSEvent`s, so the timer must yield during title-bar, menu, SwiftUI button, and other event-tracking loops rather than competing with AppKit for input delivery.
...
- OpenNOW polls Geronimo's native stream-frame rate together with the embedded Metal surface. It fails a connected stream when the renderer remains unavailable, initial native frames do not arrive within the bounded grace period, or native stream FPS remains zero after delivery has started.
