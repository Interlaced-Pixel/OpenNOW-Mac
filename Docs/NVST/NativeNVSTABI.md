...
- The vendored SDL 2.32.10 Cocoa implementation accepts either an `NSWindow *` or `NSView *`, but SDL Metal attaches its renderer to the owning window's content view. OpenNOW therefore passes a dedicated borderless child `NSWindow` instead of a SwiftUI-owned view.
- `SDLWindow::initWindow(SDLWindow::InitParams const&)` reads `SDLWindow::InitParams + 0x70` as the create-from handle. When non-null, Geronimo calls `WindowNativeEventHandler::setCreateFromHandle(handle)` and then `_SDL_CreateWindowFrom(handle)` instead of creating a standalone SDL window. OpenNOW supplies a dedicated borderless child `NSWindow`; SDL Metal always inserts its view into `NSWindow.contentView`, so passing a SwiftUI-owned `NSView` is not safe.
- Native SwiftUI controls use a transparent key-capable sibling child window ordered above the non-key Geronimo renderer window. Parent-window SwiftUI content cannot overdraw an above-parent renderer child window.
...
- `SDLWindow::InitParams + 0x1c/+0x20` are the window width and height. Either value being zero makes `SDLWindow::initWindow` call `setFullscreen(true)`, which is invalid for an embedded host window. OpenNOW copies the dedicated renderer window's positive content dimensions into these fields. `+0x28` is the borrowed title C string, `+0x38` is the high-DPI flag, `+0x70` is the create-from handle, and `+0x78` enables async rendering. The embedded Metal path keeps `+0x38` disabled because Geronimo renders with logical SDL window dimensions; enabling a Retina drawable produces a viewport half the required width and height. Zeroed `+0x48/+0x50` is a valid empty `std::shared_ptr`.
...
- `OpenNOWNativeNVSTGeronimoSetVideoSurface(session, nativeHandle, ...)` stores the OpenNOW renderer `NSWindow` handle before window construction and emits native phase `22`.
...
- Destruction detaches the per-session callback owner and waits for in-flight callbacks, destroys shim-owned media, runs `GridAppD2`, releases the reference-counted process platform, frees the cloned vtable, and closes the dylib handles.
