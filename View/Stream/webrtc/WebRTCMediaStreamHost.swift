...
enum WebRTCStreamLaunchLoadingStage {
...
extension PreparedLaunchConfiguration {
    var webRTCLoadingArtworkURL: URL? {
...
struct WebRTCStreamLaunchLoadingScreen<Accessory: View>: View {
    let title: String
    let stage: String
    let artworkURL: URL?
            ...
            ZStack {
                ...
                VStack(spacing: compact ? 16 : 22) {
                    ...
                    if accessoryPresented {
                        ...
                        WebRTCStreamLaunchSignal(reduceMotion: reduceMotion)
...
private struct WebRTCStreamLaunchSignal: View {
...
struct WebRTCMediaStreamHostView: View {
    ...
    init(configuration: PreparedLaunchConfiguration,
        ...
        coordinator = StreamSessionCoordinator(
            adPresenter: WebRTCInlineStreamSessionAdPresenter(handler: onRequiredSessionAd),
...
private struct WebRTCInlineStreamSessionAdPresenter: StreamSessionAdPresenter {
    ...
    func playRequiredSessionAd(_ ad: StreamSessionAdPresentation) async throws -> Int {
        guard let handler else {
            throw StreamSessionError.sessionAllocationFailed("Required ad playback is not available.")
        }
