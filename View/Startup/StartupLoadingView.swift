    ...
    var body: some View {
        GeometryReader { proxy in
            ...
            TimelineView(.animation) { timeline in
                ...
                let progress = reduceMotion ? 1 : startupClamp(elapsed / StartupAnimation.duration)
                let pulse = reduceMotion ? 0.5 : elapsed.truncatingRemainder(dividingBy: 1.8) / 1.8

                ZStack {
                    StartupBackdrop(progress: progress, pulse: pulse)

                    VStack(spacing: 0) {
                        Spacer()

                        StartupMark(progress: progress, pulse: pulse, compact: compact, reduceMotion: reduceMotion)
                        ...
                        StartupSignalRail(progress: progress, pulse: pulse)
                ...
                .accessibilityLabel("PixelNOW is starting")
...
private struct StartupBackdrop: View {
    let progress: Double
    let pulse: Double
...
private struct StartupMark: View {
    let progress: Double
    let pulse: Double
    let compact: Bool
...
private struct StartupSignalRail: View {
    let progress: Double
    let pulse: Double
